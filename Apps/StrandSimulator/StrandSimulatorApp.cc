...
void idle()
{
        ...
        if( g_render )
        {
            ...
            if(mojave_counter < 2) glutReshapeWindow(g_window_width - 1 + mojave_counter++, g_window_height);
#endif
            glutPostRedisplay();
        }
}
...
Vec3d calcSimCenter()
{
    ...
    // TODO:
//    for( std::vector<RenderBase*>::size_type i = 0; i < renderable_objects.size(); ++i )
//    {
//        center += renderable_objects[i]->calculateObjectCenter();
//    }
//    
//    if( !renderable_objects.empty() ) center /= ( (double) renderable_objects.size() );
...
//        const Scalar r = renderable_objects[i]->calculateObjectBoundingRadius( center );
...
{
    const bool zooming = (button == GLUT_RIGHT_BUTTON) || ((button == GLUT_LEFT_BUTTON) && (glutGetModifiers() & GLUT_ACTIVE_CTRL));
...
void cleanup()
{
    for( std::vector<ProblemStepper*>::size_type i = 1; i < problems.size(); ++i )
    {
...
int parseCommandLine( int argc, char** argv )
{
    ...
    try
    {
        TCLAP::CmdLine cmd("A Multi-Scale Model for Coupling Strands with Shear-Dependent Liquid");
        ...
        TCLAP::ValueArg<int> statlog( "l", "statlog", "Log runtime stats: 0: no stats, 1: timestep stats, 2: all stats", false, 0, &statLoggingLevels, cmd );
...
void printCommandLineSplashScreen()
{
    ...
    std::cout << "# problem: " << problems[g_problem_idx]->ProblemName() << std::endl;
    std::cout << "\033[m";
}
