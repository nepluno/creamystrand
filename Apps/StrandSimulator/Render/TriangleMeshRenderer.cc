...
namespace strandsim
{
    ...
    TriangleMeshRenderer::TriangleMeshRenderer( TriangularMesh& mesh, int num_components )
    : m_mesh(mesh)
    , m_mode(FLAT)
    , m_controller(NULL)
    {
    ...
    void TriangleMeshRenderer::setFluidController( const std::shared_ptr< FluidScriptingController >& controller )
    {
...
//                glPolygonMode( GL_FRONT_AND_BACK, GL_LINE );
                const VecXx& flow_height = m_mesh.m_associatedFlow->getCurrentFlowHeight();
...
//                        accum += flow_height(vidx);
                    ...
                    for(int r=0; r < 3; ++r) {
                        ...
                        if(!m_controller) {
                            GLfloat blue[] = {0.0f, 0.0f, 0.8f, (float) flow_height( idx_vert ) / 0.1f};
                            ...
                            GLfloat blue[] = {(GLfloat)color(0), (GLfloat)color(1), (GLfloat)color(2), (GLfloat) flow_height( idx_vert ) / 0.1f};
                            glMaterialfv(GL_FRONT_AND_BACK,GL_AMBIENT_AND_DIFFUSE,blue);
                        }
...
//                glPolygonMode( GL_FRONT_AND_BACK, GL_FILL );
                ...
                for(int i = 0; i < num_faces; ++i)
                {
                    ...
                    const Vec3x center = (m_mesh.getVertex(f(0)) + m_mesh.m_vertex_normals[f(0)] * flow_height(f(0)) +
                                          m_mesh.getVertex(f(1)) + m_mesh.m_vertex_normals[f(1)] * flow_height(f(1)) +
                                          m_mesh.getVertex(f(2)) + m_mesh.m_vertex_normals[f(2)] * flow_height(f(2))) / 3.0;
                    const Vec3x vel = m_mesh.m_associatedFlow->getCurrentFlowVelocity().segment<3>(i * 3);
