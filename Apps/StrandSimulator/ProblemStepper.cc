...
ProblemStepper::~ProblemStepper()
{
  ...
  for( std::vector<strandsim::ElasticStrand*>::size_type i = 0; i < m_strands.size(); ++i )
  {
    if( m_strands[i] != NULL )
    {
      delete m_strands[i];
  ...
  for( std::vector<strandsim::TriangleMeshRenderer*>::size_type i = 0; i < m_mesh_renderers.size(); ++i )
  {
  ...
  for( std::vector<strandsim::FluidsRenderer*>::size_type i = 0; i < m_fluids_renderers.size(); ++i )
  {
  ...
  for( std::vector<DOFScriptingController*>::size_type i = 0; i < m_dof_scripting_controllers.size(); ++i )
  {
...
bool ProblemStepper::step()
{
    ...
    {
        std::cout << "\n# Stepping [" << i_step << "/" << num_substeps << "] @ time: " << m_t << " with " << dt_total << " (max: " << dt_cfl << ")" << std::endl;
        m_t += dt_total;
        
        if(!executeScript(i_step, dt_total)) return false;
        
        if( m_isSimulated )
            m_stepper->execute(num_substeps, i_step, dt_total);
    }
...
void  ProblemStepper::render()
{
	...
	for(auto m_itr = m_fluids_renderers.begin(); m_itr != m_fluids_renderers.end(); ++ m_itr)
    {
        (*m_itr)->render();
    }
}
...
void ProblemStepper::transformTriangleObject( TriangularMesh& triMesh, const Mat3x& transformation, const Vec3x& center, const Vec3x& translate )
{
...
void ProblemStepper::transformRodRootVtx( RodData& rd, const Mat3x& transformation, const Vec3x& center, const Vec3x& translate, const Vec3x& scale, int vtx )
{
    if(m_simulation_params.m_useSoftAttachConstraints) {
        Vec3x vert = rd.getDofController().getVertexGoal(vtx);//rd.getStrand().getVertex( vtx );
        Vec3x vertNext = transformation * Vec3x((vert - center).array() * scale.array()) + center + translate;
        rd.getDofController().setVertexGoal(vtx, vertNext);
    } else {
        Vec3x vert = rd.getStrand().getVertex( vtx );
        Vec3x vertNext = transformation * Vec3x((vert - center).array() * scale.array()) + center + translate;
        rd.getDofController().setVertexDisplacement( vtx, (vertNext - vert) / (Scalar) m_simulation_params.m_subSteps );
    }
}

void ProblemStepper::transformTriangleObject( TriangularMesh& triMesh, const Eigen::Quaternion<double>& transformation, const Vec3x& center, const Vec3x& translate )
{
    for( unsigned i = 0; i < triMesh.nv(); ++i )
    {
        Vec3x vert = triMesh.getVertex(i);
...
void ProblemStepper::transformRodRootVtx( RodData& rd, const Eigen::Quaternion<double>& transformation, const Vec3x& center, const Vec3x& translate, const Vec3x& scale, int vtx )
{
    if(m_simulation_params.m_useSoftAttachConstraints) {
...
void ProblemStepper::dumpFluid(DumpData* data, std::string outputdirectory, int current_frame, int file_width) const
{
    int mesh_num = 0;
    for(auto m_itr = m_fluidScripting_controllers.begin(); m_itr != m_fluidScripting_controllers.end(); ++ m_itr, ++ mesh_num)
    {
        // new obj per mesh
        std::stringstream name;
        name << std::setfill('0');
        name << outputdirectory << "/fluid" <<  mesh_num << "_" << std::setw(file_width) << current_frame << ".ply";
...
void ProblemStepper::dumpMesh(DumpData* data, std::string outputdirectory, int current_frame, int file_width) const
{
    int mesh_num = 0;
    ...
    for(auto m_itr = m_meshScripting_controllers.begin(); m_itr != m_meshScripting_controllers.end(); ++ m_itr, ++ mesh_num)
    {
        ...
        std::stringstream name;
        name << std::setfill('0');
        name << outputdirectory << "/mesh" <<  mesh_num << "_" << std::setw(file_width) << current_frame << ".ply";
        ...
        if(flow) {
            ...
            for ( int v = 0; v < nv; ++v )
            {
                ...
                data->mesh_colors[mesh_num][v] = flow_colors.segment(v * m_num_components, m_num_components);
            }
        } else {
        ...
        for ( int f = 0; f < nf; ++f )
        {
            data->mesh_indices[mesh_num][f] = Vec3i(mesh->getFace(f).idx[0], mesh->getFace(f).idx[1], mesh->getFace(f).idx[2]);
        }
    }
}
...
void ProblemStepper::dumpBinaryCheckpoint(std::string outputdirectory, int current_frame, int current_checkpoint, int file_width) const
{
}

void ProblemStepper::dumpRods(DumpData* data, std::string outputdirectory, int current_frame, int file_width) const
{
    std::stringstream name;
    name << std::setfill('0');
    ...
    // we add extra 2x2 vertices before first and after last, to have a nice cap for the flow during reconstruction
    ...
    for(auto rd_itr = m_rodDatum.begin(); rd_itr != m_rodDatum.end(); ++rd_itr, ++rod_num)
    {
        ...
        VecXx c0 = strand.getStepper()->flowComponents().segment(0, m_num_components);
        ...
        for (int j = 0; j < n; ++j)
        {
            ...
            data->rod_indices.push_back( rod_num );
            data->rod_radius.push_back( Vec2x( strand.getRadiusA(j),  strand.getRadiusB(j) ) );
            data->rod_flow_height.push_back( strand.getCurrentFlowHeight(j) );
            data->rod_groups.push_back( strand.getParameters().getParamIndex() );
            data->rod_actual.push_back( 1 );
            data->rod_colors.push_back( VecXx(strand.getStepper()->flowComponents().segment(j * m_num_components, m_num_components)) );
        }
        ...
        VecXx c1 = strand.getStepper()->flowComponents().segment((n - 1) * m_num_components, m_num_components);
...
void ProblemStepper::projectConstraint(const std::vector<ImplicitStepper*>& steppers)
{
}
...
void dump_data_subprog( DumpData* data )
{
    ...
    // header
    os_fluid << "ply" << std::endl << "format ascii 1.0" <<std::endl << "comment created by ADONIS" << std::endl;
    ...
    os_fluid << "element face 0"<< std::endl << "property list int int vertex_indices" << std::endl << "end_header " << std::endl;

    const int numv = (int) data->fluid_particles.size();
    for ( int v = 0; v < numv; ++v )
    {
        const Vec3x& p = data->fluid_particles[v];
        os_fluid << p(0) << " " << p(1) << " " << p(2) << " " << data->fluid_radius[v] << " ";
    ...
    for(int mesh_num = 0; mesh_num < num_meshes; ++mesh_num)
    {
        ...
        os << "ply" << std::endl << "format ascii 1.0" << std::endl << "comment created by ADONIS" << std::endl;
        ...
        << "property list int int vertex_indices" << std::endl << "end_header " << std::endl;
        
        for ( int v = 0; v < num_verts; ++v )
        {
            os << data->mesh_vertices[mesh_num][v](0) << " " << data->mesh_vertices[mesh_num][v](1) << " " << data->mesh_vertices[mesh_num][v](2) << " " << data->mesh_flow_height[mesh_num][v] << " ";
        ...
        {
            os << "3 " << data->mesh_indices[mesh_num][f](0) << " " << data->mesh_indices[mesh_num][f](1) << " " << data->mesh_indices[mesh_num][f](2) << std::endl;
        }
    ...
    // header
    os_rod << "ply" << std::endl << "format ascii 1.0" <<std::endl << "comment created by ADONIS" << std::endl;
