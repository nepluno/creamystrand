...
public:
    ...
    RodData( strandsim::ElasticStrand& strand, DOFScriptingController& controller, int num_components ) :
    m_strand( strand ),
    m_controller( controller )
    {
        m_renderer = new strandsim::StrandRenderer( m_strand, m_currentVertices,
                                                   m_currentMaterialFrames, m_strandMutex, num_components );
    ...
    void updateDrawingData()
    {
        ...
        Eigen::Map<Eigen::VectorXf> vert( &m_currentVertices[0], m_currentVertices.size() );
        m_strand.getCurrentVertices( vert );
        
        const Vec3xArray& materialFrames1 = m_strand.getCurrentMaterialFrames1();
        int i = 0;
        for ( auto mf1 = materialFrames1.begin(); mf1 != materialFrames1.end(); ++mf1, i += 3 )
        {
    ...
    Vec3x calculateObjectCenter()
    {
        return m_renderer->calculateObjectCenter();
    }
    ...
    DOFScriptingController& getDofController()
    {
        return m_controller;
    }
...
class ProblemStepper : public strandsim::StrandImplicitManager::SubStepCallback
{
public:
    virtual void executeCallback();
    
    explicit ProblemStepper( const std::string& name = "", const std::string& desc = "" );
    
    virtual ~ProblemStepper();
    
    void isSimulated( bool isSimulated )
    {
    ...
    // output
    void dumpMesh(DumpData* data, std::string outputdirectory, int current_frame, int file_width) const;
    void dumpRods(DumpData* data, std::string outputdirectory, int current_frame, int file_width) const;
    void dumpFluid(DumpData* data, std::string outputdirectory, int current_frame, int file_width) const;
    
    void dumpData(std::string outputdirectory, int current_frame, int file_width) const;
    
    virtual void dumpBinaryCheckpoint(std::string outputdirectory, int current_frame, int current_checkpoint, int file_width) const;
protected:
    ...
    virtual bool executeScript(int total_substep_id, const Scalar total_substep_dt) = 0; // execute scripting and any per-step updating for problem
    
    virtual void setRodCollisionParameters( CollisionParameters& param, const ElasticStrandParameters& strand_param ) = 0;
    ...
    std::map<std::pair<int, int>, std::set< std::pair<int, int> > > m_collision_free;
    
    std::vector<RodData*> m_rodDatum;
    
    std::vector< std::shared_ptr<strandsim::MeshScriptingController> > m_meshScripting_controllers;
    
    std::vector< std::shared_ptr<FluidScriptingController> > m_fluidScripting_controllers;
    ...
    // transform utilities
    void transformTriangleObject( TriangularMesh& triMesh, const Mat3x& transformation, const Vec3x& center, const Vec3x& translate );
    void transformRodRootVtx( RodData& rd, const Mat3x& transformation, const Vec3x& center, const Vec3x& translate, const Vec3x& scale, int vtx );
    
    void transformTriangleObject( TriangularMesh& triMesh, const Eigen::Quaternion<double>& transformation, const Vec3x& center, const Vec3x& translate );
    void transformRodRootVtx( RodData& rd, const Eigen::Quaternion<double>& transformation, const Vec3x& center, const Vec3x& translate, const Vec3x& scale, int vtx );
    
    void freezeTriangleObject( TriangularMesh& triMesh );
    void translateRodVertex( RodData& rd, int vtx_id, Vec3x& translate )
    {
        rd.getDofController().setVertexDisplacement( vtx_id, translate / (Scalar) m_simulation_params.m_subSteps );
    }
};
