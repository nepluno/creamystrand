...
XMLReader::XMLReader() :
ProblemStepper("XMLReader", "read from XML files")
{
}
...
void XMLReader::loadXMLFile( const std::string& filename, std::vector<char>& xmlchars, rapidxml::xml_document<>& doc )
{
    // Attempt to read the text from the user-specified xml file
    std::string filecontents;
    if( !loadTextFileIntoString(filename,filecontents) )
    {
        std::cerr << "ERROR IN TWODSCENEXMLPARSER: XML scene file " << filename << ". Failed to read file." << std::endl;
        exit(1);
    }
...
bool XMLReader::loadTextFileIntoString( const std::string& filename, std::string& filecontents )
{
...
void XMLReader::loadCollisionFree( rapidxml::xml_node<>* node )
{
    ...
    for( rapidxml::xml_node<>* nd = node->first_node("collisionfree"); nd; nd = nd->next_sibling("collisionfree") )
    {
        ...
        for( rapidxml::xml_node<>* subnd = nd->first_node("p"); subnd; subnd = subnd->next_sibling("p") )
        {
            ...
            if( subnd->first_attribute("i") )
            {
                ...
                {
                    std::cerr << outputmod::startred << "ERROR IN XMLSCENEPARSER:" << outputmod::endred << " Failed to parse value of i attribute for collisionfree " << forcenum << ". Value must be integer. Exiting." << std::endl;
                    exit(1);
                }
            }
...
int XMLReader::LoadOptions(const char* filename)
{
    ...
    {
        std::cerr << "ERROR IN XMLSCENEPARSER: Failed to parse xml scene file. Failed to locate root <scene> node. Exiting." << std::endl;
        exit(1);
    }
...
void XMLReader::loadSolidObject( rapidxml::xml_node<>* node, int& maxgroup, const double& dt )
{
    maxgroup = 0;
    ...
    for( rapidxml::xml_node<>* subnd = node->first_node("distancefield"); subnd; subnd = subnd->next_sibling("distancefield") )
    {
        ...
        if(subnd->first_attribute("type"))
        {
            ...
            else if(handlertype == "sequence") bt = DFT_SEQUENCE;
            else {
                std::cerr << outputmod::startred << "ERROR IN XMLSCENEPARSER:" << outputmod::endred << " Failed to parse value of type attribute for distancefield parameters. Exiting." << std::endl;
                exit(1);
            }
        ...
        if( subnd->first_attribute("colorIndex") )
        {
            ...
            {
                std::cerr << outputmod::startred << "ERROR IN XMLSCENEPARSER:" << outputmod::endred << " Failed to parse value of colorIndex attribute for distancefield parameters. Value must be numeric. Exiting." << std::endl;
                exit(1);
            }
        }
        int group = 0;
        if( subnd->first_attribute("group") )
        {
            ...
            {
                std::cerr << outputmod::startred << "ERROR IN XMLSCENEPARSER:" << outputmod::endred << " Failed to parse value of group attribute for distancefield parameters. Value must be numeric. Exiting." << std::endl;
                exit(1);
            }
        }
        ...
        if( subnd->first_attribute("inverted") )
        {
            ...
            {
                std::cerr << outputmod::startred << "ERROR IN XMLSCENEPARSER:" << outputmod::endred << " Failed to parse value of inverted attribute for distancefield parameters. Value must be numeric. Exiting." << std::endl;
                exit(1);
            }
        }
        ...
        if( subnd->first_attribute("surfflow") )
        {
            ...
            {
                std::cerr << outputmod::startred << "ERROR IN XMLSCENEPARSER:" << outputmod::endred << " Failed to parse value of surfflow attribute for distancefield parameters. Value must be numeric. Exiting." << std::endl;
                exit(1);
            }
        }
        ...
        if( subnd->first_attribute("clamped") )
        {
            ...
            {
                std::cerr << outputmod::startred << "ERROR IN XMLSCENEPARSER:" << outputmod::endred << " Failed to parse value of clamped attribute for distancefield parameters. Value must be numeric. Exiting." << std::endl;
                exit(1);
            }
        }
        ...
        if( subnd->first_attribute("flowheight") )
        {
            ...
            {
                std::cerr << outputmod::startred << "ERROR IN XMLSCENEPARSER:" << outputmod::endred << " Failed to parse value of flowheight attribute for distancefield parameters. Value must be numeric. Exiting." << std::endl;
                exit(1);
            }
        }
        ...
        if(dfu == DFU_SOURCE) {
            for(rapidxml::xml_node<>* subsubnd = subnd->first_node("emit"); subsubnd; subsubnd = subsubnd->next_sibling("emit")) {
                ...
                if( subsubnd->first_attribute("start") )
                {
                    ...
                    {
                        std::cerr << outputmod::startred << "ERROR IN XMLSCENEPARSER:" << outputmod::endred << " Failed to parse value of start attribute for distancefield parameters. Value must be numeric. Exiting." << std::endl;
                        exit(1);
                    }
                }
                ...
                if( subsubnd->first_attribute("end") )
                {
                    ...
                    {
                        std::cerr << outputmod::startred << "ERROR IN XMLSCENEPARSER:" << outputmod::endred << " Failed to parse value of end attribute for distancefield parameters. Value must be numeric. Exiting." << std::endl;
                        exit(1);
                    }
                }
                ...
                if( subsubnd->first_attribute("evx") )
                {
                    ...
                    {
                        std::cerr << outputmod::startred << "ERROR IN XMLSCENEPARSER:" << outputmod::endred << " Failed to parse value of ev(0) attribute for distancefield parameters. Value must be numeric. Exiting." << std::endl;
                        exit(1);
                    }
                }
                ...
                if( subsubnd->first_attribute("evy") )
                {
                    ...
                    {
                        std::cerr << outputmod::startred << "ERROR IN XMLSCENEPARSER:" << outputmod::endred << " Failed to parse value of ev(1) attribute for distancefield parameters. Value must be numeric. Exiting." << std::endl;
                        exit(1);
                    }
                }
                ...
                if( subsubnd->first_attribute("evz") )
                {
                    ...
                    {
                        std::cerr << outputmod::startred << "ERROR IN XMLSCENEPARSER:" << outputmod::endred << " Failed to parse value of ev(2) attribute for distancefield parameters. Value must be numeric. Exiting." << std::endl;
                        exit(1);
                    }
                }
				...
				if( subsubnd->first_attribute("maxvol") )
				{
					...
					{
						std::cerr << outputmod::startred << "ERROR IN XMLSCENEPARSER:" << outputmod::endred << " Failed to parse value of maxvol attribute for distancefield parameters. Value must be numeric. Exiting." << std::endl;
						exit(1);
					}
				}
			...
			if(emits.size() == 0) {
				...
				DistanceFieldObject::EMIT_INFO emit = {eject_vel, 0.0, 0.0, std::numeric_limits<Scalar>::max(), true};
				emits.push_back(emit);
			}
        }
        
        if(bt == DFT_BOX || bt == DFT_SPHERE || bt == DFT_CAPSULE || bt == DFT_CYLINDER || bt == DFT_FILE || bt == DFT_SEQUENCE) {
            Vec3x center;
            ...
            switch (bt) {
                ...
                case DFT_SEQUENCE:
                    ...
                    if( subnd->first_attribute("startTime") )
                    {
                        ...
                        if( !stringutils::extractFromString(attribute,parameter(0)) )
                        {
                            std::cerr << outputmod::startred << "ERROR IN XMLSCENEPARSER:" << outputmod::endred << " Failed to parse value of startTime attribute for distancefield parameters. Value must be numeric. Exiting." << std::endl;
                            exit(1);
                        }
                    }
                    if( subnd->first_attribute("endTime") )
                    {
                        ...
                        if( !stringutils::extractFromString(attribute,parameter(1)) )
                        {
                            std::cerr << outputmod::startred << "ERROR IN XMLSCENEPARSER:" << outputmod::endred << " Failed to parse value of endTime attribute for distancefield parameters. Value must be numeric. Exiting." << std::endl;
                            exit(1);
                        }
                    }
                    if( subnd->first_attribute("startFrame") )
                    {
                        std::string attribute(subnd->first_attribute("startFrame")->value());
                        if( !stringutils::extractFromString(attribute,parameter(2)) )
                        {
                            std::cerr << outputmod::startred << "ERROR IN XMLSCENEPARSER:" << outputmod::endred << " Failed to parse value of startFrame attribute for distancefield parameters. Value must be numeric. Exiting." << std::endl;
                            exit(1);
                        }
                    }
                    if( subnd->first_attribute("endFrame") )
                    {
                        ...
                        if( !stringutils::extractFromString(attribute,parameter(3)) )
                        {
                            std::cerr << outputmod::startred << "ERROR IN XMLSCENEPARSER:" << outputmod::endred << " Failed to parse value of endFrame attribute for distancefield parameters. Value must be numeric. Exiting." << std::endl;
                            exit(1);
                        }
                    }
                    if( subnd->first_attribute("dx") )
            ...
            if(bt == DFT_SEQUENCE) {
                ...
                if(!filename.empty()) {
                    if(dfu == DFU_SOLID) {
                        m_fields.push_back( DistanceFieldObject(center, parameter, bt, dfu, raxis, rangle, group, color_index, dt, dx_seq, emits, inverted, false, surfflow, filename, "") );
                        m_solid_flow_height.push_back(flow_height);
                    } else if(dfu == DFU_SOURCE) {
                        m_sources.push_back( DistanceFieldObject(center, parameter, bt, dfu, raxis, rangle, group, color_index, dt, dx_seq, emits, inverted, clamped, surfflow, filename, "") );
                    } else if(dfu == DFU_TERMINATOR) {
                        m_terminators.push_back( DistanceFieldObject(center, parameter, bt, dfu, raxis, rangle, group, color_index, dt, dx_seq, emits, inverted, false, surfflow, filename, "") );
                    }
                } else {
                    continue;
                }
                ...
                if(!filename.empty()) {
                    if(dfu == DFU_SOLID) {
                        m_fields.push_back( DistanceFieldObject(center, parameter, bt, dfu, raxis, rangle, group, color_index, dt, dx_seq, emits, inverted, false, surfflow, filename, cachename) );
                        m_solid_flow_height.push_back(flow_height);
                    } else if(dfu == DFU_SOURCE) {
                        m_sources.push_back( DistanceFieldObject(center, parameter, bt, dfu, raxis, rangle, group, color_index, dt, dx_seq, emits, inverted, clamped, surfflow, filename, cachename) );
                    } else if(dfu == DFU_TERMINATOR) {
                        m_terminators.push_back( DistanceFieldObject(center, parameter, bt, dfu, raxis, rangle, group, color_index, dt, dx_seq, emits, inverted, false, surfflow, filename, cachename) );
                    }
                } else {
                    continue;
                }
                ...
                if(dfu == DFU_SOLID) {
                    m_fields.push_back( DistanceFieldObject(center, parameter, bt, dfu, raxis, rangle, group, color_index, dt, dx_seq, emits, inverted, false, surfflow) );
                    m_solid_flow_height.push_back(flow_height);
                } else if(dfu == DFU_SOURCE) {
                    m_sources.push_back( DistanceFieldObject(center, parameter, bt, dfu, raxis, rangle, group, color_index, dt, dx_seq, emits, inverted, clamped, surfflow) );
                } else {
                    m_terminators.push_back( DistanceFieldObject(center, parameter, bt, dfu, raxis, rangle, group, color_index, dt, dx_seq, emits, inverted, false, surfflow) );
                }
            }
            ...
            if(dfu == DFU_SOLID) {
                ...
                dfo.mesh_controller->setDefaultFrictionCoefficient( m_simulation_params.m_hairMeshFrictionCoefficient ) ;

                auto currentMesh = dfo.mesh_controller->getCurrentMesh();

                transformTriangleObject(*currentMesh, dfo.rot, Vec3x::Zero(), dfo.center);
                
                dfo.resetDisplacement();
                    
                strandsim::TriangleMeshRenderer* mesh_renderer = new strandsim::TriangleMeshRenderer( *(dfo.mesh_controller->getCurrentMesh()), num_components);
...
template<typename T>
void XMLReader::loadParam(rapidxml::xml_node<>* nd, const char* name, Eigen::Matrix< T, Eigen::Dynamic, 1> & param, int num_components)
{
	...
	if ((subnd = nd->first_node(name)))
	{
		if (subnd->first_attribute("value")) {
			...
			{
				std::cerr << outputmod::startred << "ERROR IN XMLSCENEPARSER:" << outputmod::endred << " Failed to parse value of " << name << " attribute for " << nd->name() << ". Exiting." << std::endl;
				exit(1);
			}
			param = Eigen::Matrix< T, Eigen::Dynamic, 1>::Constant(num_components, param(0));
		}
		else {
			int i = 0;
			param = Eigen::Matrix< T, Eigen::Dynamic, 1>::Constant(num_components, param(0));

			for (rapidxml::xml_node<>* subsubnd = subnd->first_node("component"); subsubnd; subsubnd = subsubnd->next_sibling("component")) {
				if (subsubnd->first_attribute("value")) {
					...
					{
						std::cerr << outputmod::startred << "ERROR IN XMLSCENEPARSER:" << outputmod::endred << " Failed to parse " << i << "-th value of " << name << " attribute for " << nd->name() << ". Exiting." << std::endl;
						exit(1);
					}
				}
				++i;
...
void XMLReader::loadLiquid( rapidxml::xml_node<>* node, const double& dt )
{
	...
	if( nd )
	{
		...
		loadParam(nd, "surfTensionCoeff", info.surf_tension_coeff, info.num_components);
		loadParam(nd, "plasticWeakenStrain", info.plastic_weaken_strain, info.num_components);
		loadParam(nd, "liquidBulkModulus", info.liquid_bulk_modulus, info.num_components);
		loadParam(nd, "flowBehaviorIndex", info.flow_behavior_index, info.num_components);
		loadParam(nd, "flowConsistencyIndex", info.flow_consistency_index, info.num_components);
		loadParam(nd, "plasticYieldStress", info.plastic_yield_stress, info.num_components);
		loadParam(nd, "liquidShearModulus", info.liquid_shear_modulus, info.num_components);
		loadParam(nd, "plasticRelaxCoeff", info.plastic_relax_coeff, info.num_components);
		loadParam(nd, "helmholtzFirstMinima", info.helmholtz_first_minima, info.num_components);
		loadParam(nd, "helmholtzSecondMinima", info.helmholtz_second_minima, info.num_components);
		...
		loadParam(nd, "restContactAngle", info.rest_contact_angle, std::function<double(const double&)>([](const double& val) -> double {return val * M_PI / 180.0; }));
    ...
    std::shared_ptr<FluidScriptingController> controller = std::make_shared<LiquidSimulator>(m_fields, m_sources, m_terminators, m_strands, m_bucket_size, m_num_cells, dt, info);
	m_fluidScripting_controllers.push_back(controller);
	
	FluidsRenderer* render = new FluidsRenderer(controller, controller->getParticleMutex(), controller->getGridMutex(), FluidsRenderer::DrawMode::DBG);
...
void XMLReader::initGroups( int num_group )
{
    ...
    for(int i = 0; i < num_sources; ++i)
    {
        ...
        if(m_group_fields[m_sources[i].group].size() > 0) {
            std::cout << "Error: Source group is overlapping with field group!" << std::endl;
            exit(-1);
        }
    }
    ...
    for(int i = 0; i < num_hairposes; ++i)
    {
        if(current_time >= m_hairposes[i].time) {
            ...
            for(int j = 0; j < num_strands; ++j)
            {
                ...
                m_strands[j]->setFutureDegreesOfFreedom(m_strands[j]->getCurrentDegreesOfFreedom());
                m_strands[j]->setCurrentAreaDegreesOfFreedom(m_hairposes[i].areas[j]);
                m_strands[j]->setFutureAreaDegreesOfFreedom(m_strands[j]->getCurrentAreaDegreesOfFreedom());
                
                const int num_verts = m_strands[j]->getNumVertices();
                for(int k = 0; k < num_verts; ++k)
                {
                    if(m_strands[j]->isVertexFreezed(k)) {
                        m_strands[j]->dynamics().getScriptingController()->freezeVertices(k);
                    }
                    ...
                    if(m_strands[j]->isVertexGoaled(k)) {
                        ...
                        m_strands[j]->dynamics().getScriptingController()->setVertexGoal(k, pos);
                    }
                    
                    if(k != num_verts - 1 && m_strands[j]->isThetaGoaled(k)) {
                        //                controller->freezeTheta(i);
                        m_strands[j]->dynamics().getScriptingController()->setThetaGoal(k, m_hairposes[i].dofs[j](k * 4 + 3));
                    }
                }
...
bool XMLReader::executeScript(int total_substep_id, const Scalar substep_dt)
{
    ...
    for(int i = 0; i < num_groups; ++i) {
        ...
        for(const std::pair<int, int>& p : group) {
            ...
            if(m_fixed[ global_idx ]) {
                if(m_particle_closed_face_index[ global_idx ] >= 0 && m_group_fields[i].size() > 0) {
...
void XMLReader::loadParticles(rapidxml::xml_node<>* node, int& maxgroup )
{
    ...
    for( rapidxml::xml_node<>* nd = node->first_node("particle"); nd; nd = nd->next_sibling("particle") )
    {
		...
		if( nd->first_attribute("h") )
		{
			...
			{
				std::cerr << outputmod::startred << "ERROR IN XMLSCENEPARSER:" << outputmod::endred << " Failed to parse h attribute for particle " << particle << ". Value must be double. Exiting." << std::endl;
				exit(1);
			}
		}
    ...
    for( rapidxml::xml_node<>* nd = node->first_node("hairobj"); nd; nd = nd->next_sibling("hairobj") )
    {
        std::string szfn;
        ...
        if( nd->first_attribute("group") )
        {
            ...
            if( !stringutils::extractFromString(attribute,group) )
            {
                std::cerr << outputmod::startred << "ERROR IN XMLSCENEPARSER:" << outputmod::endred << " Failed to parse group attribute for hairobj. Value must be integer. Exiting." << std::endl;
                exit(1);
            }
        }
        ...
        if( nd->first_attribute("x") )
        {
            ...
            {
                std::cerr << outputmod::startred << "ERROR IN XMLSCENEPARSER:" << outputmod::endred << " Failed to parse x attribute for hairobj. Value must be scalar. Exiting." << std::endl;
                exit(1);
            }
        }
        ...
        if( nd->first_attribute("y") )
        {
            ...
            {
                std::cerr << outputmod::startred << "ERROR IN XMLSCENEPARSER:" << outputmod::endred << " Failed to parse y attribute for hairobj. Value must be scalar. Exiting." << std::endl;
                exit(1);
            }
        }
        ...
        if( nd->first_attribute("z") )
        {
            ...
            {
                std::cerr << outputmod::startred << "ERROR IN XMLSCENEPARSER:" << outputmod::endred << " Failed to parse z attribute for hairobj. Value must be scalar. Exiting." << std::endl;
                exit(1);
            }
        }
        ...
        if( nd->first_attribute("rx") )
        {
            ...
            {
                std::cerr << outputmod::startred << "ERROR IN XMLSCENEPARSER:" << outputmod::endred << " Failed to parse rx attribute for hairobj. Value must be scalar. Exiting." << std::endl;
                exit(1);
            }
        }
        ...
        if( nd->first_attribute("ry") )
        {
            ...
            {
                std::cerr << outputmod::startred << "ERROR IN XMLSCENEPARSER:" << outputmod::endred << " Failed to parse ry attribute for hairobj. Value must be scalar. Exiting." << std::endl;
                exit(1);
            }
        }
        ...
        if( nd->first_attribute("rz") )
        {
            ...
            {
                std::cerr << outputmod::startred << "ERROR IN XMLSCENEPARSER:" << outputmod::endred << " Failed to parse rz attribute for hairobj. Value must be scalar. Exiting." << std::endl;
                exit(1);
            }
        }
        ...
        if( nd->first_attribute("rw") )
        {
            ...
            {
                std::cerr << outputmod::startred << "ERROR IN XMLSCENEPARSER:" << outputmod::endred << " Failed to parse rw attribute for hairobj. Value must be scalar. Exiting." << std::endl;
                exit(1);
            }
        }
        
        Eigen::Quaternion<Scalar> qrot = Eigen::Quaternion<Scalar>(Eigen::AngleAxis<Scalar>(rot_angle, rot_axis));
...
void XMLReader::loadStrandParameters( rapidxml::xml_node<>* node, const double& dt )
{
    ...
    for( nd = node->first_node("StrandParameters"); nd; nd = nd->next_sibling("StrandParameters") )
    {
		...
		loadParam(nd, "minBendingAngle", minBendingAngle, std::function<double(const double&)>( [] (const double& val) -> double {return val * M_PI / 180.0; } ));
		loadParam(nd, "minBendingMultiplier", minBendingMultiplier);
		loadParam(nd, "maxFlowGradientRatio", maxFlowGradientRatio);

        m_strand_parameters.push_back(ElasticStrandParameters(paramsCount, radius, biradius, YoungsModulus, shearModulus, density, viscosity, air_drag, baseRotation, stretchMultiplier, fixingMultiplier, minBendingAngle, maxFlowGradientRatio, minBendingMultiplier));
...
template<typename T>
void XMLReader::loadParam(rapidxml::xml_node<>* nd, const char* name, T& param, std::function<T(const T&)> const& post_process_func)
{
	rapidxml::xml_node<>* subnd;
	if ((subnd = nd->first_node(name)))
	{
...
void XMLReader::setSimulationParameters()
{
    ...
    loadParam(nd, "useLengthProjection", m_simulation_params.m_useLengthProjection);
    loadParam(nd, "usePreFilterGeometry", m_simulation_params.m_usePreFilterGeometry);
    loadParam(nd, "useApproxRodElasticFriction", m_simulation_params.m_useApproxRodElasticFriction);
    loadParam(nd, "skipRodMeshCollisions", m_simulation_params.m_skipRodMeshCollisions);
    loadParam(nd, "skipRodRodCollisions", m_simulation_params.m_skipRodRodCollisions);
    loadParam(nd, "skipFlowFrictions", m_simulation_params.m_skipFlowFrictions);
    loadParam(nd, "useCTRodRodCollisions", m_simulation_params.m_useCTRodRodCollisions);
    ...
    loadParam(nd, "useDeterministicSolver", m_simulation_params.m_useDeterministicSolver);
    loadParam(nd, "useSoftAttachConstraints", m_simulation_params.m_useSoftAttachConstraints);
    loadParam(nd, "hairHairFrictionCoefficient", m_simulation_params.m_hairHairFrictionCoefficient);
...
void XMLReader::setRodCollisionParameters( CollisionParameters& param, const ElasticStrandParameters& strand_param )
{
    ...
    param.m_frictionCoefficient = m_simulation_params.m_hairHairFrictionCoefficient;
    param.m_meshFrictionCoefficient = m_simulation_params.m_hairMeshFrictionCoefficient;
...
void XMLReader::loadCheckpoint( rapidxml::xml_node<>* node, int& current_frame, int& current_checkpoint )
{
	for( rapidxml::xml_node<>* nd = node->first_node("checkpoint"); nd; nd = nd->next_sibling("checkpoint") )
	{
		DumpDataBinary data;
		if( nd->first_attribute("path") )
		{
			...
			if(num_goals)
			{
				ifs.read((char*) &(data.m_strand_goals[0]), sizeof(std::pair<Vec2i, Vec4x>) * num_goals);
			}
			...
			if(num_groups) {
				...
				ifs.read((char*) &(data.m_group_rot[0]), sizeof(Eigen::Quaternion<Scalar>) * num_groups);
			}
			...
			if(num_fields) {
				...
				ifs.read((char*) &(data.m_field_rot[0]), sizeof(Eigen::Quaternion<Scalar>) * num_fields);
				ifs.read((char*) &(data.m_field_future_scale[0]), sizeof(Vec3x) * num_fields);
			}
			...
			for(int i = 0; i < num_fields; ++i)
			{
				...
				ifs.read((char*) (data.m_field_parameter[i].data()), num_params * sizeof(Scalar));
			}
			...
			for(int i = 0; i < num_meshes; ++i)
			{
				...
				ifs.read((char*) &(data.m_previous_mesh_vertices[i][0]), sizeof(Vec3x) * data.m_previous_mesh_vertices[i].size());
				ifs.read((char*) &(data.m_current_mesh_vertices[i][0]), sizeof(Vec3x) * data.m_current_mesh_vertices[i].size());
				ifs.read((char*) &(data.m_next_mesh_vertices[i][0]), sizeof(Vec3x) * data.m_next_mesh_vertices[i].size());
				ifs.read((char*) &(data.m_current_mesh_displacement[i][0]), sizeof(Vec3x) * data.m_current_mesh_displacement[i].size());
			}
			...
			for(int i = 0; i < num_strands; ++i)
			{
				...
				for(int k = 0; k < num_verts; ++k)
				{
					if(m_strands[i]->isVertexFreezed(k)) {
						m_strands[i]->dynamics().getScriptingController()->freezeVertices(k);
					}
			...
			for(const std::pair<Vec2i, Vec4x>& goal : data.m_strand_goals)
			{
				if(m_strands[ goal.first[0] ]->isVertexGoaled(goal.first[1]))
					m_strands[ goal.first[0] ]->dynamics().getScriptingController()->setVertexGoal(goal.first[1], Vec3x(goal.second(0), goal.second(1), goal.second(2)));
				
				if(m_strands[ goal.first[0] ]->isThetaGoaled(goal.first[1]))
					m_strands[ goal.first[0] ]->dynamics().getScriptingController()->setThetaGoal(goal.first[1], goal.second(3));
			}
			
			// set groups
			assert( m_groups.size() >= data.m_group_pos.size() );
			
            for(int i = 0; i < (int) m_group_pos.size() && i < (int) data.m_group_pos.size(); ++i)
            {
			...
			// set fields
			assert( m_fields.size() + m_sources.size() + m_terminators.size() >= data.m_field_i_frames.size() );
			
			int k = 0;
			for(int i = 0; i < (int) m_fields.size() && k < (int) data.m_field_i_frames.size(); ++i, ++k)
			{
			...
			for(int i = 0; i < (int) m_sources.size() && k < (int) data.m_field_i_frames.size(); ++i, ++k)
			{
				m_sources[i].i_frame = data.m_field_i_frames[k];
				m_sources[i].future_center = m_sources[i].center = data.m_field_center[k];
			...
			for(int i = 0; i < (int) m_terminators.size() && k < (int) data.m_field_i_frames.size(); ++i, ++k)
			{
				m_terminators[i].i_frame = data.m_field_i_frames[k];
				m_terminators[i].future_center = m_terminators[i].center = data.m_field_center[k];
				m_terminators[i].future_rot = m_terminators[i].rot = data.m_field_rot[k];
			...
			for(int i = 0; i < num_meshes; ++i)
			{
				m_meshScripting_controllers[i]->setIFrame( data.m_mesh_current_frames[i] );
				m_meshScripting_controllers[i]->getPreviousMesh()->m_vertices = data.m_previous_mesh_vertices[i];
				m_meshScripting_controllers[i]->getCurrentMesh()->m_vertices = data.m_current_mesh_vertices[i];
				m_meshScripting_controllers[i]->getNextMesh()->m_vertices = data.m_next_mesh_vertices[i];
				m_meshScripting_controllers[i]->getCurrentMesh()->m_previous_displacements =
				m_meshScripting_controllers[i]->getCurrentMesh()->m_displacements = data.m_current_mesh_displacement[i];
...
void XMLReader::loadHairPose( rapidxml::xml_node<>* node )
{
    for( rapidxml::xml_node<>* nd = node->first_node("pose"); nd; nd = nd->next_sibling("pose") )
    {
        ...
        if( nd->first_attribute("time") )
        {
            ...
            {
                std::cerr << outputmod::startred << "ERROR IN XMLSCENEPARSER:" << outputmod::endred << " Failed to parse value of time attribute for pose. Value must be double. Exiting." << std::endl;
                exit(1);
            }
        }
        ...
        for( rapidxml::xml_node<>* subnd = nd->first_node("selection"); subnd; subnd = subnd->next_sibling("selection") )
        {
        ...
        if( nd->first_attribute("cx") )
        {
            ...
            if( !stringutils::extractFromString(attribute,center(0)) )
            {
                std::cerr << outputmod::startred << "ERROR IN XMLSCENEPARSER:" << outputmod::endred << " Failed to parse value of center(0) attribute for pose parameters. Value must be numeric. Exiting." << std::endl;
                exit(1);
            }
        }
        ...
        if( nd->first_attribute("cy") )
        {
            ...
            if( !stringutils::extractFromString(attribute,center(1)) )
            {
                std::cerr << outputmod::startred << "ERROR IN XMLSCENEPARSER:" << outputmod::endred << " Failed to parse value of center(1) attribute for pose parameters. Value must be numeric. Exiting." << std::endl;
                exit(1);
            }
        }
        ...
        if( nd->first_attribute("cz") )
        {
            ...
            if( !stringutils::extractFromString(attribute,center(2)) )
            {
                std::cerr << outputmod::startred << "ERROR IN XMLSCENEPARSER:" << outputmod::endred << " Failed to parse value of center(2) attribute for pose parameters. Value must be numeric. Exiting." << std::endl;
                exit(1);
            }
        }
        ...
        if( nd->first_attribute("path") )
        {
            ...
            while(getline(ifs, line))
            {
                ...
                if(start_data) {
                    ...
                    if(strand_indices_selection.size()) {
                        if(strand_indices_selection.find(seg) == strand_indices_selection.end())
                            continue;
                        else
            ...
            for(int i = 0; i < total_num_verts; ++i)
            {
                ...
                if(segment[i].second == num_vert_strand - 1) {
                    pose.dofs[ segment[i].first ].segment<3>( segment[i].second * 4 ) = pos[i].segment<3>(0);
                } else {
                    pose.dofs[ segment[i].first ].segment<4>( segment[i].second * 4 ) = pos[i];
                }
...
void XMLReader::loadScripts( rapidxml::xml_node<>* node)
{
    for( rapidxml::xml_node<>* nd = node->first_node("script"); nd; nd = nd->next_sibling("script") )
    {
        ...
        if( nd->first_attribute("x") )
        {
            ...
            {
                std::cerr << outputmod::startred << "ERROR IN XMLSCENEPARSER:" << outputmod::endred << " Failed to parse value of x attribute for script. Value must be double. Exiting." << std::endl;
                exit(1);
            }
        }
        if( nd->first_attribute("y") )
        {
            ...
            {
                std::cerr << outputmod::startred << "ERROR IN XMLSCENEPARSER:" << outputmod::endred << " Failed to parse value of y attribute for script. Value must be double. Exiting." << std::endl;
                exit(1);
            }
        }
        if( nd->first_attribute("z") )
        {
            ...
            {
                std::cerr << outputmod::startred << "ERROR IN XMLSCENEPARSER:" << outputmod::endred << " Failed to parse value of z attribute for script. Value must be double. Exiting." << std::endl;
                exit(1);
            }
        }
        if( nd->first_attribute("w") )
        {
            ...
            {
                std::cerr << outputmod::startred << "ERROR IN XMLSCENEPARSER:" << outputmod::endred << " Failed to parse value of w attribute for script. Value must be double. Exiting." << std::endl;
                exit(1);
            }
        }
        ...
        if( nd->first_attribute("ox") )
        {
            ...
            {
                std::cerr << outputmod::startred << "ERROR IN XMLSCENEPARSER:" << outputmod::endred << " Failed to parse value of ox attribute for script. Value must be double. Exiting." << std::endl;
                exit(1);
            }
        ...
        if( nd->first_attribute("oy") )
        {
            ...
            {
                std::cerr << outputmod::startred << "ERROR IN XMLSCENEPARSER:" << outputmod::endred << " Failed to parse value of oy attribute for script. Value must be double. Exiting." << std::endl;
                exit(1);
            }
            scr.transform_with_origin = true;
        }
        if( nd->first_attribute("oz") )
        {
            ...
            {
                std::cerr << outputmod::startred << "ERROR IN XMLSCENEPARSER:" << outputmod::endred << " Failed to parse value of oz attribute for script. Value must be double. Exiting." << std::endl;
                exit(1);
            }
            scr.transform_with_origin = true;
        }
        ...
        if( nd->first_attribute("mask") )
        {
            ...
            {
                std::cerr << outputmod::startred << "ERROR IN XMLSCENEPARSER:" << outputmod::endred << " Failed to parse value of mask attribute for script. Value must be int. Exiting." << std::endl;
                exit(1);
            }
        }
        ...
        if( nd->first_attribute("start") )
        {
            ...
            {
                std::cerr << outputmod::startred << "ERROR IN XMLSCENEPARSER:" << outputmod::endred << " Failed to parse value of start attribute for script. Value must be double. Exiting." << std::endl;
                exit(1);
            }
        }
        ...
        if( nd->first_attribute("end") )
        {
            ...
            {
                std::cerr << outputmod::startred << "ERROR IN XMLSCENEPARSER:" << outputmod::endred << " Failed to parse value of end attribute for script. Value must be double. Exiting." << std::endl;
                exit(1);
            }
        }
        ...
        if( nd->first_attribute("amplitude") )
        {
            ...
            {
                std::cerr << outputmod::startred << "ERROR IN XMLSCENEPARSER:" << outputmod::endred << " Failed to parse value of amplitude attribute for script. Value must be double. Exiting." << std::endl;
                exit(1);
            }
        }
        ...
        if( nd->first_attribute("dt") )
        {
            ...
            {
                std::cerr << outputmod::startred << "ERROR IN XMLSCENEPARSER:" << outputmod::endred << " Failed to parse value of dt attribute for script. Value must be double. Exiting." << std::endl;
                exit(1);
            }
        }
        ...
        if( nd->first_attribute("frequency") )
        {
            ...
            {
                std::cerr << outputmod::startred << "ERROR IN XMLSCENEPARSER:" << outputmod::endred << " Failed to parse value of frequency attribute for script. Value must be double. Exiting." << std::endl;
                exit(1);
            }
        }
...
{
    {
        ...
        if( dtnd == NULL )
        {
            ...
            {
                std::cerr << outputmod::startred << "ERROR IN XMLSCENEPARSER:" << outputmod::endred << " Failed to parse value of dt attribute for integrator. Value must be numeric. Exiting." << std::endl;
                exit(1);
            }
        }
        ...
        if(nd->first_attribute("cfl")) {
            ...
            {
                std::cerr << outputmod::startred << "ERROR IN XMLSCENEPARSER:" << outputmod::endred << " Failed to parse value of cfl attribute for integrator. Value must be numeric. Exiting." << std::endl;
                exit(1);
            }
        }
    }

    {
        ...
        m_end_time = 0.0;
        if( !stringutils::extractFromString(std::string(dtnd->value()),m_end_time) )
        {
...
void XMLReader::loadHairs( rapidxml::xml_node<>* node )
{
    ...
    for( rapidxml::xml_node<>* nd = node->first_node("hair"); nd; nd = nd->next_sibling("hair") ){
        ...
        for(int i = 0; i < count; ++i) {
            if(i == count - 1) {
                dofs.segment<3>(i * 4) = Vec3x(m_particles[particle_indices[i]](0), m_particles[particle_indices[i]](1), m_particles[particle_indices[i]](2));
            } else {
...
//                controller->freezeTheta(i);
            ...
            m_global_to_local[particle_indices[i]] = std::pair<int, int>(numstrands, i);
        }
        
        ElasticStrand* strand = new ElasticStrand( dofs, area_dofs, m_strand_parameters[paramsIndex], m_collision_parameters[paramsIndex], controller );
    ...
    for( rapidxml::xml_node<>* nd = node->first_node("hairobj"); nd; nd = nd->next_sibling("hairobj") )
    {
        ...
        if( nd->first_attribute("params") )
        {
            ...
            if( !stringutils::extractFromString(attribute,paramsIndex) )
            {
                std::cerr << outputmod::startred << "ERROR IN XMLSCENEPARSER:" << outputmod::endred << " Failed to parse value of params attribute for obj " << i_obj_strand << ". Value must be integer. Exiting." << std::endl;
                exit(1);
            }
        }
        ...
        for( rapidxml::xml_node<>* subnd = nd->first_node("flow"); subnd; subnd = subnd->next_sibling("flow") )
        {
            setFlow s = {0, 0, 0.0};
        ...
        while(getline(ifs, line))
        {
            ...
            if(tokens.size() < 1 || tokens[0] != "l")
                continue;
            
            if(strand_indices_selection.size() && strand_indices_selection.find(linenumber) == strand_indices_selection.end())
            {
                linenumber++;
                continue;
            }
...
void dump_binary_checkpoint_subprog( DumpDataBinary* data )
{
	...
	if(num_goals) {
		os_data.write((char*) &(data->m_strand_goals[0]), sizeof(std::pair<Vec2i, Vec4x>) * num_goals);
	}
	...
	if(num_groups) {
		...
		os_data.write((char*) &(data->m_group_rot[0]), sizeof(Eigen::Quaternion<Scalar>) * num_groups);
	}
	...
	if(num_fields) {
		os_data.write((char*) &(data->m_field_i_frames[0]), sizeof(int) * num_fields);
		os_data.write((char*) &(data->m_field_center[0]), sizeof(Vec3x) * num_fields);
		os_data.write((char*) &(data->m_field_rot[0]), sizeof(Eigen::Quaternion<Scalar>) * num_fields);
		os_data.write((char*) &(data->m_field_future_scale[0]), sizeof(Vec3x) * num_fields);
	}
	...
	for(int i = 0; i < num_fields; ++i)
	{
		...
		os_data.write((char*) (data->m_field_parameter[i].data()), data->m_field_parameter[i].size() * sizeof(Scalar));
	}
	...
	for(int i = 0; i < num_meshes; ++i)
	{
		...
		os_data.write((char*) &(data->m_previous_mesh_vertices[i][0]), sizeof(Vec3x) * data->m_previous_mesh_vertices[i].size());
		os_data.write((char*) &(data->m_current_mesh_vertices[i][0]), sizeof(Vec3x) * data->m_current_mesh_vertices[i].size());
		os_data.write((char*) &(data->m_next_mesh_vertices[i][0]), sizeof(Vec3x) * data->m_next_mesh_vertices[i].size());
		os_data.write((char*) &(data->m_current_mesh_displacement[i][0]), sizeof(Vec3x) * data->m_current_mesh_displacement[i].size());
	}
...
void XMLReader::dumpBinaryCheckpoint(std::string outputdirectory, int current_frame, int current_checkpoint, int file_width) const
{
	std::stringstream name;
	name << std::setfill('0');
	name << outputdirectory << "/checkpoint_" << std::setw(file_width) << current_checkpoint << ".bin";
	...
	const int num_dfos = m_fields.size() + m_sources.size() + m_terminators.size();
	...
	for( int i = 0; i < num_meshes; ++i )
	{
		data->m_mesh_current_frames[i] = m_meshScripting_controllers[i]->getIFrame();
		data->m_previous_mesh_vertices[i] = m_meshScripting_controllers[i]->getPreviousMesh()->m_vertices;
		data->m_current_mesh_vertices[i] = m_meshScripting_controllers[i]->getCurrentMesh()->m_vertices;
		data->m_next_mesh_vertices[i] = m_meshScripting_controllers[i]->getNextMesh()->m_vertices;
		data->m_current_mesh_displacement[i] = m_meshScripting_controllers[i]->getCurrentMesh()->m_displacements;
	}
...
void XMLReader::apply_global_rotation(int group_idx, const Eigen::Quaternion<double>& rot)
{
    for(int i : m_group_fields[group_idx])
    {
...
void XMLReader::apply_switch(int group_idx, unsigned mask)
{
    ...
    for(const std::pair<int, int>& p : group) {
        ...
        if(mask & 1U) {
            if(rd->getDofController().isVertexGoaled(p.second) || rd->getDofController().isVertexFreezed(p.second)) {
                if(m_simulation_params.m_useSoftAttachConstraints) {
                ...
                if(m_simulation_params.m_useSoftAttachConstraints) {
                    rd->getDofController().setVertexGoal(p.second, rd->getStrand().getVertex(p.second));
                } else {
        ...
        if(mask & 2U) {
            if(rd->getDofController().isThetaGoaled(p.second) || rd->getDofController().isThetaFreezed(p.second)) {
                if(m_simulation_params.m_useSoftAttachConstraints) {
                ...
                if(m_simulation_params.m_useSoftAttachConstraints) {
                    rd->getDofController().setThetaGoal(p.second, rd->getStrand().getTheta(p.second));
                } else {
...
double Script::getNextVelocity( const double& dt, const double& current_time )
{
    if(func == Script::COSINE) {
        ...
        const double vel = weno_ease_function(current_time + dt, dt, start, end, base_dt, base_pos, base_vertices);
        base_pos += vel * dt;
        return vel;
    }
        ...
        switch (type) {
            case Script::SCALE:
            {
                if(func != Script::CUBIC) {
                    std::cout << "[error: non-cubic scaling is unsupported!]" << std::endl;
                    break;
                }
                ...
                Vec3x vel = Vec3x(cubic_ease_function(current_time + dt, start, end, start + ease_start, end - ease_end, log(v(0))),
                                  cubic_ease_function(current_time + dt, start, end, start + ease_start, end - ease_end, log(v(1))),
                                  cubic_ease_function(current_time + dt, start, end, start + ease_start, end - ease_end, log(v(2))));

                Vec3x multipliers = Vec3x(exp(vel(0) * dt),
                                          exp(vel(1) * dt),
                                          exp(vel(2) * dt));
            ...
            case Script::ROTATE:
            {
                ...
                if(transform_with_origin) {
                    const Eigen::Quaternion<double> q_diff = prot * m_scene->m_group_prev_rot[ group_index ].inverse();
                    Vec3x& trans = m_scene->m_group_pos[ group_index ];
