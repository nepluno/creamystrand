...
namespace strandsim
{
	...
	StrandRenderer::StrandRenderer( const ElasticStrand& strand,
								   const std::vector<float> &currentVertices, const std::vector<float> &currentMaterialFrames,
	...
	m_lineWidth( 1 )
	{
		...
		m_palette["simple tip"] = Color(169, 99, 49); //Color(139, 69, 19); //Color( 155, 200, 100 );
		...
		m_palette["random material"] = Color(( (float) (rand() % 255) ) / 255., ( (float) (rand() % 255) ) / 255., ( (float) (rand() % 255) ) / 255.);
	...
	void StrandRenderer::render()
	{
		...
		{
			m_palette["first material"] = Color(139, 69, 19); //Color( 55, 125, 155 ); //Color( 255, 125, 75 );
			m_palette["second material"] = Color(139, 69, 19); //Color( 55, 125, 155 ); //Color( 55, 125, 255 );
...
//        
		if ( m_drawDebug )
			if ( !m_debugVertices.empty() )
			{
				...
				for ( auto debugPts = m_debugVertices.begin(); debugPts != m_debugVertices.end();
					 ++debugPts, ++transparencyIndex )
				{
					
					// If the debugDisplaySet is empty, display all available substeps; otherwise display only those that are in the set
					if ( !s_debugDisplaySet.empty()
						&& s_debugDisplaySet.find( transparencyIndex ) == s_debugDisplaySet.end() )
					{
						continue;
					}
					
					for ( unsigned vtx = 0; vtx < numVertices; ++vtx )
						m_blendedColors[4 * vtx + 3] = transparencyIndex
						/ ( ( float ) m_debugVertices.size() + 1 );
		...
		if ( m_drawDebug )
		{
			if ( !m_hitPoints.empty() )
			{
				...
				glPointSize( 4 );
				glBegin( GL_POINTS );
				for ( auto hitPoint = m_hitPoints.begin(); hitPoint != m_hitPoints.end(); ++hitPoint )
				{
	...
	void StrandRenderer::recomputeFlowQuads()
	{
		...
		if ( !( m_colorChanged || m_flowChanged ) )
			return;
		m_flowChanged = false;
		m_colorChanged = false;
	...
	void StrandRenderer::recomputeQuads()
	{
		...
		if ( !( m_colorChanged || m_geometryChanged ) )
			return;
...
//        }
		...
		for ( unsigned j = 0; j < numVertices; ++j )
		{
			...
			if ( j + 1 < numVertices )
			{
				...
				tangent = ( Vec3f::Map( &m_currentVertices[3 * ( j + 1 )] ) - vertex ).normalized();
			}
			...
			for ( int k = 0; k < slices; ++k )
			{
                ...
                Vec4f::Map( &quads.m_quadColors[curColIndex] ) = Vec4f( (float) color(0), (float) color(1), (float) color(2), 0.2f );
				
				const float angle = k * angleSlice + M_PI / 2 ;
				
				float r = m_strand.getParameters().getRadiusAtAngle( j, numVertices, angle );
	...
	void StrandRenderer::computeQuads( QuadData &quads, DrawVolume volume ) const
	{
		...
		switch ( volume )
		{
			...
			case EXTERNAL_COLLISIONS:
				alphaCoeff = m_palette.find("external collisions volume")->second.data()[3];
				break;
			default:
				break;
		}
		...
		for ( int k = 0; k < slices; ++k )
		{
			...
			switch ( volume )
			{
				...
				default:
					c = ( ( k < ( slices + 3 ) / 4 )
						 || ( slices / 2 <= k && k < ( 3 * slices + 3 ) / 4 ) ) ? "first material" :
					"second material";
					break;
			}
		...
		for ( unsigned j = 0; j < numVertices; ++j )
		{
			...
			for ( int k = 0; k < slices; ++k )
			{
				...
				//            {
				//            case SELF_COLLISIONS:
				//                r = m_strand.collisionParameters().selfCollisionsRadius( j, angle );
				//                break;
				//            case EXTERNAL_COLLISIONS:
				//                r = m_strand.collisionParameters().externalCollisionsRadius( j, angle );
				//                break;
				//            default:
				//                r = scale * m_strand.getParameters().getRadiusAtAngle( j, angle );
				//                break;
				//            }
				
				float r = m_strand.getParameters().getRadiusAtAngle( j, m_strand.getNumVertices(), angle );
	...
	void StrandRenderer::drawQuads( const QuadData& quads )
	{
		...
		// TODO ? We could save some memory if we used GL_QUAD_STRIP with glMultiDrawElements
	...
	void StrandRenderer::drawForce()
	{
		...
		// FIXME m_geometryMutex only protects m_currentVertices and m_currentMaterialFrames
		// Access to other parts of the strands are *NOT* thread-safe
		LockGuard lock( m_geometryMutex );
	...
	Vec3x StrandRenderer::calculateObjectCenter()
	{
		...
		{
			const Vec3x& vertex = Vec3f::Map( &m_currentVertices[3 * vtx] ).cast<Scalar>();
			center += vertex;
		}
