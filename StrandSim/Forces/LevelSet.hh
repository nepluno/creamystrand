...
namespace strandsim
{
    ...
    public:
        ...
        static void calculateLevelSetSize(
        ...
        void buildLevelSet( const FaceArray &triangles, const Indices &triIndices,
                           const Vec3xArray &x, const Vec3xArray &v, const Vec3x &origin, const Scalar dx, const int nx, const int ny, const int nz,
        ...
        void buildAdaptativeLevelSet( const FaceArray &triangles, const Indices &triIndices,
                                     const Vec3xArray &x, const Vec3xArray &v,
                                     const Mat4x& transformMatrix = Mat4x::Identity() );
        ...
        void getInteriorPoints( const Vec3x& start, const Vec3x &end, const Scalar thickness,
                               std::vector<Coord> &points, bool stopAtFirstMin = false ) const;
        void getInteriorIntervals( const Vec3x& start, const Vec3x &end, const Scalar thickness,
                                  std::vector<Interval> &intervals ) const;
        
        Scalar
        getLevelSetValueVelocity( const Vec3x &x, Vec3x &v ) const;
        
        void getGradient( const Vec3x& x, Vec3x& grad, bool rotated = true ) const;
        void getEdgeGradient( const Vec3x& start, const Vec3x &end, const Coord &min, Vec3x& grad,
                             int *gradLambdaAxis = NULL ) const;
        void getIntegralGradient( const Interval &iv, Vec3x &gradStart, Vec3x &gradEnd ) const;
        ...
        unsigned getClosestTriangle( const Vec3x& x, Scalar& t1, Scalar& t2, Scalar &t3 ) const;
        Vec3x getCurrentPosition( unsigned t, Scalar t1, Scalar t2, Scalar t3 ) const;
        
        const Vec3x& getOrigin() const
        {
            return m_origin;
        }
        
        Scalar getGridSize() const
        {
            return m_dx;
        }
        ...
        const bridson::Array3x& getPhi() const
        {
            return m_phi;
        }
        ...
        size_t getFileSize(const Vec3x& origin, const Vec3x& center, const Scalar& dx) const;
        ...
        Scalar getInterpolated( int i, int j, int k, Scalar fi, Scalar fj, Scalar fk ) const;
        ...
        Scalar getFractionInsideU( int i, int j, int k, Scalar fi, Scalar fj, Scalar fk ) const;
        Scalar getFractionInsideV( int i, int j, int k, Scalar fi, Scalar fj, Scalar fk ) const;
        Scalar getFractionInsideW( int i, int j, int k, Scalar fi, Scalar fj, Scalar fk ) const;
        Scalar FractionInside(Scalar phi_bl, Scalar phi_br, Scalar phi_tl, Scalar phi_tr) const;
        Scalar FractionInside(Scalar phi_left, Scalar phi_right) const;
        Vec3x getInterpolatedGradient( int i, int j, int k, Scalar fi, Scalar fj, Scalar fk ) const;
        Mat3x getInterpolatedHessian( int i, int j, int k, Scalar fi, Scalar fj, Scalar fk ) const;
        
        void getEdgeSamplingPoints( const Vec3x& start, const Vec3x &end, Coord &min,
                                   std::vector<Coord> *points = NULL, Scalar maxVal = 0.,
                                   bool stopAtFirstMin = false ) const;
        void getEdgeIntervals( const Vec3x& start, const Vec3x &end, const Scalar maxVal,
                              std::vector<Interval> &intervals ) const;
    protected:
        bridson::Array3x m_phi;
        bridson::Array3<Vec3x, bridson::Array1<Vec3x> > m_phiVel;
    ...
    void computeLevelSet( const FaceArray &triangles, const Indices &triIndices, const Vec3xArray &x,
                         const Vec3xArray &v, Vec3x &origin, const Vec3x &center, Scalar &dx, bridson::Array3x &phi,
    ...
    int orientation( Scalar x1, Scalar y1, Scalar x2, Scalar y2, Scalar &twice_signed_area );
    ...
    Scalar
    point_triangle_distance( const Vec3x &p, const Vec3x &a, const Vec3x &b, const Vec3x &c, Scalar &t1,
                            Scalar &t2, Scalar &t3 );
    ...
    void check_neighbour( const FaceArray &tri, const Vec3xArray &x, const Vec3xArray &v,
                         ...
                         bridson::Array3i &closest_tri, const Vec3x &gx, int i0, int j0, int k0, int i1,
                         int j1, int k1 );
    
    static void computeTrilinearValueCoeffs( const bridson::Array3x& phi,
                                            const InterpolatedLevelSet<1>::Interval& iv, Vec4x& coeffs );
    static void computeBilinearGradientCoeffs( const bridson::Array3x& phi,
                                              const InterpolatedLevelSet<1>::Interval& iv, Mat3x& coeffs );
    static void computeLinearHessianCoeffs( const bridson::Array3x& phi,
