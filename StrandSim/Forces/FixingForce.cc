...
void FixingForce<ViscousT>::computeLocal( LocalMultiplierType& localL, const ElasticStrand& strand, const IndexType vtx )
{
    // not implemented!
}

template<typename ViscousT>
Scalar FixingForce<ViscousT>::localEnergy( const ElasticStrand& strand, StrandState& geometry,
        const IndexType vtx )
{
    // not implemented!
...
template<typename ViscousT>
void FixingForce<ViscousT>::addInPositionMultiplier( VecXx& globalMultiplier, const IndexType vtx, const LocalMultiplierType& localL )
{
    globalMultiplier(vtx) += localL;
}

template<typename ViscousT>
void FixingForce<ViscousT>::computeLocal( typename FixingForce::LocalForceType& localF,
...
template<typename ViscousT>
void FixingForce<ViscousT>::computeLocal( typename FixingForce::LocalJacobianType& localJ,
        const ElasticStrand& strand, StrandState& geometry, const IndexType vtx )
{
    localJ.setZero();
...
template<typename ViscousT>
void FixingForce<ViscousT>::addInPosition( typename FixingForce::ForceVectorType& globalForce, const IndexType vtx,
...
template<typename ViscousT>
void FixingForce<ViscousT>::addInPosition( JacobianMatrixType& globalJacobian,
...
template<typename ViscousT>
void FixingForce<ViscousT>::accumulateCurrentE( Scalar& energy, ElasticStrand& strand )
{
...
template<typename ViscousT>
void FixingForce<ViscousT>::accumulateCurrentF( VecXx& force, ElasticStrand& strand )
{
    for ( IndexType vtx = s_first; vtx < strand.getNumVertices() - s_last; ++vtx )
    {
