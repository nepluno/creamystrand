...
void StretchingForce<ViscousT>::computeLocal( LocalMultiplierType& localL, const ElasticStrand& strand, const IndexType vtx )
{
    const Scalar ks = ViscousT::ks( strand, vtx ) * strand.getStepper()->getStretchMultiplier(); // dyne
    ...
    localL = -ks * (length - ViscousT::ellBar( strand, vtx ) + 
    edge.dot(strand.dynamics().getDisplacements().segment<3>((vtx + 1) * 4) - strand.dynamics().getDisplacements().segment<3>(vtx * 4))); // dyne.cm
}
...
Scalar StretchingForce<ViscousT>::localEnergy( const ElasticStrand& strand, StrandState& geometry,
        const IndexType vtx )
{
    const Scalar ks = ViscousT::ks( strand, vtx ) * strand.getStepper()->getStretchMultiplier();
    const Scalar restLength = ViscousT::ellBar( strand, vtx );
...
template<typename ViscousT>
void StretchingForce<ViscousT>::addInPositionMultiplier( VecXx& globalMultiplier, const IndexType vtx, const LocalMultiplierType& localL )
{
    globalMultiplier(vtx) += localL;
}

template<typename ViscousT>
void StretchingForce<ViscousT>::computeLocal( typename StretchingForce::LocalForceType& localF,
...
template<typename ViscousT>
void StretchingForce<ViscousT>::computeLocal( typename StretchingForce::LocalJacobianType& localJ,
        const ElasticStrand& strand, StrandState& geometry, const IndexType vtx )
{
    const Scalar ks = ViscousT::ks( strand, vtx ) * strand.getStepper()->getStretchMultiplier();
    const Scalar restLength = ViscousT::ellBar( strand, vtx );
...
template<typename ViscousT>
void StretchingForce<ViscousT>::addInPosition( typename StretchingForce::ForceVectorType& globalForce, const IndexType vtx,
