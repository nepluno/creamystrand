...
{
    return bridson::max3( bridson::max( a1, a2 ), bridson::max( a3, a4 ), bridson::max( a5, a6 ) );
}
...
{
    return value_lower
            + smooth_step( ( r - r_lower ) / ( r_upper - r_lower ) ) * ( value_upper - value_lower );
}
...
// This seems to pass several statistical tests, and is a bijective map (of 32-bit ints)
...
inline double randhashd( int seed )
{
    return randhash( seed ) / ( double ) UINT_MAX;
}
inline float randhashf( int seed )
{
...
inline void get_barycentric( const VT &x, VI& i, VT& f, const VI& i_low, const VI& i_high )
{
...
template<class S, class T>
inline S trilerp( const S& v000, const S& v100, const S& v010, const S& v110, const S& v001,
        const S& v101, const S& v011, const S& v111, T fx, T fy, T fz )
{
    return lerp( bilerp( v000, v100, v010, v110, fx, fy ), bilerp( v001, v101, v011, v111, fx, fy ),
            fz );
}

template<class S, class T>
inline S quadlerp( const S& v0000, const S& v1000, const S& v0100, const S& v1100, const S& v0010,
        const S& v1010, const S& v0110, const S& v1110, const S& v0001, const S& v1001,
        const S& v0101, const S& v1101, const S& v0011, const S& v1011, const S& v0111,
...
template<class S, class T>
inline S cubic_interp_diff( const S& value_neg1, const S& value0, const S& value1, const S& value2,
        T f )
{
    T wneg1, w0, w1, w2;
...
template<class S, class T>
inline S cubic_interp_diff2( const S& value_neg1, const S& value0, const S& value1, const S& value2,
        T f )
{
    T wneg1, w0, w1, w2;
...
{
    a.push_back( a.back() );
    for ( int i = ( int ) a.size() - 1; i > index; --i )
        a[i] = a[i - 1];
    a[index] = e;
}
...
template<class T>
void write_matlab( std::ostream& output, const std::vector<T>& a, const char *variable_name,
        bool column_vector = true, int significant_digits = 18 )
{
