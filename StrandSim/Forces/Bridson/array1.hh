...
//   WrapArray1<T>: a 1D array wrapper around an existing array (perhaps
//   objects, perhaps data)
// For the most part std::vector operations are supported, though for the Wrap
// version note that memory is never allocated/deleted and
// constructor/destructors are never called from within the class, thus only
// shallow copies can be made and some operations such as resize() and
...
struct Array1IsIntegral {
  typedef Array1False type;
};  // default: no (specializations to yes follow)
template <>
struct Array1IsIntegral<bool> {
  typedef Array1True type;
};
template <>
struct Array1IsIntegral<char> {
  typedef Array1True type;
};
template <>
struct Array1IsIntegral<signed char> {
  typedef Array1True type;
};
template <>
struct Array1IsIntegral<unsigned char> {
  typedef Array1True type;
};
template <>
struct Array1IsIntegral<short> {
  typedef Array1True type;
};
template <>
struct Array1IsIntegral<unsigned short> {
  typedef Array1True type;
};
template <>
struct Array1IsIntegral<int> {
  typedef Array1True type;
};
template <>
struct Array1IsIntegral<unsigned int> {
  typedef Array1True type;
};
template <>
struct Array1IsIntegral<long> {
  typedef Array1True type;
};
template <>
struct Array1IsIntegral<unsigned long> {
  typedef Array1True type;
};
template <>
struct Array1IsIntegral<long long> {
  typedef Array1True type;
};
template <>
struct Array1IsIntegral<unsigned long long> {
  typedef Array1True type;
};
...
struct Array1 {
  ...
  // STL vector's interface, with additions, but only valid when used with
  // plain-old-data

  Array1(void) : n(0), max_n(0), data(0) {}

  // note: default initial values are zero
  explicit Array1(unsigned long n_) : n(0), max_n(0), data(0) {
  ...
  unsigned long capacity(void) const { return max_n; }
  ...
  const T* end(void) const { return data + n; }

  T* end(void) { return data + n; }
  ...
  unsigned long max_size(void) const { return ULONG_MAX / sizeof(T); }
  ...
  void set_zero(void) { std::memset(data, 0, n * sizeof(T)); }
...
struct WrapArray1 {
  ...
  // Note that if the underlying arrays change where their data is, the
  // WrapArray may be screwed up

  explicit WrapArray1(Array1<T>& a) : n(a.n), max_n(a.max_n), data(a.data) {}
  ...
  unsigned long max_size(void) const { return max_n; }
  ...
  // note: shouldn't be used when T is a full object (setting to zero may not
  // make sense)
  void set_zero(void) { std::memset(data, 0, n * sizeof(T)); }
