...
namespace strandsim {
	namespace bridson {
		...
		struct FixedSparseMatrix
		{
			...
			std::vector<ptrdiff_t> colindex; // corresponding column indices
			std::vector<ptrdiff_t> rowstart; // where each row starts in value and colindex (and last entry is one past the end, the number of nonzeros)

			explicit FixedSparseMatrix(size_t n_ = 0)
		...
		void multiplyMat(const FixedSparseMatrix<T>& A, const FixedSparseMatrix<T>& B, FixedSparseMatrix<T>& C, T scale)
		{
			...
			c.resize(A.n);
			c.zero();
			for_each(0U, c.n, [&](unsigned int i) {
