...
namespace strandsim {
    ...
    std::shared_ptr<FluidScriptingController> FluidPressureForce::s_controller = NULL;
    ...
    void FluidPressureForce::computeLocal( LocalForceType& localF, const ElasticStrand& strand,
                                      StrandState& geometry, const IndexType vtx )
    {
        if(!s_controller || !strand.getStepper() || !s_controller->isStrandSubmerged(strand.getGlobalIndex()) || strand.isVertexFreezed(vtx)) {
    ...
    void FluidPressureForce::computeLocal( LocalJacobianType& localJ, const ElasticStrand& strand,
                                      StrandState& geometry, const IndexType vtx )
    {
//        if(!s_controller || !strand.getStepper() || !s_controller->isStrandSubmerged(strand.getGlobalIndex()) || strand.isVertexFreezed(vtx)) {
...
//        localJ = -gp * vol;
    ...
    void FluidPressureForce::setScriptingController(const std::shared_ptr<FluidScriptingController>& controller)
    {
        s_controller = controller;
    }
    
    void FluidPressureForce::addInPosition( ForceVectorType& globalForce, const IndexType vtx,
                                       const LocalForceType& localForce )
    {
        globalForce.segment<3>( 4 * vtx ) += localForce;
    }
    
    void FluidPressureForce::addInPosition( JacobianMatrixType& globalJacobian, const IndexType vtx,
