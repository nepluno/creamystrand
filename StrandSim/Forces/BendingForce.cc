...
void BendingForce<ViscousT>::computeLocal( typename BendingForce::LocalMultiplierType& localL, const ElasticStrand& strand, const IndexType vtx )
{
    // B.ilen.(phi+hJv)
    ...
    Vec4x Jvh = gradKappa.transpose() * strand.dynamics().getDisplacements().segment<11>(4 * (vtx - 1));

    localL.segment<2>(0) = -ilen * B * (kappa.segment<2>(0) - kappaBar.segment<2>(0) + Jvh.segment<2>(0));
    localL.segment<2>(2) = -ilen * B * (kappa.segment<2>(2) - kappaBar.segment<2>(2) + Jvh.segment<2>(2));
}
...
Scalar BendingForce<ViscousT>::localEnergy( const ElasticStrand& strand, StrandState& geometry,
        const IndexType vtx )
{
    const Mat2x& B = ViscousT::bendingMatrix( strand, vtx ) * strand.getStepper()->getStretchMultiplier();
    const Vec4x& kappaBar = ViscousT::kappaBar( strand, vtx );
    const Scalar ilen = strand.m_invVoronoiLengths[vtx];
...
template<typename ViscousT>
void BendingForce<ViscousT>::computeLocal( typename BendingForce::LocalForceType& localF,
...
template<typename ViscousT>
void BendingForce<ViscousT>::computeLocal( typename BendingForce::LocalJacobianType& localJ,
        const ElasticStrand& strand, StrandState& geometry, const IndexType vtx )
{

    localJ = geometry.m_bendingProducts[vtx] * 0.5;
    
    const Mat2x& bendingMatrixBase = strand.m_parameters.bendingMatrixBase() * strand.getStepper()->getStretchMultiplier();
    const Vec4x& kappaBar = ViscousT::kappaBar( strand, vtx );
    const Vec4x& kappa = geometry.m_kappas[vtx];
    ...
    const Vec2x& temp_e = bendingMatrixBase * ( kappa.segment<2>(0) - kappaBar.segment<2>(0) );
    const Vec2x& temp_f = bendingMatrixBase * ( kappa.segment<2>(2) - kappaBar.segment<2>(2) );
    
    localJ += (temp_e( 0 ) * hessKappa0 + temp_e( 1 ) * hessKappa1 + temp_f( 0 ) * hessKappa2 + temp_f( 1 ) * hessKappa3) * 0.5;
    ...
    if(vtx > 0 && vtx < strand.getNumVertices() - 1) {
        ...
        if(l0 < restLength) {
            ...
            M = ks
            * ( ( 1.0 / restState - 1.0 / length ) * Mat3x::Identity()
...
template<typename ViscousT>
void BendingForce<ViscousT>::addInPositionMultiplier( VecXx& globalMultiplier, const IndexType vtx, const LocalMultiplierType& localL )
{
