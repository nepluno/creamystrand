...
void TwistingForce<ViscousT>::computeLocal( LocalMultiplierType& localL, const ElasticStrand& strand, const IndexType vtx )
{
    ...
    localL = -kt * ilen * (twist - undefTwist + strand.m_currentState->m_gradTwists[vtx].dot(strand.dynamics().getDisplacements().segment<11>(4 * (vtx - 1))));    
}
...
Scalar TwistingForce<ViscousT>::localEnergy( const ElasticStrand& strand, StrandState& geometry,
        const IndexType vtx )
{
    const Scalar kt = ViscousT::kt( strand, vtx ) * strand.getStepper()->getStretchMultiplier();
    const Scalar undefTwist = ViscousT::thetaBar( strand, vtx );
    const Scalar ilen = strand.m_invVoronoiLengths[vtx];
...
template<typename ViscousT>
void TwistingForce<ViscousT>::computeLocal( typename TwistingForce::LocalForceType& localF,
...
template<typename ViscousT>
void TwistingForce<ViscousT>::computeLocal( typename TwistingForce::LocalThetaForceType& localF,
        const ElasticStrand& strand, StrandState& geometry, const IndexType vtx )
{
    const Scalar kt = ViscousT::kt( strand, vtx ) * strand.getStepper()->getStretchMultiplier();
    const Scalar undefTwist = ViscousT::thetaBar( strand, vtx );
...
template<typename ViscousT>
void TwistingForce<ViscousT>::computeLocal( typename TwistingForce::LocalJacobianType& localJ,
        const ElasticStrand& strand, StrandState& geometry, const IndexType vtx )
{
    const Scalar kt = ViscousT::kt( strand, vtx ) * strand.getStepper()->getStretchMultiplier();
    const Scalar ilen = strand.m_invVoronoiLengths[vtx];
...
//    for(int i = 0; i < 3; ++i) {
//        localJ.row(0 + i).setZero();
//        localJ.row(4 + i).setZero();
//        localJ.row(8 + i).setZero();
//        localJ.col(0 + i).setZero();
//        localJ.col(4 + i).setZero();
//        localJ.col(8 + i).setZero();
//    }
}

template<typename ViscousT>
void TwistingForce<ViscousT>::computeLocal( typename TwistingForce::LocalThetaJacobianType& localJ,
        const ElasticStrand& strand, StrandState& geometry, const IndexType vtx )
{
    const Scalar kt = ViscousT::kt( strand, vtx ) * strand.getStepper()->getStretchMultiplier();
    const Scalar undeformedTwist = ViscousT::thetaBar( strand, vtx );
...
template<typename ViscousT>
void TwistingForce<ViscousT>::addInPositionMultiplier( VecXx& globalMultiplier, const IndexType vtx, const LocalMultiplierType& localL )
{
