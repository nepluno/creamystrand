    ...
    for ( int elemId = 0; elemId < m_elementProxies.size() ; ++elemId )
    {
        ...
        if ( elemBBoxSize > s_maxSizeForElementBBox )
        {
            WarningStream( g_log, "" ) << "Element " << *elem << " has large bounding box: "
                    << elemBBox << "; will not be considered for collision detection";
...
void CollisionDetector::updateBoundingBox( BVHNodeType& node )
{
    ...
    if ( node.IsLeaf() ) // The leaf's bounding box contains the whole trajectory of its object(s) during this time step.
    {
        ...
        for ( uint32_t i = leaf_begin; i < leaf_end; ++i )
        {
            ...
            {
                WarningStream( g_log, "" ) << "Element " << *m_elementProxies[i]
...
{
    for ( auto collision = m_continuousTimeCollisions.begin(); collision != m_continuousTimeCollisions.end(); ++collision )
    {
        delete *collision;
    }
    for ( auto collision = m_proximityCollisions.begin(); collision != m_proximityCollisions.end(); ++collision )
    {
        delete *collision;
    }
...
#pragma omp critical (pushCTCollision)
        {
            m_continuousTimeCollisions.insert( m_continuousTimeCollisions.end(),
                                               potentialCollisions.begin(), potentialCollisions.end() );
        }
    }
...
template<>
bool CollisionDetector::appendCollision<EdgeFaceIntersection>( ElementProxy* elem_a,
        ElementProxy* elem_b )
{
    EdgeProxy* const edge_a = dynamic_cast<EdgeProxy*>( elem_a );
...
bool CollisionDetector::appendIntersection( EdgeProxy* edge_a, const FaceProxy* triangle_b )
{
    if ( !triangle_b->allApicesEnabled() )
    {
        return false;
    ...
    EdgeFaceIntersection intersection(  edge_a->getStrandPointer(),
            edge_a->getVertexIndex(), edge_a, triangle_b );
        ...
        if ( &node_a == &node_b ) // As DK noticed, this case still needs to be considered. We still can avoid colliding an element with itself though.
        {
            ...
            if( !m_ignoreContinuousTime )
            {
                for ( unsigned int i = leaf_a_begin; i < leaf_a_end; ++i )
                    ...
                    {
                        appendCollision<ContinuousTimeCollision>( m_elementProxies[i], m_elementProxies[j] );
                    }
            }
            if( !m_ignoreProximity )
            {
                for ( unsigned int i = leaf_a_begin; i < leaf_a_end; ++i )
                    for ( unsigned int j = leaf_a_begin; j < i; ++j )
                    {
                        appendCollision<EdgeFaceIntersection>( m_elementProxies[i], m_elementProxies[j] );
                    }
            }
        }
        else
