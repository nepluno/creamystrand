...
//                }
            ...
            const Scalar cpr = sqrt(m_firstStrand->getRadiusA( m_firstVertex ) * m_firstStrand->getRadiusB( m_firstVertex ));
            ...
            if(cpA > 1e-12) {
                ...
                if(m_firstVertex > 0) {
                    ...
                    const Vec3x dp_prev = m_firstStrand->dynamics().getDisplacement( m_prevVertex );
                    Vec3x pcol_prev = (p_prev + dtime * dp_prev + pcol) * 0.5;
                    Scalar A0 = (m_firstStrand->getCurrentFlowDOFArea( m_prevVertex ) + cpA) * 0.5;
                    
                    const Vec3x cp_prev = ClosestPtPointTriangle( pcol_prev, f0col, f1col, f2col );
                    const Scalar d0 = std::max(0., ( pcol_prev - cp_prev ).dot( m_normal )) + extraRadius;
                    const Scalar l = (cp_prev - cp).norm();
                    
                    const Scalar abs0 = wedge_wet_abscissa(cpA, A0, extraRadius, d0, theta);
                    
                    const Scalar Ac = cpA * (1. - abs0 * 0.5) + A0 * abs0 * 0.5;
                    const Scalar dc = extraRadius * (1. - abs0 * 0.5) + d0 * abs0 * 0.5;
                    
                    const Scalar diff_vel = sgn(relative_vel) * pow(fabs(relative_vel) / std::max(cpr, dc), n);
                    
                    const Scalar elastic_force = 0.75 * M_PI * cpr * ( eta * diff_vel + tilde_sigma_Y );
                    
                    l0 = abs0 * l;
                    
                    m_adhesive_force += (m_firstStrand->collisionParameters().adhesionForcePlanar(Ac, dc, color) + elastic_force) * l0;
                ...
                if(m_firstVertex < num_verts - 1) {
                    ...
                    const Vec3x dp_next = m_firstStrand->dynamics().getDisplacement( m_nextVertex );
                    Vec3x pcol_next = (p_next + dtime * dp_next + pcol) * 0.5;
                    Scalar A1 = (m_firstStrand->getCurrentFlowDOFArea( m_nextVertex ) + cpA) * 0.5;
                    
                    const Vec3x cp_next = ClosestPtPointTriangle( pcol_next, f0col, f1col, f2col );
                    const Scalar d1 = std::max(0., ( pcol_next - cp_next ).dot( m_normal )) + extraRadius;
                    const Scalar l = (cp_next - cp).norm();
                    
                    const Scalar abs1 = wedge_wet_abscissa(cpA, A1, extraRadius, d1, theta);
                    ...
                    m_adhesive_force += (m_firstStrand->collisionParameters().adhesionForcePlanar(Ac, dc, color) + elastic_force) * l1;
