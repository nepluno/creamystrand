...
OrientedBoundingBox::OrientedBoundingBox( const ElasticStrand& strand, const unsigned edge,
        CollisionParameters::CollisionType type )
{
    init( strand.getCurrentState(), edge, strand.collisionParameters(), type );
}

OrientedBoundingBox::OrientedBoundingBox( const StrandState& geometry, const unsigned edge,
...
// Adapated from http://www.geometrictools.com/LibMathematics/Intersection/Wm5IntrBox3Box3.cpp
// Geometric Tools, LLC
        ...
        const OrientedBoundingBox& obb1 )
{
    ...
    const Real cutoff = ( Real ) 1 - SMALL_NUMBER<float>(); //Math<Real>::ZERO_TOLERANCE;
