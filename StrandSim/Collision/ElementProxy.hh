...
public:
    ...
    virtual void computeBoundingBox( BBoxType& boundingBox, bool statique ) const = 0;
...
public:
    ...
    void computeBoundingBox( BBoxType& boundingBox, bool statique ) const
    {
        ...
        if ( !statique ) // If non-static, the strand's current contain the post-step positions
        {
...
public:
    ...
    void computeBoundingBox( BBoxType& boundingBox, bool statique ) const
    {
        ...
        Vec3x min, max ;
        m_strand.getFutureAABB( m_vertexIndex, min, max, CollisionParameters::EXTERNAL ) ;
        boundingBox.min = min.cast< float >() ;
        boundingBox.max = max.cast< float >() ;

        if ( !statique ) // If non-static, the strand's current contain the post-step positions
        {
            m_strand.getAABB( m_vertexIndex, min, max,
