...
void CTCD::distancePoly3D(const Vector3d &x10, const Vector3d &x20,
  ...
  double B = x10.dot(v20.cross(v30)) + v10.dot(x20.cross(v30)) +
             v10.dot(v20.cross(x30));
  double C = x10.dot(x20.cross(v30)) + x10.dot(v20.cross(x30)) +
             v10.dot(x20.cross(x30));
...
bool CTCD::edgeEdgeCTCD(const Eigen::Vector3d &q0start,
  ...
  // towards e2, and on the side of e2 towards e1
  ...
  for (int i = 0; i < rawcoplane.size(); i++) {
    ...
    if (x10.cross(x20).norm() < 1e-8) {
      ...
      std::cout << "parallel edges detected! in CTCD, edgeEdgeCTCD"
                << std::endl;
      std::exit(EXIT_FAILURE);
    } else
      coplane.push_back(rawcoplane[i]);
  }
  if (coplane.empty() && parallel.empty()) return false;
...
bool CTCD::vertexVertexCTCD(const Vector3d &q1start, const Vector3d &q2start,
  ...
  // t term
  double b = 2 * (v1.dot(q1start) - v2.dot(q1start) - v1.dot(q2start) +
                  v2.dot(q2start));
  // current distance - min_d
  double c = q1start.dot(q1start) + q2start.dot(q2start) -
             2 * q1start.dot(q2start) - min_d;
  if (a != 0) {
