...
namespace strandsim
{
    ...
    m_strand( strand ), m_edge( edge ), m_edgeP( edgeP ), m_triangle( triangle ), m_yield(0.), m_eta(0.), m_power(1.),  m_adhesive_force(0.)
    {
    }
    ...
    bool EdgeFaceIntersection::barycentricCoordinatesIfCloser( const Scalar radius, const Scalar A0, const Scalar A1, const Scalar theta, const Scalar s0,
                                                              ...
                                                              Scalar &dist, Scalar &s, Scalar &u, Scalar &v, Scalar &w, Scalar& al, Scalar& A_adh, Scalar& d_adh )
    {
        ...
        if ( d > -radius && d < dist )
        {
            ...
            if ( lu > 0 && lv > 0 && lw > 0 )
            {
                ...
                const Scalar abs_adh = wedge_wet_abscissa(acol, aanother, dcol, danother, theta);
    ...
    bool EdgeFaceIntersection::analyseProximity()
    {
		...
		if(disp_rel > -1e-7) {
			const Scalar area = (m_strand->getCurrentFlowDOFArea(m_edge) + m_strand->getCurrentFlowDOFArea(m_edge + 1)) * 0.5;
        ...
        if( nsign )
        {
            ...
            found = barycentricCoordinatesIfCloser( radius, A0, A1, theta, 1, p0, p1, q0, q1, q2, m_distance, m_s, m_u, m_v,
                                                   m_w, adhesive_length, A_adh, d_adh ) || found;
            ...
            if(A_adh < 1e-12) {
                ...
                const Scalar r = sqrt(m_strand->getRadiusA( m_edge + 1 ) * m_strand->getRadiusB( m_edge + 1 ));
                ...
                const Scalar tilde_sigma_Y = m_strand->getFlowYieldStress(color) * 0.8164965809;
                
                const Scalar relative_vel = disp_rel / m_strand->getDt();
                
                const Scalar tangential_vel = tang_rel / m_strand->getDt();
                
                const Scalar diff_vel = sgn(relative_vel) * pow(fabs(relative_vel) / std::max(r, d_adh), n);
                
                const Scalar diff_vel_horizontal = pow(fabs(tangential_vel) / std::max(r, d_adh), n);
                
                const Scalar elastic_force = 0.75 * M_PI * r * ( eta * diff_vel + tilde_sigma_Y );
                
                m_adhesive_force = (m_strand->collisionParameters().adhesionForcePlanar(A_adh, d_adh, color) + elastic_force) * adhesive_length;
                m_yield = 0.75 * M_PI * r * tilde_sigma_Y * adhesive_length;
                m_eta = 0.75 * M_PI * r * eta / pow(std::max(r, d_adh), n) * adhesive_length;
                m_power = n;
            }
        }
                ...
                if ( lu > 0 && lv > 0 && lw > 0 )
                {
                    ...
                    if( nsign && m_distance > 0 )
                    {
                        ...
                        if(cpA < 1e-12) {
                            ...
                            const Scalar cpr = sqrt(m_strand->getRadiusA( m_edge ) * m_strand->getRadiusB( m_edge ) * (1. - s0) + m_strand->getRadiusA( m_edge + 1 ) * m_strand->getRadiusB( m_edge + 1 ) * s0);
							...
							VecXx color = (m_strand->getFlowNewComponents( m_edge ) + m_strand->getFlowNewComponents( m_edge + 1 ));
                            ...
                            const Scalar diff_vel_0 = sgn(relative_vel) * pow(fabs(relative_vel) / std::max(cpr, dc0), n);
                            
                            const Scalar elastic_force_0 = 0.75 * M_PI * cpr * ( eta * diff_vel_0 + tilde_sigma_Y );

                            const Scalar diff_vel_1 = sgn(relative_vel) * pow(fabs(relative_vel) / std::max(cpr, dc1), n);

                            const Scalar elastic_force_1 = 0.75 * M_PI * cpr * ( eta * diff_vel_1 + tilde_sigma_Y );

                            m_adhesive_force = (m_strand->collisionParameters().adhesionForcePlanar(Ac0, dc0, color) + elastic_force_0) * l0 + (m_strand->collisionParameters().adhesionForcePlanar(Ac1, dc1, color) + elastic_force_1) * l1;
                            
                            m_yield = 0.75 * M_PI * cpr * tilde_sigma_Y * (l0 + l1);
                            m_eta = 0.75 * M_PI * cpr * eta * (l0 / pow(std::max(cpr, dc0), n) + l1 / pow(std::max(cpr, dc1), n));
                            m_power = n;
                        }
                    }
        ...
        for ( unsigned k = 0; k < 3; ++k )
        {
            ...
            if ( accept )
            {
                ...
                if(cpA < 1e-12) {
                    ...
                    const Scalar cpr = sqrt(m_strand->getRadiusA( m_edge ) * m_strand->getRadiusB( m_edge ) * (1. - m_s) + m_strand->getRadiusA( m_edge + 1 ) * m_strand->getRadiusB( m_edge + 1 ) * m_s);
            ...
            if ( ( pcol - cp ).squaredNorm() <= SQ_TOLERANCE )
            {
                ...
                if ( m_u > 0 && m_v > 0 && m_w > 0 ) {
                    ...
                    if(cpA < 1e-12) {
                        ...
                        const Scalar tang_rel = sqrt(std::max(0., dpq.squaredNorm() - disp_rel * disp_rel));
