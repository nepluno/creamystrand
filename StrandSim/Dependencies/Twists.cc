...
void HessTwists::compute()
{
    ...
    for ( IndexType vtx = m_firstValidIndex; vtx < size(); ++vtx )
    {
        ...
        const Mat3x& D2mDe2 = -0.5 / (chi * square( norm_e )) * symPart( outerProd<3>(kb, tilde_t + (1.0 + tilde_t.dot(tf)) * te) );
        const Mat3x& D2mDf2 = -0.5 / (chi * square( norm_f )) * symPart( outerProd<3>(kb, tilde_t + (1.0 + tilde_t.dot(te)) * tf) );
        const Mat3x& D2mDeDf = 1.0 / (norm_e * norm_f * chi) * ( crossMat(te) - 0.5 * outerProd<3>(kb, tilde_t + tilde_t.dot(te) * tf) );
