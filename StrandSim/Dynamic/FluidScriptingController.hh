...
namespace strandsim
{
	...
	public:
		struct FluidTiming
		{
			FluidTiming() : stress( 0 ), datastructure( 0 ), processparts( 0 ),
			solidproj( 0 ), plasticity( 0 ), weight( 0 ), advect( 0 ), mapp2g( 0 ), addforce( 0 ), computephi( 0 ),
			solvedrag( 0 ), solvevisc( 0 ), solvepressure( 0 ), applypressure( 0 ), syncflow( 0 ),
			constraintvel( 0 ), mapg2p( 0 )
			{
			}
			...
			double sum() const
			{
				return stress + datastructure + processparts + weight + advect + mapp2g + addforce + computephi + applypressure + syncflow
				+ solvedrag + solvevisc + solvepressure + constraintvel + mapg2p + solidproj + plasticity;
			}
		};
	...
	public:
		...
		virtual Vec3x computeProjFunc(const Mat3x& b_bar_trial, const Scalar& plastic_yield_stress, const Scalar& liquid_shear_modulus, const Scalar& flow_consistency_index, const Scalar& flow_behavior_index) const = 0;
		...
		virtual Scalar get_solid_phi_gradient(const Vec3x& position, Vec3x& n) const = 0;
