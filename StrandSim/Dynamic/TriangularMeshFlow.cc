...
namespace strandsim
{
    TriangularMeshFlow::TriangularMeshFlow( const std::shared_ptr< TriangularMesh >& mesh, const std::shared_ptr< FluidScriptingController >& controller, const Scalar& init_height )
...
//
    ...
    Scalar TriangularMeshFlow::getFaceHeight(int face_idx) const
    {
        ...
        return (m_current_flow_height( f(0) ) + m_current_flow_height( f(1) ) + m_current_flow_height( f(2) )) / 3.0;
    }
    ...
    void TriangularMeshFlow::step_dynamics( const Scalar dt )
    {
        ...
        m_mesh->mapFaceVecToVertex(m_current_flow_velocity, m_current_vertex_flow_velocity, 3);
    }
    ...
    void TriangularMeshFlow::advect_flow( const Scalar dt )
    {
        ...
        for_each(0, num_faces, [&] (int i) {
            int target;
            Vec3x coord;
            m_mesh->getClosestBarycentric(i, -m_current_flow_velocity.segment<3>(i * 3) * dt, target, coord);
            ...
            // sample from target
            Vec3x sampled_vel = m_current_vertex_flow_velocity.segment<3>( f(0) * 3 ) * coord(0) + m_current_vertex_flow_velocity.segment<3>( f(1) * 3 ) * coord(1) + m_current_vertex_flow_velocity.segment<3>( f(2) * 3 ) * coord(2);
            m_future_flow_velocity.segment<3>(i * 3) = parallelTransport( sampled_vel, norm_target, norm_current );
        });
        ...
        for_each(0, num_verts, [&] (int i) {
            ...
            const Vec3x offset_start = (m_mesh->m_vertices[ f_start(0) ] + m_mesh->m_vertices[ f_start(1) ] + m_mesh->m_vertices[ f_start(2) ]) / 3.0 - m_mesh->m_vertices[i];
            
            int target;
            Vec3x coord;
            m_mesh->getClosestBarycentric(start_face_idx, -vel * dt - offset_start, target, coord);
            
            const TriangularFace& f_target = m_mesh->m_faces[target];
            
            Scalar h = m_current_flow_height( f_target(0) ) * coord(0) + m_current_flow_height( f_target(1) ) * coord(1) + m_current_flow_height( f_target(2) ) * coord(2);
            m_future_flow_height(i) = h;
            
            VecXx c = m_current_flow_components.segment( f_target(0) * num_components, num_components ) * m_current_flow_height( f_target(0) ) * coord(0) +
            m_current_flow_components.segment( f_target(1) * num_components, num_components ) * m_current_flow_height( f_target(1) ) * coord(1) +
            m_current_flow_components.segment( f_target(2) * num_components, num_components ) * m_current_flow_height( f_target(2) ) * coord(2);
    ...
    Scalar TriangularMeshFlow::getFaceVol( int face_idx ) const
    {
        ...
        Scalar avg_fh = (m_current_flow_height( f(0) ) + m_current_flow_height( f(1) ) + m_current_flow_height( f(2) )) / 3.0;
    ...
    void TriangularMeshFlow::matrix_from_vec(const VecXx& vec, Eigen::SparseMatrix<Scalar>& mat)
    {
        ...
        {
            if(vec(i) != 0.0)
                tri_mat.push_back( Eigen::Triplet<Scalar>(i, i, vec(i)) );
        }
        mat.setFromTriplets(tri_mat.begin(), tri_mat.end());
    }
    
    void TriangularMeshFlow::matrix_from_vec3s(const VecXx& vec, Eigen::SparseMatrix<Scalar>& mat)
    {
    ...
    void TriangularMeshFlow::update_flow_height( const Scalar dt )
    {
        ...
        for_each(0, num_verts, [&] (int i) {
            m_future_flow_height(i) *= exp(-div_u(i) * dt);
        });
        
        if(m_controller->getMeshFlowEpsilon() > 0.0) {
            Eigen::SparseMatrix<Scalar> IL(num_verts, num_verts);
            IL.setIdentity();
            IL += m_mesh->m_G_v * m_mesh->m_laplacian_operator * m_controller->getMeshFlowEpsilon();
            ...
            m_future_flow_height = cg.solveWithGuess(m_future_flow_height, m_future_flow_height);
        }
        ...
        for_each(0, num_verts, [&] (int i) {
            Scalar bh = m_touched[ i ] ? b : 0.0;
            m_future_flow_height(i) = std::min(max_flow_h, std::max(0.0, (m_future_flow_height(i) - bh) * prop)) + bh;
        ...
        for_each(0, num_verts, [&] (int i) {
            mat_vert_flow_vel.block<1, 3>(i, 0) = vertex_flow_vel.segment<3>(i * 3).transpose();
        });
        ...
        for_each(0, num_faces, [&] (int i) {
            ...
            if(m_future_flow_height( f(0) ) < 1e-20 && m_future_flow_height( f(1) ) < 1e-20 && m_future_flow_height( f(2) ) < 1e-20) {
                m_flow_cauchy_strain.block<3, 3>(i * 3, 0).setIdentity();
                return;
            }
            
            const Mat3x gradu = mat_grad_face_flow_vel.block<3, 3>(i * 3, 0).transpose();
            ...
            VecXx color = m_future_flow_components.segment( f(0) * num_components, num_components ) + m_future_flow_components.segment( f(1) * num_components, num_components ) + m_future_flow_components.segment( f(2) * num_components, num_components );
            
            make_gibbs_simplex(color);
            
            Vec3x proj_func = m_controller->computeProjFunc(be_bar_trial, m_controller->getYieldStress(color), m_controller->getShearModulus(color), m_controller->getViscosity(color), m_controller->getFlowBehaviorIndex(color) );
    ...
    void TriangularMeshFlow::transform_base()
    {
        ...
        for_each(0, num_faces, [this] (int i) {
            ...
            m_current_flow_velocity.segment<3>(i * 3) = rot * Vec3x(m_current_flow_velocity.segment<3>(i * 3));
            m_future_flow_velocity.segment<3>(i * 3) = rot * Vec3x(m_future_flow_velocity.segment<3>(i * 3));
            
            Mat3x mrot = rot.toRotationMatrix();
            m_flow_cauchy_strain.block<3, 3>(i * 3, 0) = mrot * m_flow_cauchy_strain.block<3, 3>(i * 3, 0) * mrot.transpose();
        });
    }
    ...
    void TriangularMeshFlow::solve_flow_velocity()
    {
        ...
        for_each(0, num_faces, [this] (int i) {
            if(m_flow_lhs(i * 3) > 1e-12) {
                m_future_flow_velocity.segment<3>(i * 3) = Vec3x( m_flow_rhs.segment<3>( i * 3 ).array() / m_flow_lhs.segment<3>( i * 3 ).array() );
            } else {
                m_future_flow_velocity.segment<3>(i * 3).setZero();
            }
            
            Vec3x normal = m_mesh->m_face_frames.block<3, 1>(i * 3, 1);
            m_future_flow_velocity.segment<3>(i * 3) = (Mat3x::Identity() - normal * normal.transpose()) * m_future_flow_velocity.segment<3>(i * 3);
        });
    }
...
//        VecXx grad_height = m_mesh->m_grad_operator * m_future_flow_height;
        ...
        for_each(0, num_faces, [&] (int i) {
            ...
            Scalar h_avg = (m_future_flow_height( f(0) ) + m_future_flow_height( f(1) ) + m_future_flow_height( f(2) )) / 3.0;
			
			VecXx color = m_current_flow_components.segment( f(0) * num_components, num_components ) + m_current_flow_components.segment( f(1) * num_components, num_components ) + m_current_flow_components.segment( f(2) * num_components, num_components );
			
			make_gibbs_simplex(color);
			
            m_flow_mass_face(i) = m_mesh->m_face_areas(i) * h_avg * m_controller->getDensity(color);

            mu_dev_b.block<3, 3>(i * 3, 0) = (m_flow_cauchy_strain.block<3, 3>(i * 3, 0) - Mat3x::Identity() * m_flow_cauchy_strain.block<3, 3>(i * 3, 0).trace() / 3.0) * m_controller->getShearModulus(color);
        });
        
        MatXx div_sigma = m_mesh->m_weight_vertex_face * m_mesh->m_div_operator * mu_dev_b;
        ...
        for_each(0, num_faces, [&] (int i) {
            const TriangularFace& f = m_mesh->m_faces[i];
            Vec3x cur_pos = (m_mesh->m_vertices[ f(0) ] + m_mesh->m_vertices[ f(1) ] + m_mesh->m_vertices[ f(2) ]) / 3.0;
            Scalar dist = m_controller->get_dense_liquid_signed_distance(cur_pos);
            if(dist < -1.0 * dx) {
                Vec3x vel = m_controller->get_velocity(cur_pos);
                Vec3x normal = m_mesh->m_face_frames.block<3, 1>(i * 3, 1);
                
                m_flow_rhs.segment<3>(i * 3) = (Mat3x::Identity() - normal * normal.transpose()) * vel * m_flow_mass_face(i);
            } else {
                const TriangularFace& f = m_mesh->m_faces[i];
                Vec3x cur_disp = (m_mesh->m_displacements[ f(0) ] + m_mesh->m_displacements[ f(1) ] + m_mesh->m_displacements[ f(2) ]) / 3.0;
                Vec3x prev_disp = (m_mesh->m_previous_displacements[ f(0) ] + m_mesh->m_previous_displacements[ f(1) ] + m_mesh->m_previous_displacements[ f(2) ]) / 3.0;
                ...
                Vec3x gt = (GravitationForce::getGravity() * dt - (cur_disp - prev_disp) / dt) * m_flow_mass_face(i) + shear_force * vol * dt;

    //            Vec3x gh = grad_height.segment<3>( i * 3 );
                
                m_flow_rhs.segment<3>(i * 3) = ( m_future_flow_velocity.segment<3>(i * 3) * m_flow_mass_face(i) + (Mat3x::Identity() - normal * normal.transpose()) * gt );
            }
        });
        ...
        for_each(0, num_faces, [&] (int i) {
            ...
            Scalar hu = h_avg / std::max(m_future_flow_velocity.segment<3>(i * 3).norm(), 1e-7);
