...
#include "../Forces/Bridson/pcgsolver/blas_wrapper.hh"
#include "../Forces/Bridson/pcgsolver/sparse_matrix.hh"
#include "../Utils/ThreadUtils.hh"

using namespace std;
using namespace strandsim::bridson;
...
struct levelGen {
  ...
  void generateLevelsGalerkinCoarsening(vector<FixedSparseMatrix<T> >& A_L,
                                        ...
                                        // given
                                        FixedSparseMatrix<T>& A, int ni, int nj,
                                        int nk);
  ...
  void generateRPCompressed(const FixedSparseMatrix<T>& A,
    ...
    int slice = ni * nj;
    for_each(0, compute_num, [&](int thread_idx) {
  ...
  void generateLevelsGalerkinCoarseningCompressed(
      vector<FixedSparseMatrix<T> >& A_L, vector<FixedSparseMatrix<T> >& R_L,
      vector<FixedSparseMatrix<T> >& P_L, vector<vector<bool> >& b_L,
      int& total_level,
      // given
      FixedSparseMatrix<T>& A, vector<char>& mask, vector<int>& index_table,
    ...
    while (unknowns > 4096) {
      ...
      generateRPCompressed((A_L[i]), (R_L[i]), (P_L[i]), (b_L[i]), mask0,
                           idxTable0, mask_temp, idxTable_temp, S_L[i][0],
                           S_L[i][1], S_L[i][2]);
      ...
      idxTable_temp.resize(0);
      idxTable_temp.shrink_to_fit();
      mask_temp.resize(0);
      mask_temp.shrink_to_fit();

      unknowns = A_L[i + 1].n;
      total_level++;
    }

    generatePattern(mask0, idxTable0, (b_L[total_level - 1]),
                    S_L[total_level - 1][0], S_L[total_level - 1][1],
                    S_L[total_level - 1][2]);
  ...
  void generateRPSparse(const FixedSparseMatrix<T>& A, FixedSparseMatrix<T>& R,
                        ...
                        vector<Vec3i>& Dof_ijk_coarse, int ni, int nj, int nk) {
  ...
  void generatePatternSparse(vector<Vec3i>& Dof_ijk, vector<bool>& pattern) {
    pattern.resize(Dof_ijk.size());
    for_each((int)0, (int)Dof_ijk.size(), [&](int index) {
  ...
  void generateLevelsGalerkinCoarseningSparse(
      ...
      // given
      const std::shared_ptr<FixedSparseMatrix<T> >& A, vector<Vec3i>& Dof_ijk,
      int ni, int nj, int nk) {
#ifdef AMG_VERBOSE
    cout << "building levels ...... " << endl;
#endif
