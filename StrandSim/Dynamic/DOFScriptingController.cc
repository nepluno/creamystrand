...
namespace strandsim
{
    ...
    void DOFScriptingController::enforceDisplacements( VecXx& displacements, Scalar fraction ) const
    {
        for ( std::map<int, Scalar>::const_iterator dof = m_scriptedDegreesOfFreedom.begin();
             dof != m_scriptedDegreesOfFreedom.end(); ++dof )
        {
    ...
    void DOFScriptingController::update( StrandState& geometry, const unsigned subStepId,
