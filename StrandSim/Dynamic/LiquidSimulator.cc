...
namespace strandsim
{
	...
	: FluidScriptingController(0, dt_), m_bucket_size(bucket_size_),
	m_num_nodes(num_cells_), m_liquid_info(li_), m_fields(fields_),
	m_sources(sources_), m_terminators(terminators_), m_strands(strands_)
	{
        ...
        if(num_strands > 0) {
            ...
            for_each(0, num_strands, [&] (int i) {
                tmp_buf[i] = m_strands[i]->getNumVertices();
            });
            
            std::partial_sum(tmp_buf.begin(), tmp_buf.end(), tmp_buf.begin());
            
            for_each(0, num_strands, [&] (int i) {
                if(i == 0) m_strands_local_global_base[i] = 0;
                else m_strands_local_global_base[i] = tmp_buf[i - 1];
            });
        ...
        if(num_fields > 0) {
            ...
            for_each(0, num_fields, [&] (int i) {
                ...
                std::shared_ptr<TriangularMesh> mesh = m_fields[i].mesh_controller->getCurrentMesh();
                assert(mesh);
            ...
            for_each(0, num_fields, [&] (int i) {
                if(i == 0) m_mesh_local_global_base[i] = 0;
                else m_mesh_local_global_base[i] = tmp_buf[i - 1];
            });
	...
	VecXx LiquidSimulator::getComponentsAtPos(const Vec3x& pos) const
	{
		...
		VecXx color = interpolateValue(pos, m_node_components, m_grid_mincorner + Vec3x(0.5, 0.5, 0.5) * dx, VecXx::Zero( m_liquid_info.num_components ), 1, m_liquid_info.num_components);
	...
	int LiquidSimulator::getMaxIters() const
	{
		return m_liquid_info.pcg_max_iters;
	}
	...
	{
        const Scalar dx = getDx();
        return 1.0 - interpolateValue(pos, m_node_elastic_vf_p, m_grid_mincorner + Vec3x(0.5, 0.5, 0.5) * dx, 0.0);
	}
	...
	Vec3x LiquidSimulator::nodePosFromBucket(int bucket_idx, int raw_node_idx, const Vec3x& offset) const
	{
		Vec3i handle = m_particle_buckets.bucket_handle(bucket_idx);
		Vec3x bucket_left_corner = m_grid_mincorner + Vec3x(handle(0) * m_bucket_size, handle(1) * m_bucket_size, handle(2) * m_bucket_size);
	...
	Vec3x LiquidSimulator::getNodePosX(int bucket_idx, int node_idx) const
	{
		if(m_bucket_activated[bucket_idx]) {
			const Scalar dx = getDx();
			return m_node_pos[bucket_idx].segment<3>(node_idx * 3) + Vec3x(0.0, 0.5, 0.5) * dx;
		} else {
	...
	Vec3x LiquidSimulator::getNodePosY(int bucket_idx, int node_idx) const
	{
		if(m_bucket_activated[bucket_idx]) {
			const Scalar dx = getDx();
			return m_node_pos[bucket_idx].segment<3>(node_idx * 3) + Vec3x(0.5, 0.0, 0.5) * dx;
		} else {
	...
	Vec3x LiquidSimulator::getNodePosZ(int bucket_idx, int node_idx) const
	{
		if(m_bucket_activated[bucket_idx]) {
			const Scalar dx = getDx();
			return m_node_pos[bucket_idx].segment<3>(node_idx * 3) + Vec3x(0.5, 0.5, 0.0) * dx;
		} else {
	...
	Vec3x LiquidSimulator::getNodePosP(int bucket_idx, int node_idx) const
	{
		if(m_bucket_activated[bucket_idx]) {
			const Scalar dx = getDx();
			return m_node_pos[bucket_idx].segment<3>(node_idx * 3) + Vec3x(0.5, 0.5, 0.5) * dx;
		} else {
	...
	Scalar LiquidSimulator::solveHerschelBulkley(Scalar s_tr, Scalar dt, Scalar inv_eta, Scalar inv_m, Scalar yieldStress, Scalar __mu__)
	{
		const Scalar yc = sqrt(2.0/3.0) * yieldStress;
		return bisection_root_finding(s_tr, yc, m_liquid_info.pressure_pcg_criterion,
    ...
    Scalar LiquidSimulator::get_solid_phi_gradient(const Vec3x& position, Vec3x& n) const
    {
        const Scalar dx = getDx();
        return interpolateValueAndGradient(n, position, m_node_solid_phi, m_grid_mincorner, 3.0 * dx);
    }
    
    Vec3x LiquidSimulator::get_solid_gradient(const Vec3x& position) const
    {
        const Scalar dx = getDx();
        return interpolateGradient(position, m_node_solid_phi, m_grid_mincorner, 3.0 * dx);
    }
    ...
    Mat3x LiquidSimulator::get_solid_hessian(const Vec3x& position) const
    {
        const Scalar dx = getDx();
        Mat3x m;
        m.row(0) = (get_solid_gradient(position + Vec3x(0.5, 0.0, 0.0) * dx) - get_solid_gradient(position - Vec3x(0.5, 0.0, 0.0) * dx)) / dx;
        m.row(1) = (get_solid_gradient(position + Vec3x(0.0, 0.5, 0.0) * dx) - get_solid_gradient(position - Vec3x(0.0, 0.5, 0.0) * dx)) / dx;
        m.row(2) = (get_solid_gradient(position + Vec3x(0.0, 0.0, 0.5) * dx) - get_solid_gradient(position - Vec3x(0.0, 0.0, 0.5) * dx)) / dx;
    ...
    Vec3x LiquidSimulator::get_pressure_gradient(const Vec3x& position) const
    {
        const Scalar dx = getDx();
        Vec3x v;
        v(0) = interpolateValue(position, m_node_pressure_grad_x, m_grid_mincorner + Vec3x(0.0, 0.5, 0.5) * dx, 0.0);
        v(1) = interpolateValue(position, m_node_pressure_grad_y, m_grid_mincorner + Vec3x(0.5, 0.0, 0.5) * dx, 0.0);
        v(2) = interpolateValue(position, m_node_pressure_grad_z, m_grid_mincorner + Vec3x(0.5, 0.5, 0.0) * dx, 0.0);
    ...
    Mat3x LiquidSimulator::get_pressure_hessian(const Vec3x& position) const
    {
        const Scalar dx = getDx();
        Mat3x m;
        m.row(0) = (get_pressure_gradient(position + Vec3x(0.5, 0.0, 0.0) * dx) - get_pressure_gradient(position - Vec3x(0.5, 0.0, 0.0) * dx)) / dx;
        m.row(1) = (get_pressure_gradient(position + Vec3x(0.0, 0.5, 0.0) * dx) - get_pressure_gradient(position - Vec3x(0.0, 0.5, 0.0) * dx)) / dx;
        m.row(2) = (get_pressure_gradient(position + Vec3x(0.0, 0.0, 0.5) * dx) - get_pressure_gradient(position - Vec3x(0.0, 0.0, 0.5) * dx)) / dx;
    ...
    Vec3x LiquidSimulator::computeProjFunc(const Mat3x& be_bar_trial, const Scalar& plastic_yield_stress, const Scalar& liquid_shear_modulus, const Scalar& flow_consistency_index, const Scalar& flow_behavior_index) const
    {
        auto dgdsmu = [](const Scalar& s, const Scalar& sig, const Scalar& eta, const Scalar& mu, const Scalar& n, const Scalar& h) -> Vec3x
        {
            if(eta == 0.0) {
                ...
                return Vec3x(t3-t6*t8, -sig*t4+sig*t4*t6, h*t5*t6*t8*2.0);
            } else {
                ...
                return Vec3x(t12*t13, -1.0/(s*s)*t12+pow(t4,t5-1.0)*t13*t16, h*t7*t13*t16*-2.0);
            }
        };
        
        const Scalar Ie_bar = be_bar_trial.trace() / 3.0;
        const Mat3x s_trial = liquid_shear_modulus * (be_bar_trial - Mat3x::Identity() * Ie_bar);
        ...
        if(phi_trial <= 0.0) {
            ...
            return dgdsmu(s_trial_len, tilde_sigma_Y, flow_consistency_index, __mu__, flow_behavior_index, m_dt);
        }
    }
...
//            Mat3x B = Mat3x::Zero();
			...
			for(int i = 0; i < indices_x.rows(); ++i)
			{
                ...
                gp(0) += m_node_pressure_grad_x[node_bucket_idx](node_idx) * weights(i, 0);
...
//                B.row(0) += fnv * grad_x.row(i);
			...
			for(int i = 0; i < indices_y.rows(); ++i)
			{
                ...
                gp(1) += m_node_pressure_grad_y[node_bucket_idx](node_idx) * weights(i, 1);
//                B.row(1) += fnv * grad_y.row(i);
			...
			for(int i = 0; i < indices_z.rows(); ++i)
			{
                ...
                gp(2) += m_node_pressure_grad_z[node_bucket_idx](node_idx) * weights(i, 2);
...
//                B.row(2) += fnv * grad_z.row(i);
            ...
            const VecXx& color = m_components.segment( pidx * m_liquid_info.num_components, m_liquid_info.num_components );
            ...
            if(liquid_shear_modulus > 1e-20) {
                ...
                if(!m_weakened[pidx])
                {
                    ...
                    const Scalar largestPrevEigenValue = std::max(std::max(Sp(0), Sp(1)), Sp(2));
                    
                    es.compute(be_bar_trial);
                    const Vec3x Sn = es.eigenvalues();
                    const Scalar largestTrialEigenValue = std::max(std::max(Sn(0), Sn(1)), Sn(2));
                    
                    if(largestTrialEigenValue > largestPrevEigenValue)
                    {
                        //cancel the shear
                        //compute polar decomposition of f_bar, and only use its rotation part;
                        //i.e., compute svd of f_bar, and set the magnitudes of the singular values to one (retaining its sign).
                        
                        Eigen::JacobiSVD<Mat3x> svd(f_bar, Eigen::ComputeFullU | Eigen::ComputeFullV);
                ...
                // Apply Plasticity
                Vec3x proj_func = computeProjFunc(be_bar_trial, getYieldStress(color), liquid_shear_modulus, getViscosity(color), getFlowBehaviorIndex(color));
                
                 // check_isnan("proj_func", proj_func.sum());
                
                const Scalar Ie_bar = be_bar_trial.trace() / 3.0;
                
                const Mat3x new_be_bar = proj_func(0) * (be_bar_trial - Ie_bar * Mat3x::Identity()) + Ie_bar * Mat3x::Identity();
            ...
            m_B.block<3, 3>(pidx * 3, 0) = ((cstretch + crotate) * C + (cstretch - crotate) * C.transpose()) * 0.5;
    ...
    bool LiquidSimulator::updateDragCoeffNodes()
    {
        ...
        m_vertex_buckets.for_each_bucket([&] (int bucket_idx) {
            ...
            {
                // ignore air nodes
                if(m_node_mass_fluid_x[bucket_idx][i] < 1e-20)
                    continue;
            ...
            {
                // ignore air nodes
                if(m_node_mass_fluid_y[bucket_idx][i] < 1e-20)
                    continue;
            ...
            {
                // ignore air nodes
                if(m_node_mass_fluid_z[bucket_idx][i] < 1e-20)
                    continue;
        ...
        // check_isnan("drag_coeff", m_node_drag_coeff_x, m_node_drag_coeff_y, m_node_drag_coeff_z);
	...
	bool LiquidSimulator::map_p2g()
	{
		...
		m_particle_buckets.for_each_bucket([&] (int bucket_idx) {
			...
			for(int i = 0; i < num_nodes; ++i)
			{
                ...
                for(auto& pair : node_vertex_x) {
                    ...
                    p_elastic += m_strands[strand_idx]->getStepper()->velocities()(local_idx * 4 + 0) * mass * w;
                    m_elastic += mass * w;
                }
			...
			for(int i = 0; i < num_nodes; ++i)
			{
                ...
                for(auto& pair : node_vertex_y) {
                    ...
                    const Scalar mass = m_strands[strand_idx]->getVertexMass(local_idx);
                    p_elastic += m_strands[strand_idx]->getStepper()->velocities()(local_idx * 4 + 1) * mass * w;
                    m_elastic += mass * w;
                }
			...
			for(int i = 0; i < num_nodes; ++i)
			{
                ...
                for(auto& pair : node_vertex_z) {
                    ...
                    const Scalar mass = m_strands[strand_idx]->getVertexMass(local_idx);
                    p_elastic += m_strands[strand_idx]->getStepper()->velocities()(local_idx * 4 + 2) * mass * w;
                    m_elastic += mass * w;
                }
			...
			for(int i = 0; i < num_nodes; ++i)
			{
                ...
                m_node_components[bucket_idx].segment(i * m_liquid_info.num_components, m_liquid_info.num_components) = color_fluid;
                ...
                m_node_elastic_vf_p[bucket_idx][i] = clamp(vol_elastic / dV, 0.0, max_packing);
			}
        ...
        // check_isnan("fluidvel_after_p2g", m_node_vel_fluid_plus_x, m_node_vel_fluid_plus_y, m_node_vel_fluid_plus_z);
    ...
    void LiquidSimulator::constrainSurfFlowVelocity()
    {
        ...
        // check_isnan("flowvel_before_constraint", (int) m_strands.size(), [this] (int i) {return m_strands[i]->getStepper()->getFutureFlowVelocity();});
        ...
        for_each(0, num_edges, [&] (int eidx) {
            ...
            const Vec3x u_s = (disp.segment<3>(local_idx * 4) + disp.segment<3>((local_idx + 1) * 4)) * 0.5 / m_dt;
    ...
    void LiquidSimulator::updateSurfFlowConstraint()
    {
        ...
        // check_isnan("rhs_after_constraint", m_node_rhs_fluid_x, m_node_rhs_fluid_y, m_node_rhs_fluid_z);
        // check_isnan("coeff_after_constraint", m_node_vel_constraint_coeff_x, m_node_vel_constraint_coeff_y, m_node_vel_constraint_coeff_z);
    }
	...
	void LiquidSimulator::computeRHS()
	{
        ...
        // check_isnan("mass before compute rhs", m_node_mass_fluid_x, m_node_mass_fluid_y, m_node_mass_fluid_z);
        // check_isnan("drag before compute rhs", m_node_drag_coeff_x, m_node_drag_coeff_y, m_node_drag_coeff_z);
        // check_isnan("vel elastic before compute rhs", m_node_vel_elastic_x, m_node_vel_elastic_y, m_node_vel_elastic_z);
        // check_isnan("vel constraint before compute rhs", m_node_vel_constraint_coeff_x, m_node_vel_constraint_coeff_y, m_node_vel_constraint_coeff_z);
        ...
        if(!m_liquid_info.use_implicit_pressure) {
            m_particle_buckets.for_each_bucket([&] (int bucket_idx) {
                ...
                for(int i = 0; i < num_nodes; ++i)
                {
                    ...
                    Scalar force = m_node_mass_fluid_x[bucket_idx][i] * g(0) + m_node_drag_coeff_x[bucket_idx][i] * m_node_vel_elastic_x[bucket_idx][i];
                    ...
                    if(m_liquid_info.solve_viscosity) {
                        for(auto& pair : node_particles_x) {
                            ...
                            const Vec3x& pos = m_x.segment<3>(pidx * 3);
                            const Mat3x devb = m_b_plus.block<3, 3>(pidx * 3, 0) - m_b_plus.block<3, 3>(pidx * 3, 0).trace() / 3.0 * Mat3x::Identity();
                            const VecXx& color = m_components.segment(pidx * m_liquid_info.num_components, m_liquid_info.num_components);
                            const Scalar pressure = getBulkModulus(color) * 0.5 * (m_J(pidx) * m_J(pidx) - 1.0);
                            
                            // check_isnan("devb during rhs x", devb.sum());
                            // check_isnan("w during rhs x", m_particle_weights[pidx](pair.second, 0));
                            
                            const Scalar liquid_shear_modulus = getShearModulus(color);
                            const Vec3x dp = np - pos;
                            
                            force += -(liquid_shear_modulus * devb.row(0).dot(dp) + pressure * dp(0)) * v0 * iD * m_particle_weights[pidx](pair.second, 0);
                        }
                    }
                ...
                for(int i = 0; i < num_nodes; ++i)
                {
                    ...
                    Scalar force = m_node_mass_fluid_y[bucket_idx][i] * g(1) + m_node_drag_coeff_y[bucket_idx][i] * m_node_vel_elastic_y[bucket_idx][i];
                    ...
                    if(m_liquid_info.solve_viscosity) {
                        for(auto& pair : node_particles_y) {
                            ...
                            // check_isnan("w during rhs y", m_particle_weights[pidx](pair.second, 1));
                            
                            const Scalar liquid_shear_modulus = getShearModulus(color);
                            const Vec3x dp = np - pos;
                            
                            force += -(liquid_shear_modulus * devb.row(1).dot(dp) + pressure * dp(1)) * v0 * iD * m_particle_weights[pidx](pair.second, 1);
                        }
                    }
                ...
                for(int i = 0; i < num_nodes; ++i)
                {
                    ...
                    Scalar force = m_node_mass_fluid_z[bucket_idx][i] * g(2) + m_node_drag_coeff_z[bucket_idx][i] * m_node_vel_elastic_z[bucket_idx][i];
                    ...
                    if(m_liquid_info.solve_viscosity) {
                        for(auto& pair : node_particles_z) {
                            ...
                            // check_isnan("w during rhs z", m_particle_weights[pidx](pair.second, 2));
                            
                            const Scalar liquid_shear_modulus = getShearModulus(color);
                            const Vec3x dp = np - pos;
                            
                            force += -(liquid_shear_modulus * devb.row(2).dot(dp) + pressure * dp(2)) * v0 * iD * m_particle_weights[pidx](pair.second, 2);
                        }
                    }
                    ...
                    if(m_liquid_info.solve_viscosity) {
                        for(auto& pair : node_particles_x) {
                            ...
                            if(liquid_shear_modulus > 1e-20) {
                                ...
                                force += -liquid_shear_modulus * v0 * devb.row(0).dot(np - pos) * iD * m_particle_weights[pidx](pair.second, 0);
                            }
                        }
                    }
                    ...
                    if(m_liquid_info.solve_viscosity) {
                        for(auto& pair : node_particles_y) {
                            ...
                            if(liquid_shear_modulus > 1e-20) {
                                ...
                                force += -liquid_shear_modulus * v0 * devb.row(1).dot(np - pos) * iD * m_particle_weights[pidx](pair.second, 1);
                            }
                        }
                    }
                    ...
                    if(m_liquid_info.solve_viscosity) {
                        for(auto& pair : node_particles_z) {
                            ...
                            if(liquid_shear_modulus > 1e-20) {
                                ...
                                force += -liquid_shear_modulus * v0 * devb.row(2).dot(np - pos) * iD * m_particle_weights[pidx](pair.second, 2);
                            }
                        }
                    }
        ...
        // check_isnan("mass after compute rhs", m_node_mass_fluid_x, m_node_mass_fluid_y, m_node_mass_fluid_z);
        // check_isnan("drag after compute rhs", m_node_drag_coeff_x, m_node_drag_coeff_y, m_node_drag_coeff_z);
        // check_isnan("vel elastic after compute rhs", m_node_vel_elastic_x, m_node_vel_elastic_y, m_node_vel_elastic_z);
        // check_isnan("vel constraint after compute rhs", m_node_vel_constraint_coeff_x, m_node_vel_constraint_coeff_y, m_node_vel_constraint_coeff_z);
        // check_isnan("vol after compute rhs", m_vol);
        // check_isnan("x after compute rhs", m_x);
	}
    
    Scalar LiquidSimulator::lengthNodeVectors( const std::vector< VecXx >& node_vec_ax, const std::vector< VecXx >& node_vec_ay, const std::vector< VecXx >& node_vec_az ) const
    {
    ...
    Scalar LiquidSimulator::dotNodeVectors( const std::vector< VecXx >& node_vec_ax, const std::vector< VecXx >& node_vec_ay, const std::vector< VecXx >& node_vec_az, const std::vector< VecXx >& node_vec_bx, const std::vector< VecXx >& node_vec_by, const std::vector< VecXx >& node_vec_bz ) const
    {
	...
	void LiquidSimulator::solveForceExplicit(const vector<VecXx>& lhs_x,
											 ...
											 const vector<VecXx>& lhs_z)
	{
        solveForceExplicit(lhs_x, lhs_y, lhs_z, m_node_rhs_fluid_x, m_node_rhs_fluid_y, m_node_rhs_fluid_z, m_node_vel_fluid_plus_x, m_node_vel_fluid_plus_y, m_node_vel_fluid_plus_z);
	}
	...
	bool LiquidSimulator::addForceFluids()
	{
        m_particle_buckets.for_each_bucket([&] (int bucket_idx) {
            ...
            m_node_rhs_fluid_x[bucket_idx] = VecXx(m_node_mass_fluid_x[bucket_idx].array() * m_node_vel_fluid_x[bucket_idx].array());
            m_node_rhs_fluid_y[bucket_idx] = VecXx(m_node_mass_fluid_y[bucket_idx].array() * m_node_vel_fluid_y[bucket_idx].array());
            m_node_rhs_fluid_z[bucket_idx] = VecXx(m_node_mass_fluid_z[bucket_idx].array() * m_node_vel_fluid_z[bucket_idx].array());
        });
        
        applyGridBasedBodyFriction(m_node_mass_fluid_x,
                                   m_node_mass_fluid_y,
                                   m_node_mass_fluid_z);
        
        // check_isnan("fluidvel_after_friction", m_node_vel_fluid_plus_x, m_node_vel_fluid_plus_y, m_node_vel_fluid_plus_z);
        ...
        // check_isnan("rhs_after_computeRHS", m_node_rhs_fluid_x, m_node_rhs_fluid_y, m_node_rhs_fluid_z);
        
		computeShearLHSDiagonal();
		
        // check_isnan("lhs_after_computeLHS", m_node_lhs_fluid_x, m_node_lhs_fluid_y, m_node_lhs_fluid_z);
    ...
    bool LiquidSimulator::saveVelocity()
    {
        return true;
    }
    ...
    {
        if(!m_liquid_info.solve_color_diffusion)
            return;
        
        const int num_fluid = numParticles();
        const Scalar dx = getDx();
        
        m_particle_cells_single.sort(num_fluid, [&] (int pidx, int& i, int& j, int& k) {
        ...
        m_particle_cells_single.for_each_bucket_particles([&] (int pidx, int bucket_idx) {
            Scalar sum_w = 0.0;
            const Vec3x& pos = m_x.segment<3>(pidx * 3);
            
            m_particle_cells_single.loop_neighbor_bucket_particles(bucket_idx, [&] (int npidx, int) {
        ...
        const Scalar epsilon2 = m_liquid_info.geometric_diffusivity * m_liquid_info.geometric_diffusivity;
        ...
        m_particle_cells_single.for_each_bucket_particles([&] (int pidx, int bucket_idx) {
            ...
            m_particle_cells_single.loop_neighbor_bucket_particles(bucket_idx, [&] (int npidx, int) {
                ...
                const Scalar coeff = m_m(npidx * 3) / std::max(1e-20, sph_density(npidx)) * gwr / (r2 + eta2);
                
                local_mu += (color - m_components.segment(npidx * m_liquid_info.num_components, m_liquid_info.num_components)) * coeff;
            ...
            for(int k = 0; k < m_liquid_info.num_components; ++k)
            {
                ...
                (color(k) - m_liquid_info.helmholtz_first_minima(k)) * 2. * inv_num_components;
            ...
            mu.segment(pidx * m_liquid_info.num_components, m_liquid_info.num_components) = local_mu;
        });
        ...
        m_particle_cells_single.for_each_bucket_particles([&] (int pidx, int bucket_idx) {
            ...
            const VecXx& mu_i = mu.segment(pidx * m_liquid_info.num_components, m_liquid_info.num_components);
            ...
            m_particle_cells_single.loop_neighbor_bucket_particles(bucket_idx, [&] (int npidx, int) {
                ...
                local_laplacian_mu += (mu_i - mu.segment(npidx * m_liquid_info.num_components, m_liquid_info.num_components)) * coeff;
            ...
            VecXx color = m_components.segment(pidx * m_liquid_info.num_components, m_liquid_info.num_components) + local_laplacian_mu * m_dt;
            ...
            m_components.segment(pidx * m_liquid_info.num_components, m_liquid_info.num_components) = color;
        });
...
//        std::cout << m_components << std::endl;
                                                ...
                                                std::vector< VecXx >& ret_z)
    {
        if(!m_liquid_info.solve_viscosity) {
            m_particle_buckets.for_each_bucket([&] (int bucket_idx) {
                ...
                ret_x[bucket_idx] = VecXx((m_node_mass_fluid_x[bucket_idx] + m_node_drag_coeff_x[bucket_idx] * m_dt + m_node_vel_constraint_coeff_x[bucket_idx]).array() * bucket_vec_x.array());
                ret_y[bucket_idx] = VecXx((m_node_mass_fluid_y[bucket_idx] + m_node_drag_coeff_y[bucket_idx] * m_dt + m_node_vel_constraint_coeff_y[bucket_idx]).array() * bucket_vec_y.array());
                ret_z[bucket_idx] = VecXx((m_node_mass_fluid_z[bucket_idx] + m_node_drag_coeff_z[bucket_idx] * m_dt + m_node_vel_constraint_coeff_z[bucket_idx]).array() * bucket_vec_z.array());
            });
        } else {
            const int num_parts = numParticles();
            ...
            auto flat_mul = [] (const Mat3x& a, const Mat3x& b) -> Scalar {
                return a.col(0).dot(b.col(0)) + a.col(1).dot(b.col(1)) + a.col(2).dot(b.col(2));
            };
            
            // Calculate A_p and DW for each particle
            for_each(0, num_parts, [&] (int pidx) {
                const VecXx& color = m_components.segment( m_liquid_info.num_components * pidx, m_liquid_info.num_components );
                ...
                if(mu > 1e-20) {
                    ...
                    auto add_to_B = [&] (const vector<VecXx>& vec, const Mat27x2i& indices, int ir) {
                        for(int i = 0; i < indices.rows(); ++i)
                        {
                            ...
                            B.col(ir) += (np - pos) * iD * weights(i, ir) * vec[node_bucket_idx][node_idx];
                        }
                    };
                    ...
                    // Elastic Part
                    Mat3x Ap = (B.transpose() * b_bar - 2.0 / 3.0 * B.trace() * devb_bar - 1.0 / 3.0 * (BTb_bar_tr * Mat3x::Identity() - b_bar_tr * B)) * m_proj_func(pidx * 3);
                    ...
                    if(s > 0.0) {
                        ...
                        // Plastic Part
                        Mat3x plastic_part = devb_bar * (dgds * ( mu * (devb_bar * B.transpose() * b_bar).trace() / devb_bar_n - 2.0 * s / 3.0 * B_tr) +
                    ...
                    m_Ap.block<3, 3>(pidx * 3, 0) = m_vol(pidx) * coeff * Ap * (1.0 + m_liquid_info.liquid_shear_damping / m_dt);
                } else {
            ...
            m_particle_buckets.for_each_bucket([&] (int bucket_idx) {
                ...
                auto add_Ap_to_ret = [&] (const vector< pair<int, short> >& node_particles, int i, int ir) -> Scalar {
                    Scalar ret = 0.0;
                    ...
                    for(auto& pair : node_particles) {
                        ...
                        ret += m_Ap.block<1, 3>(pidx * 3 + ir, 0).dot(np - pos) * m_particle_weights[pidx](pair.second, ir) * iD;
                    }
                    return ret;
                };
	...
	void LiquidSimulator::computeShearLHSDiagonal()
	{
		...
		m_particle_buckets.for_each_bucket([&] (int bucket_idx) {
            ...
            m_node_lhs_fluid_x[bucket_idx] = m_node_mass_fluid_x[bucket_idx] + m_node_drag_coeff_x[bucket_idx] * m_dt + m_node_vel_constraint_coeff_x[bucket_idx];
            m_node_lhs_fluid_y[bucket_idx] = m_node_mass_fluid_y[bucket_idx] + m_node_drag_coeff_y[bucket_idx] * m_dt + m_node_vel_constraint_coeff_y[bucket_idx];
            m_node_lhs_fluid_z[bucket_idx] = m_node_mass_fluid_z[bucket_idx] + m_node_drag_coeff_z[bucket_idx] * m_dt + m_node_vel_constraint_coeff_z[bucket_idx];
		});
        
        // check_isnan("lhs_diagonal_mass", m_node_mass_fluid_x, m_node_mass_fluid_y, m_node_mass_fluid_z);
        // check_isnan("lhs_diagonal_vel_constraint", m_node_vel_constraint_coeff_x, m_node_vel_constraint_coeff_y, m_node_vel_constraint_coeff_z);
        // check_isnan("lhs_diagonal_drag", m_node_drag_coeff_x, m_node_drag_coeff_y, m_node_drag_coeff_z);
        // check_isnan("lhs_diagonal_lhs", m_node_lhs_fluid_x, m_node_lhs_fluid_y, m_node_lhs_fluid_z);
	}
...
//        std::cout << "[compute weight 4]" << std::endl;
    ...
    template<typename T>
    void LiquidSimulator::allocateNodeVectors( std::vector< Eigen::Matrix<T, Eigen::Dynamic, 1> >& node_vec_x, std::vector< Eigen::Matrix<T, Eigen::Dynamic, 1> >& node_vec_y, std::vector< Eigen::Matrix<T, Eigen::Dynamic, 1> >& node_vec_z ) const
    {
    ...
    bool LiquidSimulator::doUseDrag()
    {
        return m_liquid_info.use_liquid_drag;
    }
    
	template<typename T>
	void LiquidSimulator::allocateNodeVectors( std::vector< Eigen::Matrix<T, Eigen::Dynamic, 1> >& node_vec_p, int num_subs, const T& val) const
	{
		const int num_buckets = m_particle_buckets.size();
		...
		m_particle_buckets.for_each_bucket([&] (int bucket_idx) {
            ...
            if(num_nodes > 0)
                node_vec_p[bucket_idx].setConstant(val);
		});
	}
    ...
    void LiquidSimulator::computeElasticLiquidDifference()
    {
        ...
        for_each(0, num_verts, [&] (int pidx) {
            ...
            Vec3x elastic_vel = m_strands[strand_idx]->dynamics().getDisplacement(local_idx) / m_dt;
		...
		m_particle_buckets.for_each_bucket([&] (int bucket_idx) {
			...
			for(int i = 0; i < num_nodes_p; ++i)
			{
				...
				if(m_node_liquid_phi[bucket_idx][i] < 0) {
					...
					if((bucket_idx_left >= 0 && node_idx_left >= 0 && m_bucket_activated[bucket_idx_left] && m_node_liquid_weight_x[bucket_idx_left][node_idx_left]) ||
					   (bucket_idx_right >= 0 && node_idx_right >= 0 && m_bucket_activated[bucket_idx_right] && m_node_liquid_weight_x[bucket_idx_right][node_idx_right]) ||
					   (bucket_idx_bottom >= 0 && node_idx_bottom >= 0 && m_bucket_activated[bucket_idx_bottom] && m_node_liquid_weight_y[bucket_idx_bottom][node_idx_bottom]) ||
					   (bucket_idx_top >= 0 && node_idx_top >= 0 && m_bucket_activated[bucket_idx_top] && m_node_liquid_weight_y[bucket_idx_top][node_idx_top]) ||
					   (bucket_idx_near >= 0 && node_idx_near >= 0 && m_bucket_activated[bucket_idx_near] && m_node_liquid_weight_z[bucket_idx_near][node_idx_near]) ||
					   (bucket_idx_far >= 0 && node_idx_far >= 0 && m_bucket_activated[bucket_idx_far] && m_node_liquid_weight_z[bucket_idx_far][node_idx_far]))
					{
		...
		m_particle_buckets.for_each_bucket([&] (int bucket_idx) {
			...
			for(int i = 0; i < num_nodes_p; ++i)
			{
				if(m_node_global_pressure_indices[bucket_idx][i] >= 0) {
					if(bucket_idx > 0) {
						m_node_global_pressure_indices[bucket_idx][i] += num_effective_nodes[bucket_idx - 1];
					}
...
//        std::cout << "[solvePressure: 3]" << std::endl;
		...
		for_each(0, total_num_nodes, [&] (int dof_idx) {
			...
			if(bucket_idx_left >= 0 && node_idx_left >= 0 && m_bucket_activated[bucket_idx_left]) {
				const Scalar& w = m_node_liquid_weight_x[bucket_idx_left][node_idx_left];
                
				w_center += w;
				
				m_pressure_rhs[dof_idx] += (m_node_vel_fluid_plus_x[bucket_idx_left][node_idx_left] * w ) / dx;
                if(w > 0.0) {
                    const Scalar inv_density = (m_node_mass_fluid_x[bucket_idx_left][node_idx_left] < 1e-20) ? 1.0 : (m_node_vol_fluid_x[bucket_idx_left][node_idx_left] / m_node_mass_fluid_x[bucket_idx_left][node_idx_left]);
                    
                    Scalar term = w * coeff * inv_density;
                     // check_isnan("pleft", term);
                    
					const int bucket_pressure_left = m_node_index_pressure_x[bucket_idx_left][node_idx_left * 4 + 0];
					const int node_pressure_left = m_node_index_pressure_x[bucket_idx_left][node_idx_left * 4 + 1];
					
					if(bucket_pressure_left >= 0 && node_pressure_left >= 0 && m_bucket_activated[bucket_pressure_left]) {
						const Scalar left_phi = m_node_liquid_phi[bucket_pressure_left][node_pressure_left];
						
						if(left_phi < 0.0) {
							const int dof_left = m_node_global_pressure_indices[bucket_pressure_left][node_pressure_left];
							assert(dof_left >= 0);
							...
							const Scalar theta = std::max(fraction_inside(center_phi, left_phi), m_liquid_info.theta_criterion);
			...
			if(bucket_idx_right >= 0 && node_idx_right >= 0 && m_bucket_activated[bucket_idx_right]) {
				const Scalar& w = m_node_liquid_weight_x[bucket_idx_right][node_idx_right];
                
				w_center += w;
				
				m_pressure_rhs[dof_idx] -= (m_node_vel_fluid_plus_x[bucket_idx_right][node_idx_right] * w) / dx;


				if(w > 0.0) {
                    const Scalar inv_density = (m_node_mass_fluid_x[bucket_idx_right][node_idx_right] < 1e-20) ? 1.0 : (m_node_vol_fluid_x[bucket_idx_right][node_idx_right] / m_node_mass_fluid_x[bucket_idx_right][node_idx_right]);
                    
                    Scalar term = w * coeff * inv_density;
                     // check_isnan("pright", term);
                    
					const int bucket_pressure_right = m_node_index_pressure_x[bucket_idx_right][node_idx_right * 4 + 2];
					const int node_pressure_right = m_node_index_pressure_x[bucket_idx_right][node_idx_right * 4 + 3];
					
					if(bucket_pressure_right >= 0 && node_pressure_right >= 0 && m_bucket_activated[bucket_pressure_right]) {
						const Scalar right_phi = m_node_liquid_phi[bucket_pressure_right][node_pressure_right];
						
						if(right_phi < 0.0) {
							const int dof_right = m_node_global_pressure_indices[bucket_pressure_right][node_pressure_right];
							assert(dof_right >= 0);
							...
							const Scalar theta = std::max(fraction_inside(center_phi, right_phi), m_liquid_info.theta_criterion);
			...
			if(bucket_idx_bottom >= 0 && node_idx_bottom >= 0 && m_bucket_activated[bucket_idx_bottom]) {
				const Scalar& w = m_node_liquid_weight_y[bucket_idx_bottom][node_idx_bottom];
                
				w_center += w;
				
				m_pressure_rhs[dof_idx] += (m_node_vel_fluid_plus_y[bucket_idx_bottom][node_idx_bottom] * w) / dx;
                
                if(w > 0.0) {
                    const Scalar inv_density = (m_node_mass_fluid_y[bucket_idx_bottom][node_idx_bottom] < 1e-20) ? 1.0 : (m_node_vol_fluid_y[bucket_idx_bottom][node_idx_bottom] / m_node_mass_fluid_y[bucket_idx_bottom][node_idx_bottom]);
                    
                    Scalar term = w * coeff * inv_density;
                     // check_isnan("pbottom", term);
                    
					const int bucket_pressure_bottom = m_node_index_pressure_y[bucket_idx_bottom][node_idx_bottom * 4 + 0];
					const int node_pressure_bottom = m_node_index_pressure_y[bucket_idx_bottom][node_idx_bottom * 4 + 1];
					
					if(bucket_pressure_bottom >= 0 && node_pressure_bottom >= 0 && m_bucket_activated[bucket_pressure_bottom]) {
						const Scalar bottom_phi = m_node_liquid_phi[bucket_pressure_bottom][node_pressure_bottom];
						
						if(bottom_phi < 0.0) {
							const int dof_bottom = m_node_global_pressure_indices[bucket_pressure_bottom][node_pressure_bottom];
							assert(dof_bottom >= 0);
							...
							const Scalar theta = std::max(fraction_inside(center_phi, bottom_phi), m_liquid_info.theta_criterion);
			...
			if(bucket_idx_top >= 0 && node_idx_top >= 0 && m_bucket_activated[bucket_idx_top]) {
				const Scalar& w = m_node_liquid_weight_y[bucket_idx_top][node_idx_top];
                
				w_center += w;
				
				m_pressure_rhs[dof_idx] -= (m_node_vel_fluid_plus_y[bucket_idx_top][node_idx_top] * w) / dx;

				if(w > 0.0) {
                    const Scalar inv_density = (m_node_mass_fluid_y[bucket_idx_top][node_idx_top] < 1e-20) ? 1.0 : (m_node_vol_fluid_y[bucket_idx_top][node_idx_top] / m_node_mass_fluid_y[bucket_idx_top][node_idx_top]);
                    
                    Scalar term = w * coeff * inv_density;
                     // check_isnan("ptop", term);
                    
					const int bucket_pressure_top = m_node_index_pressure_y[bucket_idx_top][node_idx_top * 4 + 2];
					const int node_pressure_top = m_node_index_pressure_y[bucket_idx_top][node_idx_top * 4 + 3];
					
					if(bucket_pressure_top >= 0 && node_pressure_top >= 0 && m_bucket_activated[bucket_pressure_top]) {
						const Scalar top_phi = m_node_liquid_phi[bucket_pressure_top][node_pressure_top];
						
						if(top_phi < 0.0) {
							const int dof_top = m_node_global_pressure_indices[bucket_pressure_top][node_pressure_top];
							assert(dof_top >= 0);
							...
							const Scalar theta = std::max(fraction_inside(center_phi, top_phi), m_liquid_info.theta_criterion);
			...
			if(bucket_idx_near >= 0 && node_idx_near >= 0 && m_bucket_activated[bucket_idx_near]) {
				const Scalar& w = m_node_liquid_weight_z[bucket_idx_near][node_idx_near];
                
				w_center += w;
				
				m_pressure_rhs[dof_idx] += (m_node_vel_fluid_plus_z[bucket_idx_near][node_idx_near] * w) / dx;

                
                if(w > 0.0) {
                    const Scalar inv_density = (m_node_mass_fluid_z[bucket_idx_near][node_idx_near] < 1e-20) ? 1.0 : (m_node_vol_fluid_z[bucket_idx_near][node_idx_near] / m_node_mass_fluid_z[bucket_idx_near][node_idx_near]);
                    
                    Scalar term = w * coeff * inv_density;
                     // check_isnan("pnear", term);
                    
					const int bucket_pressure_near = m_node_index_pressure_z[bucket_idx_near][node_idx_near * 4 + 0];
					const int node_pressure_near = m_node_index_pressure_z[bucket_idx_near][node_idx_near * 4 + 1];
					
					if(bucket_pressure_near >= 0 && node_pressure_near >= 0 && m_bucket_activated[bucket_pressure_near]) {
						const Scalar near_phi = m_node_liquid_phi[bucket_pressure_near][node_pressure_near];
						
						if(near_phi < 0.0) {
							const int dof_near = m_node_global_pressure_indices[bucket_pressure_near][node_pressure_near];
							assert(dof_near >= 0);
							...
							const Scalar theta = std::max(fraction_inside(center_phi, near_phi), m_liquid_info.theta_criterion);
			...
			if(bucket_idx_far >= 0 && node_idx_far >= 0 && m_bucket_activated[bucket_idx_far]) {
				const Scalar& w = m_node_liquid_weight_z[bucket_idx_far][node_idx_far];
                
				w_center += w;
				
				m_pressure_rhs[dof_idx] -= (m_node_vel_fluid_plus_z[bucket_idx_far][node_idx_far] * w) / dx;

				if(w > 0.0) {
                    const Scalar inv_density = (m_node_mass_fluid_z[bucket_idx_far][node_idx_far] < 1e-20) ? 1.0 : (m_node_vol_fluid_z[bucket_idx_far][node_idx_far] / m_node_mass_fluid_z[bucket_idx_far][node_idx_far]);
                    
                    Scalar term = w * coeff * inv_density;
                     // check_isnan("pfar", term);
                    
					const int bucket_pressure_far = m_node_index_pressure_z[bucket_idx_far][node_idx_far * 4 + 2];
					const int node_pressure_far = m_node_index_pressure_z[bucket_idx_far][node_idx_far * 4 + 3];
					
					if(bucket_pressure_far >= 0 && node_pressure_far >= 0 && m_bucket_activated[bucket_pressure_far]) {
						const Scalar far_phi = m_node_liquid_phi[bucket_pressure_far][node_pressure_far];
						
						if(far_phi < 0.0) {
							const int dof_far = m_node_global_pressure_indices[bucket_pressure_far][node_pressure_far];
							assert(dof_far >= 0);
							...
							const Scalar theta = std::max(fraction_inside(center_phi, far_phi), m_liquid_info.theta_criterion);
            ...
            if(center_J > 0.0 && w_center > 0.0) {
                const VecXx& color = m_node_components[bucket_idx].segment(node_idx * m_liquid_info.num_components, m_liquid_info.num_components);
            ...
            if(m_liquid_info.use_varying_volume_fraction && w_center > 0.0) {
                ...
                for(auto& pair : node_vertex_p) {
                    ...
                    const Scalar inv_V_c = 1.0 / ((1.0 - m_node_elastic_vf_p[bucket_idx][node_idx]) * dV);

                    // check_isnan("inv_V_c", inv_V_c);
                    // Note: This is the (negative) divergence at grid node! So we use (pos - np) instead of otherwise!
                    m_pressure_rhs[dof_idx] -= inv_V_c * weights(pair.second, 3) * iD * m_elastic_liquid_diffs.segment<3>(pidx * 3).dot(pos - np) * w_center;
                }
            }
		});
        ...
        bool success = AMGPCGSolveSparse(m_pressure_matrix, m_pressure_rhs, result, dof_ijk, 1e-8, 1000, tolerance, iterations, ni, nj, nk);
        if(!success)
            std::cout << "WARNING: Pressure solve failed!" << std::endl;
...
//        std::cout << "[solvePressure: 9]" << std::endl;
	...
	void LiquidSimulator::extrapolate( vector< VecXuc >& node_valid, vector< VecXx >& node_vel )
	{
		const Vec3i offsets[] = {
			Vec3i(-1, 0, 0),
			Vec3i(1, 0, 0),
		...
		for(int layers = 0; layers < 4; ++layers) {
			...
			m_particle_buckets.for_each_bucket([&] (int bucket_idx) {
				...
				{
					const int center_cell_idx = k * m_num_nodes * m_num_nodes + j * m_num_nodes + i;
					...
					for(int r = 0; r < 6; ++r) {
						...
						const int node_bucket_idx = m_particle_buckets.bucket_index(node_bucket_handle);
						if(!m_bucket_activated[node_bucket_idx]) continue;
						
						const int neigh_cell_idx = neigh_idx(2) * m_num_nodes * m_num_nodes + neigh_idx(1) * m_num_nodes + neigh_idx(0);
        ...
        m_particle_buckets.for_each_bucket([&] (int bucket_idx) {
            ...
            for(int i = 0; i < num_rhs_x; ++i) {
                ...
                if(bucket_weight_x(i) > 0.) {
                    const Vec4i& indices = m_node_index_pressure_x[bucket_idx].segment<4>(i * 4);
                    
                    Scalar left_phi(0), right_phi(0);
                    if(indices[0] >= 0 && indices[1] >= 0) left_phi = m_node_liquid_phi[indices[0]][indices[1]];
                    
                    if(indices[2] >= 0 && indices[3] >= 0) right_phi = m_node_liquid_phi[indices[2]][indices[3]];
                    
                    if(left_phi < 0.0 || right_phi < 0.0) {
                        Scalar theta = 1.;
                        if(right_phi >= 0.0 || left_phi >= 0.0)
                            theta = std::max(fraction_inside(left_phi, right_phi), m_liquid_info.theta_criterion);
                        
                        Scalar pressure_grad = 0;
                        if(indices[0] >= 0 && indices[1] >= 0) pressure_grad -= m_node_pressure[indices[0]][indices[1]];
                        
                        if(indices[2] >= 0 && indices[3] >= 0) pressure_grad += m_node_pressure[indices[2]][indices[3]];
            ...
            for(int i = 0; i < num_rhs_y; ++i) {
                ...
                if(bucket_weight_y(i) > 0.) {
                    const Vec4i& indices = m_node_index_pressure_y[bucket_idx].segment<4>(i * 4);
                    
                    Scalar bottom_phi(0), top_phi(0);
                    if(indices[0] >= 0 && indices[1] >= 0) bottom_phi = m_node_liquid_phi[indices[0]][indices[1]];
                    
                    if(indices[2] >= 0 && indices[3] >= 0) top_phi = m_node_liquid_phi[indices[2]][indices[3]];
                    
                    if(bottom_phi < 0.0 || top_phi < 0.0) {
                        Scalar theta = 1.;
                        if(top_phi >= 0.0 || bottom_phi >= 0.0)
                            theta = std::max(fraction_inside(bottom_phi, top_phi), m_liquid_info.theta_criterion);
            ...
            for(int i = 0; i < num_rhs_z; ++i) {
                ...
                if(bucket_weight_z(i) > 0.) {
                    const Vec4i& indices = m_node_index_pressure_z[bucket_idx].segment<4>(i * 4);
                    
                    Scalar near_phi(0), far_phi(0);
                    if(indices[0] >= 0 && indices[1] >= 0) near_phi = m_node_liquid_phi[indices[0]][indices[1]];
                    
                    if(indices[2] >= 0 && indices[3] >= 0) far_phi = m_node_liquid_phi[indices[2]][indices[3]];
                    
                    if(near_phi < 0.0 || far_phi < 0.0) {
                        Scalar theta = 1.;
                        if(far_phi >= 0.0 || near_phi >= 0.0)
                            theta = std::max(fraction_inside(near_phi, far_phi), m_liquid_info.theta_criterion);
	...
	bool LiquidSimulator::solveViscosity()
	{
        ...
        solveForceExplicit(m_node_lhs_fluid_x,
                           m_node_lhs_fluid_y,
                           m_node_lhs_fluid_z);
        ...
        // check_isnan("fluidvel_after_addforce", m_node_vel_fluid_plus_x, m_node_vel_fluid_plus_y, m_node_vel_fluid_plus_z);
        
        Scalar res_norm_0 = lengthNodeVectors(m_node_rhs_fluid_x, m_node_rhs_fluid_y, m_node_rhs_fluid_z);
        ...
        if(res_norm_0 > m_liquid_info.shear_pcg_criterion) {
            ...
            multiplyHessianMatrix(m_node_vel_fluid_plus_x, m_node_vel_fluid_plus_y, m_node_vel_fluid_plus_z, m_node_r_x, m_node_r_y, m_node_r_z);
            
            m_particle_buckets.for_each_bucket([&] (int bucket_idx) {
                if(!m_bucket_activated[bucket_idx]) return;
                
                m_node_r_x[bucket_idx] = m_node_rhs_fluid_x[bucket_idx] - m_node_r_x[bucket_idx];
                m_node_r_y[bucket_idx] = m_node_rhs_fluid_y[bucket_idx] - m_node_r_y[bucket_idx];
                m_node_r_z[bucket_idx] = m_node_rhs_fluid_z[bucket_idx] - m_node_r_z[bucket_idx];
            });
            ...
            if(res_norm < m_liquid_info.shear_pcg_criterion) {
                ...
                << ", res: " << res_norm << "/" << m_liquid_info.shear_pcg_criterion
                << ", abs. res: " << (res_norm * res_norm_0) << "/" << (m_liquid_info.shear_pcg_criterion * res_norm_0)
                << "]" << std::endl;
            } else {
                solveForceExplicit(m_node_lhs_fluid_x, m_node_lhs_fluid_y, m_node_lhs_fluid_z,
                ...
                multiplyHessianMatrix(m_node_p_x, m_node_p_y, m_node_p_z, m_node_q_x, m_node_q_y, m_node_q_z);
                ...
                const Scalar rho_criterion = (m_liquid_info.shear_pcg_criterion * res_norm_0) * (m_liquid_info.shear_pcg_criterion * res_norm_0);
                
                Scalar rho_old, beta;
                for(; iter < m_liquid_info.pcg_max_iters && res_norm > m_liquid_info.shear_pcg_criterion && rho > rho_criterion; ++iter)
                {
                    rho_old = rho;
                    ...
                    if(m_liquid_info.iteration_print_step > 0 && iter % m_liquid_info.iteration_print_step == 0)
                        std::cout << "[pcg total iter: " << iter
                        << ", res: " << res_norm << "/" << m_liquid_info.shear_pcg_criterion
                        << ", abs. res: " << (res_norm * res_norm_0) << "/" << (m_liquid_info.shear_pcg_criterion * res_norm_0)
	...
	bool LiquidSimulator::solveDrag()
	{
		return true;
	}
...
//        
        // check_isnan("pressure_grad_before_constrain", m_node_pressure_grad_x, m_node_pressure_grad_y, m_node_pressure_grad_z);
...
//
        // check_isnan("pressure_grad_after_extrapolate", m_node_pressure_grad_x, m_node_pressure_grad_y, m_node_pressure_grad_z);
        // check_isnan("vel_fluid_before_constrain", m_node_vel_fluid_plus_x, m_node_vel_fluid_plus_y, m_node_vel_fluid_plus_z);
        ...
        // check_isnan("vel_fluid_after_extrapolate", m_node_vel_fluid_plus_x, m_node_vel_fluid_plus_y, m_node_vel_fluid_plus_z);
        ...
        // check_isnan("vel_fluid_after_constrain", m_node_vel_fluid_plus_x, m_node_vel_fluid_plus_y, m_node_vel_fluid_plus_z);
        
		return true;
	}
	Vec3x LiquidSimulator::getOrigin() const
	{
		return m_grid_mincorner;
	}
    ...
    Scalar LiquidSimulator::getRadius(int i) const
    {
        return m_radius(i);
    }
	...
	bool LiquidSimulator::loadParticles()
	{
		return true;
	}
	int LiquidSimulator::numParticles() const
	{
		return m_x.size() / 3;
	}
    Vec3x LiquidSimulator::get_future_velocity(const Vec3x& position) const
    {
        const Scalar dx = getDx();
        Vec3x v;
        v(0) = interpolateValue(position, m_node_vel_fluid_plus_x, m_grid_mincorner + Vec3x(0.0, 0.5, 0.5) * dx, 0.0);
        v(1) = interpolateValue(position, m_node_vel_fluid_plus_y, m_grid_mincorner + Vec3x(0.5, 0.0, 0.5) * dx, 0.0);
        v(2) = interpolateValue(position, m_node_vel_fluid_plus_z, m_grid_mincorner + Vec3x(0.5, 0.5, 0.0) * dx, 0.0);
    ...
    Scalar LiquidSimulator::get_mass(const Vec3x& position) const
    {
        const Scalar dx = getDx();
        Vec3x v;
        v(0) = interpolateValue(position, m_node_mass_fluid_x, m_grid_mincorner + Vec3x(0.0, 0.5, 0.5) * dx, 0.0);
        v(1) = interpolateValue(position, m_node_mass_fluid_y, m_grid_mincorner + Vec3x(0.5, 0.0, 0.5) * dx, 0.0);
        v(2) = interpolateValue(position, m_node_mass_fluid_z, m_grid_mincorner + Vec3x(0.5, 0.5, 0.0) * dx, 0.0);
    ...
    Scalar LiquidSimulator::get_volume(const Vec3x& position) const
    {
        const Scalar dx = getDx();
        Vec3x v;
        v(0) = interpolateValue(position, m_node_vol_fluid_x, m_grid_mincorner + Vec3x(0.0, 0.5, 0.5) * dx, 0.0);
        v(1) = interpolateValue(position, m_node_vol_fluid_y, m_grid_mincorner + Vec3x(0.5, 0.0, 0.5) * dx, 0.0);
        v(2) = interpolateValue(position, m_node_vol_fluid_z, m_grid_mincorner + Vec3x(0.5, 0.5, 0.0) * dx, 0.0);
	...
	Vec3x LiquidSimulator::get_velocity(const Vec3x& position) const
	{
		const Scalar dx = getDx();
		Vec3x v;
		v(0) = interpolateValue(position, m_node_vel_fluid_x, m_grid_mincorner + Vec3x(0.0, 0.5, 0.5) * dx, 0.0);
		v(1) = interpolateValue(position, m_node_vel_fluid_y, m_grid_mincorner + Vec3x(0.5, 0.0, 0.5) * dx, 0.0);
		v(2) = interpolateValue(position, m_node_vel_fluid_z, m_grid_mincorner + Vec3x(0.5, 0.5, 0.0) * dx, 0.0);
	...
	Vec3x LiquidSimulator::get_solid_velocity(const Vec3x& position) const
	{
		const Scalar dx = getDx();
		Vec3x v;
		v(0) = interpolateValue(position, m_node_solid_vel_x, m_grid_mincorner + Vec3x(0.0, 0.5, 0.5) * dx, 0.0);
		v(1) = interpolateValue(position, m_node_solid_vel_y, m_grid_mincorner + Vec3x(0.5, 0.0, 0.5) * dx, 0.0);
		v(2) = interpolateValue(position, m_node_solid_vel_z, m_grid_mincorner + Vec3x(0.5, 0.5, 0.0) * dx, 0.0);
	...
	{
		const Scalar dx = getDx();
		return interpolateValue(position, m_node_liquid_phi, m_grid_mincorner + Vec3x(0.5, 0.5, 0.5) * dx, 3.0 * dx);
	}
	
    void LiquidSimulator::storeDragCoeff(int strand_idx, int local_idx, const Scalar& coeff)
    {
    ...
    Vec3x LiquidSimulator::get_velocity(int strand_idx, int local_idx) const
    {
        ...
        for(int i = 0; i < indices_x.rows(); ++i)
        {
            const int node_bucket_idx = indices_x(i, 0);
            const int node_idx = indices_x(i, 1);
            if(node_bucket_idx == -1 || node_idx == -1 || !m_bucket_activated[node_bucket_idx] || weights(i, 0) == 0.0) continue;
        ...
        for(int i = 0; i < indices_y.rows(); ++i)
        {
            const int node_bucket_idx = indices_y(i, 0);
            const int node_idx = indices_y(i, 1);
            if(node_bucket_idx == -1 || node_idx == -1 || !m_bucket_activated[node_bucket_idx] || weights(i, 1) == 0.0) continue;
        ...
        for(int i = 0; i < indices_z.rows(); ++i)
        {
            const int node_bucket_idx = indices_z(i, 0);
            const int node_idx = indices_z(i, 1);
            if(node_bucket_idx == -1 || node_idx == -1 || !m_bucket_activated[node_bucket_idx] || weights(i, 2) == 0.0) continue;
	...
	VecXx LiquidSimulator::interpolateValue(const Vec3x& pos, const std::vector< VecXx >& phi, const Vec3x& phi_ori, const VecXx& default_val, int subcells, int N) const
	{
		const Scalar dx = getDx();
		Vec3x grid_pos = pos - phi_ori;
		Vec3x base_pos = grid_pos / dx * (Scalar) subcells;
		Vec3i base_idx = Vec3i((int) floor(base_pos(0)),
		...
		for(int t = 0; t < 2; ++t) for(int s = 0; s < 2; ++s) for(int r = 0; r < 2; ++r) {
			...
			Vec3i bucket_handle = Vec3i(query_idx(0) / num_sub_nodes,
										query_idx(1) / num_sub_nodes,
			...
			Vec3i node_handle = Vec3i(query_idx(0) - bucket_handle(0) * num_sub_nodes,
									  query_idx(1) - bucket_handle(1) * num_sub_nodes,
									  query_idx(2) - bucket_handle(2) * num_sub_nodes);
			
			const int node_idx = node_handle(2) * num_sub_nodes * num_sub_nodes + node_handle(1) * num_sub_nodes + node_handle(0);
		...
		return trilerp(buf[0], buf[1], buf[2], buf[3], buf[4], buf[5], buf[6], buf[7], frac[0], frac[1], frac[2]);
	}
	
    template< int N >
    Eigen::Matrix<Scalar, N, 1> LiquidSimulator::interpolateValue(const Vec3x& pos, const std::vector< VecXx >& phi, const Vec3x& phi_ori, const Eigen::Matrix<Scalar, N, 1>& default_val, int subcells) const
    {
        const Scalar dx = getDx();
        Vec3x grid_pos = pos - phi_ori;
        Vec3x base_pos = grid_pos / dx * (Scalar) subcells;
	...
	Scalar LiquidSimulator::interpolateValue(const Vec3x& pos, const std::vector< VecXx >& phi, const Vec3x& phi_ori, const Scalar& default_val, int subcells) const
	{
		const Scalar dx = getDx();
		Vec3x grid_pos = pos - phi_ori;
		Vec3x base_pos = grid_pos / dx * (Scalar) subcells;
    ...
    Vec3x LiquidSimulator::interpolateGradient(const Vec3x& pos, const std::vector< VecXx >& phi, const Vec3x& phi_ori, const Scalar& default_val) const
    {
        const Scalar dx = getDx();
        Vec3x grid_pos = pos - phi_ori;
        ...
        for(int t = 0; t < 2; ++t) for(int s = 0; s < 2; ++s) for(int r = 0; r < 2; ++r) {
            int local_idx = t * 4 + s * 2 + r;
            Vec3i query_idx = base_idx + Vec3i(r, s, t);
            Vec3i bucket_handle = Vec3i(query_idx(0) / m_num_nodes,
            ...
            Vec3i node_handle = Vec3i(query_idx(0) - bucket_handle(0) * m_num_nodes,
    ...
    Scalar LiquidSimulator::interpolateValueAndGradient(Vec3x& n, const Vec3x& pos, const std::vector< VecXx >& phi, const Vec3x& phi_ori, const Scalar& default_val) const
	{
		const Scalar dx = getDx();
		Vec3x grid_pos = pos - phi_ori;
		Vec3x base_pos = grid_pos / dx;
    ...
    void LiquidSimulator::makeParticlesComponentConsistent()
    {
        ...
        for_each(0, num_fluid, [&] (int pidx) {
            const Scalar sum = m_components.segment(pidx * m_liquid_info.num_components, m_liquid_info.num_components).sum();
            if(sum == 0.0) {
                m_components.segment(pidx * m_liquid_info.num_components, m_liquid_info.num_components).setZero();
                m_components(pidx * m_liquid_info.num_components) = 1.0;
            } else {
                m_components.segment(pidx * m_liquid_info.num_components, m_liquid_info.num_components) /= sum;
            }
        });
    }
    ...
    void LiquidSimulator::absorbLiquidFromMesh()
    {
        ...
        m_mesh_vertex_buckets.for_each_bucket_colored([&] (int bucket_idx) {
            ...
            m_mesh_vertex_buckets.get_bucket(bucket_idx, [&] (int pidx) {
                ...
                if(!m_fields[mesh_idx].has_surf_flow)
                    return;
                
                std::shared_ptr<TriangularMeshFlow> flow = m_fields[mesh_idx].mesh_controller->getMeshFlow();
                
                if(!flow) return;
                
                std::shared_ptr<TriangularMesh> mesh = m_fields[mesh_idx].mesh_controller->getCurrentMesh();
                ...
                Vec3i ilocal_x = Vec3i( clamp( (int) local_x(0), 0, m_num_nodes ), clamp( (int) local_x(1), 0, m_num_nodes ), clamp( (int) local_x(2), 0, m_num_nodes ) );
                const int node_idx = ilocal_x(2) * m_num_nodes * m_num_nodes + ilocal_x(1) * m_num_nodes + ilocal_x(0);
            ...
            if(!has_at_least_one_cell_available)
                return;
            
            Vec3i bucket_handle = m_mesh_vertex_buckets.bucket_handle(bucket_idx);
            Vec3i cell_handle_base = Vec3i( bucket_handle(0) * m_num_nodes, bucket_handle(1) * m_num_nodes, bucket_handle(2) * m_num_nodes );
            
            VecXx old_reservoir = reservoir;
            // for each reservoir cell, firstly try put liquid onto particles
            for(int k = 0; k < m_num_nodes; ++k) for(int j = 0; j < m_num_nodes; ++j) for(int i = 0; i < m_num_nodes; ++i)
            {
                int local_idx = k * m_num_nodes * m_num_nodes + j * m_num_nodes + i;
                if(reservoir(local_idx) == 0.0) continue;
                
                Vec3i cell_handle = cell_handle_base + Vec3i( i, j, k );
                if(!m_particle_cells_single.has_bucket(cell_handle))
                    continue;
                
                int cell_idx = m_particle_cells_single.bucket_index(cell_handle);
                int num_parts = m_particle_cells_single.get_neighbor_bucket_particles_size(cell_idx);
                if(!num_parts)
                    continue;
                
                Scalar vol_each_p = reservoir(local_idx) / (Scalar) num_parts;
                
                m_particle_cells_single.loop_neighbor_bucket_particles(cell_idx, [&] (int pidx, int) {
                    m_vol(pidx) *= (m_rest_vol(pidx) + vol_each_p) / m_rest_vol(pidx);
    ...
    void LiquidSimulator::transferBulkLiquidMesh()
    {
        ...
        m_mesh_vertex_buckets.sort(m_mesh_global_local.size(), [&] (int pidx, int& i, int& j, int& k) {
            const pair<int, int>& local_idx = m_mesh_global_local[pidx];
            const std::shared_ptr<TriangularMesh> mesh = m_fields[local_idx.first].mesh_controller->getCurrentMesh();
        ...
        if(m_liquid_info.use_liquid_capture && num_fluid)
            captureBulkLiquidMesh();
...
//            absorbLiquidFromMesh();
    ...
    void LiquidSimulator::captureBulkLiquidMesh()
    {
        ...
        for_each(0, num_fields, [&] (int field_idx) {
            std::shared_ptr<TriangularMeshFlow> flow = m_fields[field_idx].mesh_controller->getMeshFlow();
            if(!flow) return;
        ...
        m_mesh_vertex_buckets.for_each_bucket_particles_colored([&] (int pidx, int) {
            ...
            if(!flow || !flow->isVertexPassing(local_idx))
                return;
            ...
            if(ipos(0) < 0 || ipos(0) >= m_particle_cells_single.ni ||
               ipos(1) < 0 || ipos(1) >= m_particle_cells_single.nj ||
               ipos(2) < 0 || ipos(2) >= m_particle_cells_single.nk)
            ...
            VecXx color = flow->getCurrentComponents().segment( local_idx * m_liquid_info.num_components, m_liquid_info.num_components ) * height * area;
            
            m_particle_cells_single.loop_neighbor_bucket_particles( cell_idx, [&] (int npidx, int) {
                const Vec3x& np = m_x.segment<3>(npidx * 3);
                Vec3x diff = (pos - np) / dx;
                const Scalar w = N_kernel<2>(diff);
                Scalar vol_capture = std::min(m_rest_vol(npidx) * w, vol_remain_to_cap);
                ...
                color += m_components.segment(npidx * m_liquid_info.num_components, m_liquid_info.num_components) * vol_capture;
            ...
            for(int fidx : faces)
            {
                ...
                new_vel_edge = (Mat3x::Identity() - normal * normal.transpose()) * new_vel_edge;
    ...
    {
        // for each vertex check if its height has reached critial threshold, if so add the extra volume into volume that can be released
        // principle: try absorb extra liquid as much as possible with current particles
        const Scalar dx = getDx();
        ...
        const Scalar rad = defaultRadiusMultiplier() * dx * m_liquid_info.particle_cell_multiplier;
        ...
        vector< vector< Sample > > bucket_samples(m_mesh_vertex_buckets.ni * m_mesh_vertex_buckets.nj * m_mesh_vertex_buckets.nk);
        
//        std::cout << getTotalVolMeshFlows() << " + " << getTotalVolParticles() << std::endl;
        ...
        m_mesh_vertex_buckets.for_each_bucket_colored([&] (int bucket_idx) {
            ...
            m_mesh_vertex_buckets.get_bucket(bucket_idx, [&] (int pidx) {
                ...
                if(height <= m_liquid_info.mesh_flow_critical_height)
                    return;
                
                std::shared_ptr<TriangularMesh> mesh = m_fields[mesh_idx].mesh_controller->getCurrentMesh();
                Scalar area = mesh->getVertexArea(local_idx);
                
                Scalar extra_vol = (height - m_liquid_info.mesh_flow_critical_height) * area;
                ...
                average_pos.segment<3>( node_idx * 3 ) += mesh->getVertex(local_idx) * extra_vol;
                average_vel.segment<3>( node_idx * 3 ) += flow->getVertexVelocity(local_idx) * extra_vol;
                average_color.segment( node_idx * m_liquid_info.num_components, m_liquid_info.num_components ) += flow->getCurrentComponents().segment( local_idx * m_liquid_info.num_components, m_liquid_info.num_components ) * extra_vol;
            });
            ...
            {
                int local_idx = k * m_num_nodes * m_num_nodes + j * m_num_nodes + i;
                if(reservoir(local_idx) == 0.0 || reservoir(local_idx) < pvol ) continue;
                ...
                const Scalar radius_released_vol = pow(actual_released_vol * 0.75 / M_PI, 1.0 / 3.0) * 2.0;
                ...
                for(int i = 0; i < num_releases; ++i) {
                    Sample s;
                    Vec3x actual_pos = pos + Q * m_sphere_pattern[num_releases].segment<3>(i * 3) * radius_released_vol;
            ...
            m_mesh_vertex_buckets.get_bucket(bucket_idx, [&] (int pidx) {
                ...
                height = m_liquid_info.mesh_flow_critical_height + (height - m_liquid_info.mesh_flow_critical_height) * reservoir(node_idx) / old_reservoir(node_idx);
                flow->setVertexHeight(local_idx, height);
            });
        }, 3);
        ...
        
        {
            ...
            for_each(0, num_new_samples, [&] (int i) {
                ...
                m_m.segment<3>(part_idx * 3).setConstant(pvol * getDensity(new_samples[i].c));
                ...
                m_components.segment(part_idx * m_liquid_info.num_components, m_liquid_info.num_components) = new_samples[i].c;
            });
        }
        makeParticlesConsistent();
...
//        std::cout << getTotalVolMeshFlows() << " + " << getTotalVolParticles() << std::endl;
    ...
    {
        return Vec3i( m_particle_buckets.ni * m_num_nodes,  m_particle_buckets.nj * m_num_nodes,  m_particle_buckets.nk * m_num_nodes );
    }
    
    void LiquidSimulator::captureBulkLiquid()
    {
        if(!m_liquid_info.use_liquid_capture)
            return;
        ...
        m_vertex_buckets.sort(m_strands_global_local.size(), [&] (int pidx, int& i, int& j, int& k) {
        ...
        m_vertex_buckets.for_each_bucket_particles_colored([&] (int pidx, int) {
            ...
            // determain volume to be transferred, ignore the ones that has more flow than we should capture
            ...
            Vec3x momentum = Vec3x::Zero();
            VecXx color = m_strands[strand_idx]->getStepper()->flowComponents().segment(local_idx * m_liquid_info.num_components, m_liquid_info.num_components) * area * len;
            
            m_particle_cells_single.loop_neighbor_bucket_particles( cell_idx, [&] (int npidx, int ncidx) {
                const Vec3x& np = m_x.segment<3>(npidx * 3);
                Vec3x diff = (pos - np) / dx;
                const Scalar w = N_kernel<2>(diff);
                Scalar vol_capture = std::min(m_rest_vol(npidx) * w, vol_remain_to_cap);
            ...
            m_strands[strand_idx]->getStepper()->flowComponents().segment( local_idx * m_liquid_info.num_components, m_liquid_info.num_components ) = color;
	...
	void LiquidSimulator::transferLiquidFlowGrid()
	{
		...
		// for all grid node find inside vertices and register their extra flow volume onto grid
		m_vertex_buckets.for_each_bucket_particles_colored([&] (int pidx, int) {
			const int strand_idx = m_strands_global_local[pidx].first;
			const int local_idx = m_strands_global_local[pidx].second;
            ...
            const VecXx& c = m_strands[strand_idx]->getStepper()->flowComponents().segment( local_idx * m_liquid_info.num_components, m_liquid_info.num_components );
			...
			m_particle_cells_single.loop_neighbor_bucket_particles( cell_idx, [&] (int npidx, int ncidx) {
                ...
                m_v.segment<3>(npidx * 3) = (mom_share + mom_ori) / std::max(1e-20, m_rest_vol(npidx) + vol_share);
                
                const VecXx color_ori = m_components.segment(npidx * m_liquid_info.num_components, m_liquid_info.num_components) * m_rest_vol(npidx);
                VecXx color_new = color_ori + color_share;
                make_gibbs_simplex(color_new);
                
                m_components.segment(npidx * m_liquid_info.num_components, m_liquid_info.num_components) = color_new;
													 ...
													 const vector<VecXx>& lhs_z)
	{
		...
		m_particle_buckets.for_each_bucket([&] (int bucket_idx) {
			...
			auto project_vel = [&] (vector<VecXx>& node_rhs_fluid, const vector<VecXx>& node_solid_vel, const vector<VecXx>& node_mass_fluid, const int ir) {
				for(int i = 0; i < num_nodes; ++i) {
					if(node_mass_fluid[bucket_idx][i] < 1e-20) continue;
					
					const Vec3x np = getNodePos(bucket_idx, i, ir);
					Vec3x n = Vec3x::Zero();
					const Scalar phi = interpolateValueAndGradient(n, np, m_node_solid_phi, m_grid_mincorner, 3.0 * dx);
                    ...
                    Vec3x proj_vel = clamp(1. - m_liquid_info.liquid_boundary_friction, 0.0, 1.0) * tvel + nvel + svel;
					
                    // No Slip! Damn yeah no slip!
                    node_rhs_fluid[bucket_idx][i] = proj_vel[ir] * node_mass_fluid[bucket_idx][i];
				}
			};
...
//        m_particle_grad_z.resize(num_particles);
	...
	void LiquidSimulator::dripFromReservoir()
	{
		...
		for(int i = 0; i < num_end_verts; ++i)
		{
			...
			bbx_min = Vec3x(std::min(p(0), bbx_min(0)), std::min(p(1), bbx_min(1)), std::min(p(2), bbx_min(2)));
			bbx_max = Vec3x(std::max(p(0), bbx_max(0)), std::max(p(1), bbx_max(1)), std::max(p(2), bbx_max(2)));
		}
		
		bbx_min = Vec3x(floor(bbx_min(0) / dx) * dx, floor(bbx_min(1) / dx) * dx, floor(bbx_min(2) / dx) * dx);
		bbx_max = Vec3x(ceil(bbx_max(0) / dx) * dx, ceil(bbx_max(1) / dx) * dx, ceil(bbx_max(2) / dx) * dx);
        ...
        const int clampedInvCellMultiplier = (int) ceil(1.0 / m_liquid_info.particle_cell_multiplier);
        
		Sorter sorter_existed(ni * clampedInvCellMultiplier, nj * clampedInvCellMultiplier, nk * clampedInvCellMultiplier);
		...
		sorter.for_each_bucket_colored([&] (int cell_idx) {
			Scalar vol = 0.0;
			Vec3x vel = Vec3x::Zero();
			...
			sorter.get_bucket(cell_idx, [&] (int pidx) {
				int strand_idx = pidx / 2;
				int tip_idx = (m_strands[strand_idx]->getNumVertices() - 1) * (pidx & 1);
				int tip_edge_idx = (m_strands[strand_idx]->getNumEdges() - 1) * (pidx & 1);
                ...
                color += m_strands[strand_idx]->getStepper()->flowComponents().segment( tip_idx * m_liquid_info.num_components, m_liquid_info.num_components ) * w;
			});
			
			if(vol < pvol) return;
			
			int num_release = std::min((int) floor(vol / pvol), (int) sphere_pattern::max_vector_length);
			...
			Vec3i bucket_handle = Vec3i(cell_handle(0) / m_num_nodes, cell_handle(1) / m_num_nodes, cell_handle(2) / m_num_nodes);
			const int bucket_idx = bucket_handle(2) * sni * snj + bucket_handle(1) * sni + bucket_handle(0);
			
			const Scalar radius_released_vol = pow(actual_released_vol * 0.75 / M_PI, 1.0 / 3.0);
			...
			for(int i = 0; i < num_release; ++i) {
				Sample s;
				Vec3x actual_pos = pos + Q * m_sphere_pattern[num_release].segment<3>(i * 3) * radius_released_vol;
				
				// check if pos available (not blocked by existing particles)
				Vec3i ipos_existed = Vec3i((int)std::floor((actual_pos(0) - bbx_min(0)) / half_dx),
				...
				if(sorter_existed.has_bucket(ipos_existed)) {
					const int cell_idx_existed = sorter_existed.bucket_index(ipos_existed);
	...
	void LiquidSimulator::sampleLiquidDistanceFields( Scalar cur_time )
	{
		...
		for(int igroup = 0; igroup < num_group; ++igroup)
		{
			...
			if(m_sources[igroup].usage != DFU_SOURCE ||
			   !m_sources[igroup].check_durations(cur_time, m_shooting_vol_accum[igroup], shooting_vel)) continue;
			
			VecXx additional_pos;
			
			m_sources[igroup].resample_internal(m_fields, dx * m_liquid_info.particle_cell_multiplier, m_x, additional_pos, (int) ceil((Scalar) m_num_nodes / m_liquid_info.particle_cell_multiplier));
			...
			for_each(0, df_size, [&] (int i) {
				...
				m_J(part_idx) = 1.0;
				m_weakened[part_idx] = false;
                m_components.segment( part_idx * m_liquid_info.num_components, m_liquid_info.num_components ) = color;
			});
		}
		...
		if(numParts == 0 && !m_strands.size()) {
			...
			bbmin = reduction((Vec3x*) m_x.data(), numParticles(), bbmin,
							  [] (Vec3x x, Vec3x y)->Vec3x {
								  return Vec3x(std::min(x(0), y(0)), std::min(x(1), y(1)), std::min(x(2), y(2)));
							  });
			
			bbmax = reduction((Vec3x*) m_x.data(), numParticles(), bbmax,
							  [] (Vec3x x, Vec3x y)->Vec3x {
								  return Vec3x(std::max(x(0), y(0)), std::max(x(1), y(1)), std::max(x(2), y(2)));
							  });
            ...
            m_bbx_min = Vec3x(floor(bbmin(0) / dx) * dx, floor(bbmin(1) / dx) * dx, floor(bbmin(2) / dx) * dx);
            m_bbx_max = Vec3x(ceil(bbmax(0) / dx) * dx, ceil(bbmax(1) / dx) * dx, ceil(bbmax(2) / dx) * dx);
		}
        
        std::cout << "[bounding box min: " << m_bbx_min(0) << ", " << m_bbx_min(1) << ", " << m_bbx_min(2) << "; max: " << m_bbx_max(0) << ", " << m_bbx_max(1) << ", " << m_bbx_max(2) << "]" << std::endl;
	}
	...
	void LiquidSimulator::rebucketizeParticles()
	{
		...
		Vec3x content_size = m_bbx_max - m_bbx_min + Vec3x::Constant(m_bucket_size * extra_border * 2.0);
		
		Vec3i grid_num_cells = Vec3i(std::max(1, (int) std::ceil(content_size(0) / dx)),
									 std::max(1, (int) std::ceil(content_size(1) / dx)),
									 std::max(1, (int) std::ceil(content_size(2) / dx)));
		
		Vec3x grid_size = Vec3x( (Scalar) grid_num_cells[0] * dx,
								(Scalar) grid_num_cells[1] * dx,
								(Scalar) grid_num_cells[2] * dx );
		
		m_grid_mincorner = m_bbx_min - Vec3x::Constant(m_bucket_size * extra_border);
		
		Vec3i num_buckets = Vec3i(std::max(1, (int) std::ceil(grid_size(0) / m_bucket_size)),
		...
		m_particle_cells_single.resize(grid_num_cells(0), grid_num_cells(1), grid_num_cells(2));
		
		m_particle_buckets.sort(numParticles(), [&] (int pidx, int& i, int& j, int& k) {
			i = (int)std::floor((m_x(pidx * 3 + 0) - m_grid_mincorner(0)) / m_bucket_size);
			j = (int)std::floor((m_x(pidx * 3 + 1) - m_grid_mincorner(1)) / m_bucket_size);
			k = (int)std::floor((m_x(pidx * 3 + 2) - m_grid_mincorner(2)) / m_bucket_size);
		});
        ...
        std::cout << "[num buckets: " << num_buckets(0) << ", " << num_buckets(1) << ", " << num_buckets(2) << "]" << std::endl;
    }
    ...
    int LiquidSimulator::getDefaultNumNodes() const
    {
        return m_num_nodes;
    }
	
    Scalar LiquidSimulator::get_dense_liquid_signed_distance(const Vec3x& pos) const
    {
        return interpolateValue(pos, m_node_signed_distance, m_grid_mincorner, 0.0, m_liquid_info.signed_distance_multiplier);
    }
	...
	bool LiquidSimulator::computeSignedDistance()
	{
		...
		const int dni = m_particle_buckets.ni * m_num_nodes * m_liquid_info.signed_distance_multiplier;
		const int dnj = m_particle_buckets.nj * m_num_nodes * m_liquid_info.signed_distance_multiplier;
		const int dnk = m_particle_buckets.nk * m_num_nodes * m_liquid_info.signed_distance_multiplier;
		
 //       std::cout << "[computeSignedDistance 0]" << std::endl;
        
        std::cout << "[csd num nodes: " << dni << ", " << dnj << ", " << dnk << "]" << std::endl;
		...
		allocateNodeVectors(m_node_signed_distance, m_liquid_info.signed_distance_multiplier * m_liquid_info.signed_distance_multiplier * m_liquid_info.signed_distance_multiplier, std::numeric_limits<Scalar>::max());
		
 //       std::cout << "[computeSignedDistance 1]" << std::endl;
        
		const int num_nodes_dist = m_num_nodes * m_liquid_info.signed_distance_multiplier;
		
		m_particle_buckets.for_each_bucket([&] (int bucket_idx) {
			if(!m_bucket_activated[bucket_idx]) return;
			
			const Vec3x base_np = getNodePos(bucket_idx, 0, 4);
			const Vec3i base_handle = m_particle_buckets.bucket_handle(bucket_idx) * num_nodes_dist;
			
			VecXx& bucket_signed_distance = m_node_signed_distance[bucket_idx];
			
			for(int k = 0; k < num_nodes_dist; ++k) for(int j = 0; j < num_nodes_dist; ++j) for(int i = 0; i < num_nodes_dist; ++i)
			{
				const int local_node_idx = k * num_nodes_dist * num_nodes_dist + j * num_nodes_dist + i;
                ...
                bucket_signed_distance[local_node_idx] = DistanceFieldObject::computePhi(np, m_fields);
				
				m_particle_cells.loop_neighbor_bucket_particles(node_idx, [&] (int pidx, int) -> bool {
 ...
 //       std::cout << "[computeSignedDistance 2]" << std::endl;
		...
		m_particle_buckets.for_each_bucket([&] (int bucket_idx) {
			...
			for(int k = 0; k < num_nodes_dist; ++k) for(int j = 0; j < num_nodes_dist; ++j) for(int i = 0; i < num_nodes_dist; ++i)
			{
				...
				const int cur_node_local_index = k * num_nodes_dist * num_nodes_dist + j * num_nodes_dist + i;
				
				const Scalar& cur_node_dist = m_node_signed_distance[bucket_idx][cur_node_local_index];
				...
				for(int r = 0; r < 3; ++r) {
					...
					if(next_bucket_handle(r) >= m_particle_buckets.dim_size(r)) continue;
					
					const int next_bucket_idx = m_particle_buckets.bucket_index(next_bucket_handle);
					if(!m_bucket_activated[next_bucket_idx]) continue;
					
					const int next_node_local_index = next_node_handle(2) * num_nodes_dist * num_nodes_dist + next_node_handle(1) * num_nodes_dist + next_node_handle(0);
					
					const Scalar& next_node_dist = m_node_signed_distance[next_bucket_idx][next_node_local_index];
...
//        std::cout << "[computeSignedDistance 3]" << std::endl;
		...
		std::partial_sum(bucket_num_ips.begin(), bucket_num_ips.end(), bucket_num_ips.begin());
 ...
 //       std::cout << "[computeSignedDistance 4]" << std::endl;
		...
		for_each(0, num_buckets, [&] (int bucket_idx) {
			...
			for(int i = 0; i < num_ips_local; ++i) {
				m_interface_points.segment<3>((base_idx + i) * 3) = interface_points[bucket_idx][i];
			}
		});
 ...
 //       std::cout << "[computeSignedDistance 5]" << std::endl;
		...
		m_particle_buckets.for_each_bucket([&] (int bucket_idx) {
            ...
            for(int i = 0; i < num_cells; ++i) {
                m_node_signed_distance[bucket_idx][i] = sgn(m_node_signed_distance[bucket_idx][i]) * std::numeric_limits<Scalar>::max();
            }
        });
		
 //       std::cout << "[computeSignedDistance 6]" << std::endl;
        
		m_interface_buckets.resize(m_particle_buckets.ni, m_particle_buckets.nj, m_particle_buckets.nk);
		m_interface_buckets.sort(num_ips, [&] (int pidx, int& i, int& j, int& k) {
			i = (int) std::floor((m_interface_points(pidx * 3 + 0) - m_grid_mincorner(0)) / m_bucket_size);
			j = (int) std::floor((m_interface_points(pidx * 3 + 1) - m_grid_mincorner(1)) / m_bucket_size);
			k = (int) std::floor((m_interface_points(pidx * 3 + 2) - m_grid_mincorner(2)) / m_bucket_size);
		});
        
 //       std::cout << "[computeSignedDistance 7]" << std::endl;
		
		m_interface_buckets.for_each_bucket_particles_colored_randomized([&] (int pidx, int bucket_idx) {
			const Vec3x& ip_pos = m_interface_points.segment<3>(pidx * 3);
			const Vec3i global_cell_handle = Vec3i((int) std::floor((ip_pos(0) - m_grid_mincorner(0)) / dr),
												   (int) std::floor((ip_pos(1) - m_grid_mincorner(1)) / dr),
												   (int) std::floor((ip_pos(2) - m_grid_mincorner(2)) / dr));
			
			const Vec3i bucket_handle = m_interface_buckets.bucket_handle(bucket_idx);
			const Vec3i local_cell_handle = Vec3i(global_cell_handle(0) - bucket_handle(0) * num_nodes_dist,
												  global_cell_handle(1) - bucket_handle(1) * num_nodes_dist,
												  global_cell_handle(2) - bucket_handle(2) * num_nodes_dist);
			
			const Vec3x np_base = getNodePos(bucket_idx, 0, 4) + Vec3x(local_cell_handle(0) * dr,
																	   local_cell_handle(1) * dr,
																	   local_cell_handle(2) * dr);
			
			for(int k = -4; k <= 4; ++k) for(int j = -4; j <= 4; ++j) for(int i = -4; i <= 4; ++i) {
				...
				if(!m_interface_buckets.has_bucket(neighbor_bucket_handle)) continue;
				const int neigh_bucket_idx = m_interface_buckets.bucket_index(neighbor_bucket_handle);
				if(!m_bucket_activated[neigh_bucket_idx]) continue;
				
				const int neigh_node_idx = neighbor_cell_handle(2) * num_nodes_dist * num_nodes_dist + neighbor_cell_handle(1) * num_nodes_dist + neighbor_cell_handle(0);
				
				if(dist < fabs(m_node_signed_distance[neigh_bucket_idx][neigh_node_idx])) {
					m_node_signed_distance[neigh_bucket_idx][neigh_node_idx] = sgn(m_node_signed_distance[neigh_bucket_idx][neigh_node_idx]) * dist;
				}
			}
		}, 3);
 ...
 //       std::cout << "[computeSignedDistance 8]" << std::endl;
    ...
    {
        return m_components.segment( i * m_liquid_info.num_components, m_liquid_info.num_components );
    }
	
	void LiquidSimulator::findNodes( const Sorter& buckets, std::vector< Mat27x2i >& particle_nodes, const Vec3x& offset, const std::function< Vec3x(int) > get_pos, bool activate )
	{
		const Scalar dx = getDx();
		...
		buckets.for_each_bucket_colored([&] (int bucket_idx) {
			...
			buckets.get_bucket(bucket_idx, [&] (int pidx) {
				...
				Vec3i ilocal_pos = Vec3i( (int) floor(local_pos(0)), (int) floor(local_pos(1)), (int) floor(local_pos(2)) );
				...
				for(int k = klow; k <= khigh; ++k) for(int j = jlow; j <= jhigh; ++j) for(int i = ilow; i <= ihigh; ++i)
				{
					...
					for(int r = 0; r < 3; ++r) {
						...
						if( node_bucket_handle(r) < 0 || node_bucket_handle(r) >= buckets.dim_size(r)) {
							out_of_range = true;
							break;
						}
					}
	...
	{
		auto check_bucket = [&] (const Vec3i& bucket_handle, const std::vector<unsigned char>& activated) {
	...
	void LiquidSimulator::generateNodes()
	{
		...
		m_particle_buckets.for_each_bucket([&] (int bucket_idx) {
			...
			for(int k = 0; k < m_num_nodes; ++k) for(int j = 0; j < m_num_nodes; ++j) for(int i = 0; i < m_num_nodes; ++i)
			{
				...
				bucket_node_pos.segment<3>(node_idx * 3) = cell_local_corner + Vec3x(i, j, k) * dx;
			}
	...
	void LiquidSimulator::connectPressureNodes()
	{
		...
		m_particle_buckets.for_each_bucket_colored([&] (int bucket_idx) {
			...
			VecXi& bucket_node_pressure_neighbors = m_node_pressure_neighbors[bucket_idx];
	...
	void LiquidSimulator::postAllocateNodes()
	{
        ...
        allocateNodeVectors(m_node_mass_fluid_x, m_node_mass_fluid_y, m_node_mass_fluid_z);
        allocateNodeVectors(m_node_vel_fluid_x, m_node_vel_fluid_y, m_node_vel_fluid_z);
        allocateNodeVectors(m_node_vel_fluid_plus_x, m_node_vel_fluid_plus_y, m_node_vel_fluid_plus_z);
        allocateNodeVectors(m_node_vol_fluid_x, m_node_vol_fluid_y, m_node_vol_fluid_z);
        allocateNodeVectors(m_node_rhs_fluid_x, m_node_rhs_fluid_y, m_node_rhs_fluid_z);
        allocateNodeVectors(m_node_vel_elastic_x, m_node_vel_elastic_y, m_node_vel_elastic_z);
        allocateNodeVectors(m_node_drag_coeff_x, m_node_drag_coeff_y, m_node_drag_coeff_z);
        allocateNodeVectors(m_node_vel_constraint_coeff_x, m_node_vel_constraint_coeff_y, m_node_vel_constraint_coeff_z);
        allocateNodeVectors(m_node_pressure_grad_x, m_node_pressure_grad_y, m_node_pressure_grad_z);
        allocateNodeVectors(m_node_solid_vel_x, m_node_solid_vel_y, m_node_solid_vel_z);
        allocateNodeVectors(m_node_liquid_valid_x, m_node_liquid_valid_y, m_node_liquid_valid_z);
...
//        allocateNodeVectors(m_node_pressure_hessian_p, 6);
	...
	void LiquidSimulator::resampleNodes()
	{
        ...
        if(num_parts > 0) {
            ...
            findNodes(m_particle_buckets, m_particle_nodes_x, Vec3x(0.0, 0.5, 0.5), part_pos);
            findNodes(m_particle_buckets, m_particle_nodes_y, Vec3x(0.5, 0.0, 0.5), part_pos);
            findNodes(m_particle_buckets, m_particle_nodes_z, Vec3x(0.5, 0.5, 0.0), part_pos);
            findNodes(m_particle_buckets, m_particle_nodes_p, Vec3x(0.5, 0.5, 0.5), part_pos);
        }
		...
		if(m_strands_global_local.size() > 0) {
			...
			findNodes(m_vertex_buckets, m_vertex_nodes_x, Vec3x(0.0, 0.5, 0.5), vert_pos);
			findNodes(m_vertex_buckets, m_vertex_nodes_y, Vec3x(0.5, 0.0, 0.5), vert_pos);
			findNodes(m_vertex_buckets, m_vertex_nodes_z, Vec3x(0.5, 0.5, 0.0), vert_pos);
			findNodes(m_vertex_buckets, m_vertex_nodes_p, Vec3x(0.5, 0.5, 0.5), vert_pos);
		}
    ...
    void LiquidSimulator::updateVertexWeights()
    {
        ...
        auto accum_weight = [&] (const Mat27x2i& indices, const Vec3x& pos, Mat27x4f& weights, int ir) -> bool {
            bool touched = false;
            ...
            for(int nidx = 0; nidx < indices.rows(); ++nidx)
            {
                ...
                if(node_bucket_idx != -1 && node_idx != -1 && m_bucket_activated[node_bucket_idx]) {
...
//            std::cout << "vt " << i << ": " << (int) vertex_touched[i] << std::endl;
//        }
...
//
//        for(int i = 0; i < num_strands; ++i) {
//            std::cout << "sbm " << i << ": " << (int) m_strands_submerged[i] << std::endl;
//        }
    }
    ...
    void LiquidSimulator::findInterfacingSegments()
    {
        ...
        std::vector< std::vector< std::pair<int, int> > > strand_inside_edges(num_strand);
        ...
        for_each(0, num_strand, [&] (int strand_idx) {
            ...
            for(int i = 0; i < num_edges; ++i) {
                strand_inside_edges[strand_idx][i] = std::pair<int, int>(strand_idx, inside_edges[i]);
            }
        });
        ...
        auto edge_pos = [this] (int eidx) -> Vec3x {
            ...
            const ElasticStrand* strand = m_strands[local_idx.first];
            return (strand->getVertex(local_idx.second) + strand->getVertex(local_idx.second + 1)) * 0.5;
        };
        
        m_edge_buckets.resize(m_particle_buckets.ni, m_particle_buckets.nj, m_particle_buckets.nk);
        ...
        findNodes(m_edge_buckets, m_inside_edge_nodes_x, Vec3x(0.0, 0.5, 0.5), edge_pos, false);
        findNodes(m_edge_buckets, m_inside_edge_nodes_y, Vec3x(0.5, 0.0, 0.5), edge_pos, false);
        findNodes(m_edge_buckets, m_inside_edge_nodes_z, Vec3x(0.5, 0.5, 0.0), edge_pos, false);
    ...
    void LiquidSimulator::updateEdgeWeights()
    {
        ...
        auto accum_weight = [&] (const Mat27x2i& indices, const Vec3x& pos, Mat27x3f& weights, int ir) {
            for(int nidx = 0; nidx < indices.rows(); ++nidx)
            {
                const int node_bucket_idx = indices(nidx, 0);
                const int node_idx = indices(nidx, 1);
	...
	void LiquidSimulator::updateParticleWeights()
	{
        ...
        auto accum_weight = [&] (const Mat27x2i& indices, const Vec3x& pos, Mat27x4f& weights, int ir) {
            for(int nidx = 0; nidx < indices.rows(); ++nidx)
            {
                const int node_bucket_idx = indices(nidx, 0);
                const int node_idx = indices(nidx, 1);
...
//            accum_weight_grad(indices_z, pos, weights, grad_z, 2);
    ...
    template< int N >
    void LiquidSimulator::buildNodePairs(const Sorter& buckets, NodeObjIndex& node_obj, const vector< Eigen::Matrix<float, 27, N> >& weights, const vector< Mat27x2i >& obj_node, int isel, std::function< bool(int) > func)
    {
        ...
        buckets.for_each_bucket([&] (int bucket_idx) {
            ...
            if((int) bucket_node_particles.size() != num_nodes) bucket_node_particles.resize( num_nodes );
        ...
        buckets.for_each_bucket_particles_colored([&] (int pidx, int bucket_idx) {
            if(!m_bucket_activated[bucket_idx]) return;
            ...
            for(int i = 0; i < indices.rows(); ++i)
            {
                if(indices(i, 0) >= 0 && indices(i, 1) >= 0 && m_bucket_activated[indices(i, 0)] && w(i, isel) > 0.0) {
                    node_obj[ indices(i, 0) ][ indices(i, 1) ].emplace_back( std::pair<int, short>( pidx, i ) );
                }
            }
        }, 3);
    }
    ...
    void LiquidSimulator::buildNodeEdgePairs()
    {
        ...
        buildNodePairs<3>(m_edge_buckets, m_node_edge_x, m_inside_edge_weights, m_inside_edge_nodes_x, 0, func);
        buildNodePairs<3>(m_edge_buckets, m_node_edge_y, m_inside_edge_weights, m_inside_edge_nodes_y, 1, func);
        buildNodePairs<3>(m_edge_buckets, m_node_edge_z, m_inside_edge_weights, m_inside_edge_nodes_z, 2, func);
    }
    
    void LiquidSimulator::buildNodeVertexPairs()
    {
        buildNodePairs<4>(m_vertex_buckets, m_node_vertex_x, m_vertex_weights, m_vertex_nodes_x, 0);
        buildNodePairs<4>(m_vertex_buckets, m_node_vertex_y, m_vertex_weights, m_vertex_nodes_y, 1);
        buildNodePairs<4>(m_vertex_buckets, m_node_vertex_z, m_vertex_weights, m_vertex_nodes_z, 2);
        buildNodePairs<4>(m_vertex_buckets, m_node_vertex_p, m_vertex_weights, m_vertex_nodes_p, 3);
    }
    
	void LiquidSimulator::buildNodeParticlePairs()
	{
        //std::cout << "[buildNodeParticlePairs 0]" << std::endl;
        buildNodePairs<4>(m_particle_buckets, m_node_particles_x, m_particle_weights, m_particle_nodes_x, 0);
        
        //std::cout << "[buildNodeParticlePairs 1]" << std::endl;
        buildNodePairs<4>(m_particle_buckets, m_node_particles_y, m_particle_weights, m_particle_nodes_y, 1);
        
        //std::cout << "[buildNodeParticlePairs 2]" << std::endl;
        buildNodePairs<4>(m_particle_buckets, m_node_particles_z, m_particle_weights, m_particle_nodes_z, 2);
        
        //std::cout << "[buildNodeParticlePairs 3]" << std::endl;
        buildNodePairs<4>(m_particle_buckets, m_node_particles_p, m_particle_weights, m_particle_nodes_p, 3);
	...
	bool LiquidSimulator::updatePlasticity()
	{
		...
		for_each(0, numParticles(), [&] (int pidx) {
            ...
            const Scalar plastic_relax_coeff = m_liquid_info.plastic_relax_coeff.dot(color);
            const Scalar P_coeff = (exp(-m_dt / plastic_relax_coeff) - 1.0) * 0.5;
            
			newP(0, 0) = pow(P(0, 0), P_coeff); newP(1, 1) = pow(P(1, 1), P_coeff); newP(2, 2) = pow(P(2, 2), P_coeff);
...
#pragma omp critical
                {
                    ...
                    std::cout << "relaxcoeff: " << plastic_relax_coeff << std::endl << std::endl;
	...
	bool LiquidSimulator::updateSolidPhi()
	{
		m_particle_buckets.for_each_bucket([&] (int bucket_idx) {
			...
			{
				Vec3x vel;
				node_phi(i) = DistanceFieldObject::computePhi(getNodePosSolidPhi(bucket_idx, i), m_fields);
				
				DistanceFieldObject::computePhiVel(getNodePosX(bucket_idx, i), vel, m_fields);
				node_solid_vel_x(i) = vel(0);
				
				DistanceFieldObject::computePhiVel(getNodePosY(bucket_idx, i), vel, m_fields);
				node_solid_vel_y(i) = vel(1);
				
				DistanceFieldObject::computePhiVel(getNodePosZ(bucket_idx, i), vel, m_fields);
	...
	void LiquidSimulator::updateSolidWeights()
	{
		...
		m_particle_buckets.for_each_bucket([&] (int bucket_idx) {
			...
			const VecXi& bucket_node_idx_solid_phi_x = m_node_index_solid_phi_x[bucket_idx];
			const VecXi& bucket_node_idx_solid_phi_y = m_node_index_solid_phi_y[bucket_idx];
			const VecXi& bucket_node_idx_solid_phi_z = m_node_index_solid_phi_z[bucket_idx];
			...
			for(int i = 0; i < num_solid_phi; ++i)
			{
				...
				bucket_weight_x(i) = clamp( 1.0 - fraction_inside(phi0, phi1, phi2, phi3), 0.0, 1.0 );
			}
			...
			for(int i = 0; i < num_solid_phi; ++i)
			{
				...
				bucket_weight_y(i) = clamp( 1.0 - fraction_inside(phi0, phi1, phi2, phi3), 0.0, 1.0 );
			}
			...
			for(int i = 0; i < num_solid_phi; ++i)
			{
				...
				bucket_weight_z(i) = clamp( 1.0 - fraction_inside(phi0, phi1, phi2, phi3), 0.0, 1.0 );
			}
		});
	}
	...
	void LiquidSimulator::updateLiquidPhi()
	{
		...
		m_particle_buckets.for_each_bucket_particles_colored([&] (int pidx, int) {
			const auto& indices = m_particle_nodes_p[pidx];
			
			const Vec3x& pos = m_x.segment<3>(pidx * 3);// + m_fluid_v.segment<3>(pidx * 4) * dt;
			...
			for(int i = 0; i < indices.rows(); ++i) {
				...
				const Scalar phi = (pos - np).norm() - std::max(dx * 0.883644, m_radius(pidx));
	...
	{
		const Scalar rad_fine = defaultRadiusMultiplier() * getDx() * m_liquid_info.particle_cell_multiplier;
	...
	void LiquidSimulator::particleCheckWeakened()
	{
		...
		for_each(0, num_parts, [&] (int pidx) {
            ...
            const VecXx& color = m_components.segment(pidx * m_liquid_info.num_components, m_liquid_info.num_components);
			m_weakened[pidx] = len_dev_Cp >= m_liquid_info.plastic_weaken_strain.dot(color);
		});
	}
	...
	void LiquidSimulator::splitLiquidParticles()
	{
		...
		for_each(0, num_fluids, [&] (int pidx) {
			...
			const int n_split = std::min((int) std::ceil(m_vol(pidx) / V_fine), (int) sphere_pattern::max_vector_length);
			if(n_split <= 1) return;
			...
			{
				new_part_pos[pidx][i - 1] = center + Q * m_sphere_pattern[n_split].segment<3>(i * 3) * splat_rad;
			}
			
			n_additional[pidx] = n_split - 1;
			
			m_x.segment<3>(pidx * 3) = center + Q * m_sphere_pattern[n_split].segment<3>(0) * splat_rad;
		...
		for_each(0, num_fluids, [&] (int pidx_parent) {
			...
			for(int i = 0; i < num_new_parts; ++i) {
				...
				m_b.block<3, 3>(pidx * 3, 0) = m_b.block<3, 3>(pidx_parent * 3, 0);
                m_b_plus.block<3, 3>(pidx * 3, 0) = m_b_plus.block<3, 3>(pidx_parent * 3, 0);
				m_Fe.block<3, 3>(pidx * 3, 0) = m_Fe.block<3, 3>(pidx_parent * 3, 0);
                m_Fe_plus.block<3, 3>(pidx * 3, 0) = m_Fe_plus.block<3, 3>(pidx_parent * 3, 0);
                m_b_trial.block<3, 3>(pidx * 3, 0) = m_b_trial.block<3, 3>(pidx_parent * 3, 0);
                m_proj_func.segment<3>(pidx * 3) = m_proj_func.segment<3>(pidx_parent * 3);
				m_J(pidx) = m_J(pidx_parent);
				m_weakened(pidx) = m_weakened(pidx_parent);
				m_classifier[pidx] = m_classifier[pidx_parent];
                m_components.segment(pidx * m_liquid_info.num_components, m_liquid_info.num_components) = m_components.segment(pidx_parent * m_liquid_info.num_components, m_liquid_info.num_components);
			}
		});
	...
	void LiquidSimulator::mergeLiquidParticles()
	{
        ...
        std::vector< VecXx > gathered_color( num_parts, VecXx::Zero( m_liquid_info.num_components ) );
		...
		m_particle_buckets.for_each_bucket_particles_colored_randomized([&] (int pidx, int bucket_idx) {
			...
			if(m_classifier[pidx] == PC_S) {
				...
				m_particle_buckets.loop_neighbor_bucket_particles(bucket_idx, [&] (int npidx, int) {
					if( !removed[npidx] && pidx != npidx && (m_classifier[npidx] == PC_S || m_classifier[npidx] == PC_s || m_classifier[npidx] == PC_o) )
					{
						const Scalar neigh_vol = m_vol(npidx) + gathered_vol[npidx];
						if(neigh_vol > V_fine) return false;
						
						const Scalar dist = ( m_x.segment<3>(pidx * 3) - m_x.segment<3>(npidx * 3) ).norm();
						if(dist < should_rad) {
				...
				Vec3x distrib_moment = (m_v.segment<3>(pidx * 3) * m_vol[pidx] + gathered_moment[pidx]) * invN;
 				Mat3x distrib_def_grad = (m_Fe.block<3, 3>(pidx * 3, 0) * m_vol[pidx] + gathered_def_grad[pidx]) * invN;
				Mat3x distrib_L = (m_B.block<3, 3>(pidx * 3, 0) * m_vol[pidx] + gathered_L[pidx]) * invN;
                Mat3x distrib_b = (m_b.block<3, 3>(pidx * 3, 0) * m_vol[pidx] + gathered_b[pidx]) * invN;
                VecXx distrib_color = (m_components.segment( pidx * m_liquid_info.num_components, m_liquid_info.num_components ) * m_vol[pidx] + gathered_color[pidx]) * invN;
				...
				m_particle_buckets.loop_neighbor_bucket_particles(bucket_idx, [&] (int npidx, int) {
					if( pidx != npidx && !removed[npidx] && m_classifier[npidx] == PC_s )
					{
						const Scalar neigh_vol = m_vol(npidx) + gathered_vol[npidx];
						if(neigh_vol > V_fine) return false;
				...
				Vec3x distrib_moment = (m_v.segment<3>(pidx * 3) * m_vol[pidx] + gathered_moment[pidx]) * coeff;
 				Mat3x distrib_def_grad = (m_Fe.block<3, 3>(pidx * 3, 0) * m_vol[pidx] + gathered_def_grad[pidx]) * coeff;
				Mat3x distrib_L = (m_B.block<3, 3>(pidx * 3, 0) * m_vol[pidx] + gathered_L[pidx]) * coeff;
                Mat3x distrib_b = (m_b.block<3, 3>(pidx * 3, 0) * m_vol[pidx] + gathered_b[pidx]) * coeff;
				VecXx distrib_color = (m_components.segment( pidx * m_liquid_info.num_components, m_liquid_info.num_components ) * m_vol[pidx] + gathered_color[pidx]) * coeff;
		...
		
        {
            ...
            for_each(0, num_parts, [&] (int pidx) {
                ...
                const Vec3x full_moment = m_v.segment<3>(pidx * 3) * m_vol[pidx] + gathered_moment[pidx];
                const Scalar full_vol = m_vol[pidx] + gathered_vol[pidx];
                ...
                // check_isnan("unmerge b trial", m_b_trial.block<3, 3>(pidx * 3, 0).sum());
                // check_isnan("unmerge proj func", m_proj_func.segment<3>(pidx * 3).sum());
                // check_isnan("unmerge b", m_b.block<3, 3>(pidx * 3, 0).sum());
                // check_isnan("unmerge b plus", m_b_plus.block<3, 3>(pidx * 3, 0).sum());
                ...
                Mat3x avg_def_grad = (m_Fe.block<3, 3>(pidx * 3, 0) * m_vol[pidx] + gathered_def_grad[pidx]) * inv_full_vol;
                ...
                const Mat3x avg_L = (m_B.block<3, 3>(pidx * 3, 0) * m_vol[pidx] + gathered_L[pidx]) * inv_full_vol;
                const Mat3x avg_b = (m_b.block<3, 3>(pidx * 3, 0) * m_vol[pidx] + gathered_b[pidx]) * inv_full_vol;
                VecXx avg_c = m_components.segment( pidx * m_liquid_info.num_components, m_liquid_info.num_components ) * m_vol[pidx] + gathered_color[pidx];
                ...
                m_Fe.block<3, 3>(pidx * 3, 0) = pow(m_J(pidx) / det_avg_def_grad, 1.0 / 3.0) * avg_def_grad;
                
                Mat3x be_bar_trial = avg_b / pow(avg_b.determinant(), 1.0 / 3.0);
                
                Vec3x proj_func = computeProjFunc(be_bar_trial, getYieldStress(avg_c), getShearModulus(avg_c), getViscosity(avg_c), getFlowBehaviorIndex(avg_c));
                Scalar Ie_bar = be_bar_trial.trace() / 3.0;
                
                m_b.block<3, 3>(pidx * 3, 0) = proj_func(0) * (be_bar_trial - Ie_bar * Mat3x::Identity()) + Ie_bar * Mat3x::Identity();
                m_b_trial.block<3, 3>(pidx * 3, 0) = be_bar_trial;
                ...
                m_components.segment( pidx * m_liquid_info.num_components, m_liquid_info.num_components ) = avg_c;
		...
		if(new_num_parts < num_parts) {
			...
			if(sort) {
				m_particle_buckets.sort(new_num_parts, [&] (int pidx, int& i, int& j, int& k) {
					i = (int)floor((m_x(pidx * 3 + 0) - m_grid_mincorner(0)) / m_bucket_size);
					j = (int)floor((m_x(pidx * 3 + 1) - m_grid_mincorner(1)) / m_bucket_size);
					k = (int)floor((m_x(pidx * 3 + 2) - m_grid_mincorner(2)) / m_bucket_size);
				});
			}
		}
	}
...
//        std::cout << "[correctLiquidParticles 1]" << std::endl;
        ...
        m_particle_cells_single.for_each_bucket_particles_colored([&] (int liquid_pidx, int cell_idx) {

            if(liquid_pidx % m_liquid_info.correction_step != correction_selector) return;

            const Vec3x& pos = m_x.segment<3>(liquid_pidx * 3);
            Scalar dist = interpolateValue(pos, m_node_signed_distance, m_grid_mincorner, 0.0, m_liquid_info.signed_distance_multiplier);
            if(dist > -0.75 * dx) {
                return;
            }
            ...
            m_particle_cells_single.loop_neighbor_bucket_particles(cell_idx, [&] (int liquid_npidx, int) -> bool {
 ...
 //       std::cout << "[correctLiquidParticles 2]" << std::endl;
	...
	bool LiquidSimulator::isWeakened(int pidx) const
	{
		return m_weakened[pidx];
	}
	...
	bool LiquidSimulator::solidProjection()
	{
		...
		for_each(0, num_parts, [&] (int pidx) {
			const Vec3x& pos = m_x.segment<3>(pidx * 3);
			Vec3x n = Vec3x::Zero();
			const Scalar phi = interpolateValueAndGradient(n, pos, m_node_solid_phi, m_grid_mincorner, 3.0 * dx);
			
			n.normalize();
			
            if(m_liquid_info.solid_projection) {
                const Vec3x dpos = (m_v.segment<3>(pidx * 3) - get_solid_velocity(pos)) * m_dt;
    ...
    Scalar LiquidSimulator::getTotalVolMeshFlows() const
    {
        ...
        const int num_fields = (int) m_fields.size();
        for_each(0, num_fields, [&] (int field_idx) {
            if(!m_fields[field_idx].has_surf_flow)
                return;
            
            std::shared_ptr<TriangularMesh> mesh = m_fields[field_idx].mesh_controller->getCurrentMesh();
            std::shared_ptr<TriangularMeshFlow> flow = m_fields[field_idx].mesh_controller->getMeshFlow();
    ...
    MutexType& LiquidSimulator::getParticleMutex()
    {
        return m_particle_mutex;
    }
    
    MutexType& LiquidSimulator::getGridMutex()
    {
        return m_grid_mutex;
    }
	
	const VecXx& LiquidSimulator::getX() const
	{
		return m_x;
	}
	const VecXx& LiquidSimulator::getV() const
	{
		return m_v;
	}
	const VecXx& LiquidSimulator::getM() const
	{
		return m_m;
	}
	const VecXx& LiquidSimulator::getRadius() const
	{
		return m_radius;
	}
	const VecXx& LiquidSimulator::getJ() const
	{
		return m_J;
	}
	const VecXx& LiquidSimulator::getVol() const
	{
		return m_vol;
	}
	const VecXx& LiquidSimulator::getRestVol() const
	{
		return m_rest_vol;
	}
	...
	const VecXuc& LiquidSimulator::getWeakened() const
	{
		return m_weakened;
	}
	const VecXx& LiquidSimulator::getComponents() const
	{
		return m_components;
	}
	const VecXx& LiquidSimulator::getProjFunc() const
	{
		return m_proj_func;
	}
	const MatXx& LiquidSimulator::getFe() const
	{
		return m_Fe;
	}
	const MatXx& LiquidSimulator::getb() const
	{
		return m_b;
	}
	const MatXx& LiquidSimulator::getB() const
	...
	const MatXx& LiquidSimulator::getbtrial() const
	{
		return m_b_trial;
	}
	const MatXx& LiquidSimulator::getFePlus() const
	{
		return m_Fe_plus;
	}
	const MatXx& LiquidSimulator::getbPlus() const
	{
		return m_b_plus;
	}
	
	VecXx& LiquidSimulator::getX()
	{
		return m_x;
	}
	VecXx& LiquidSimulator::getV()
	{
		return m_v;
	}
	VecXx& LiquidSimulator::getM()
	{
		return m_m;
	}
	VecXx& LiquidSimulator::getRadius()
	{
		return m_radius;
	}
	VecXx& LiquidSimulator::getJ()
	{
		return m_J;
	}
	VecXx& LiquidSimulator::getVol()
	{
		return m_vol;
	}
	VecXx& LiquidSimulator::getRestVol()
	{
		return m_rest_vol;
	}
	...
	VecXuc& LiquidSimulator::getWeakened()
	{
		return m_weakened;
	}
	VecXx& LiquidSimulator::getComponents()
	{
		return m_components;
	}
	VecXx& LiquidSimulator::getProjFunc()
	{
		return m_proj_func;
	}
	MatXx& LiquidSimulator::getFe()
	{
		return m_Fe;
	}
	MatXx& LiquidSimulator::getb()
	{
		return m_b;
	}
	MatXx& LiquidSimulator::getB()
	...
	MatXx& LiquidSimulator::getbtrial()
	{
		return m_b_trial;
	}
	MatXx& LiquidSimulator::getFePlus()
	{
		return m_Fe_plus;
	}
	MatXx& LiquidSimulator::getbPlus()
	{
		return m_b_plus;
	}
};
