...
{
private:
    inline static double DEFAULT_CELL_SIZE()
    { return 1. ; }
    inline static double MAX_MEAN_OBJECTS_PET_CELL()
    { return 2. ; }
...
public:
    ...
    explicit SpatialHashMap( const Scalar cellSize = DEFAULT_CELL_SIZE() ) :
            m_minCellSize( cellSize ), m_cellSize( cellSize ), m_invCellSize(
                    1. / cellSize )
    {
    }
    ...
    void updateObject( DataT & object, const IteratorT& begin, const IteratorT& end );
    //! Batch updates a list of objects, sampling from DataT::subsamples_begin() to DataT::subsamples_end()
    // with | DataT::subsamples_begin() - DataT::subsamples_end() | <= maxSubsamples
    void batchUpdate( const std::vector<DataT *> & object, const size_t maxSubSamples );
    ...
    void addToFootPrint( DataT & object, const IteratorT& begin, const IteratorT& end );
    //! Check if an object is at least partially in the footprint
    bool isInFootPrint( DataT & object, const IteratorT& begin, const IteratorT& end ) const ;
    ...
    public:
        Cell( int i, int j, int k ) :
                m_i( i ), m_j( j ), m_k( k ), m_hash(
                        ( m_i * 73856093 ) ^ ( m_j * 19349663 ) ^ ( m_k * 83492791 ) )
        {
        }
        ...
        struct Hasher
        {
            long operator()( const Cell &k ) const
            {
                return k.hash();
            }
        };
    ...
    typedef std::unordered_set<Cell, typename Cell::Hasher, typename Cell::Comparer> HashSet;
    typedef std::map<DataT*, std::vector<IteratorT> > Samples;
    typedef std::unordered_map<Cell, Samples, typename Cell::Hasher, typename Cell::Comparer> HashMap;
...
public:
    ...
    {
    public:
        typedef std::unordered_map< DataT*, std::unordered_map< DataT*,
        std::set< std::pair< IteratorT, IteratorT > > > > Collisions;
    ...
    void compute(const typename ObjectCells::const_iterator objectCellsIt,
        bool selfCollisions, bool checkAllObjectsInCells, typename Result::Collisions &collisions ) const;
    void compute( typename HashMap::const_iterator, typename Samples::const_iterator object1,
            bool selfCollisions, bool checkAllObjectsInCells, typename Result::Collisions &collisions ) const;

    //! Generates cells from a 3D segment
    template < typename ContainerT >
    static void generateCellsFromSegment( const DataT &object, const IteratorT &it, const Scalar invCellSize,
                                          ContainerT &cells );

    //! Generates cells from a 3d axis-aligned bounding box
    template < typename ContainerT >
    static void generateCellsFromAABB( DataT &object, const IteratorT &it, const Scalar invCellSize,
            ContainerT &cells );
...
void generateHashMapCells( DataT& object, const IteratorT& it, const Scalar invCellSize,
                           ContainerT &cells,
                           const typename SpatialHashMap<DataT, IteratorT, false>::Cell::GeneratorType )
{
    SpatialHashMap<DataT, IteratorT, false>::generateCellsFromSegment( object, it, invCellSize, cells ) ;
}

template<typename DataT, typename IteratorT, typename ContainerT >
void generateHashMapCells( DataT& object, const IteratorT& it, const Scalar invCellSize,
                           ContainerT &cells,
                           const typename SpatialHashMap<DataT, IteratorT, true>::Cell::GeneratorType )
{
    SpatialHashMap<DataT, IteratorT, true >::generateCellsFromAABB( object, it, invCellSize, cells ) ;
}
            ...
            ContainerT &cells )
{
    ...
    generateHashMapCells< DataT, IteratorT, ContainerT >( object, it, invCellSize, cells, CellT::GENERATOR ) ;
...
template<typename DataT, typename IteratorT, bool GAABB>
void SpatialHashMap<DataT, IteratorT, GAABB>::addObject( DataT & object, const IteratorT& begin,
        const IteratorT& end )
{
    HashSet objectCells;
    ...
    {
        generateHashMapCells< DataT, IteratorT > ( object, it, m_invCellSize, sampleCells[it] ) ;
    }
    ...
    for ( IteratorT it = begin; it < end; ++it )
    {
        ...
        for ( typename std::vector<Cell>::const_iterator cell = sampleCells[it].begin();
    ...
    //std::cout << "Add -> Current hash map size:" << m_hashMap.size() << std::endl ;
...
//! Adds a single object to the footprint, sampling from begin to end
template<typename DataT, typename IteratorT, bool GAABB>
void SpatialHashMap<DataT, IteratorT, GAABB>::addToFootPrint( DataT & object, const IteratorT& begin,
        const IteratorT& end )
{
    ...
    for ( IteratorT it = begin; it < end; ++it )
    {
        generateHashMapCells< DataT, IteratorT > ( object, it, m_invCellSize, sampleCells[it-begin] ) ;
    }

    for ( IteratorT it = begin; it < end; ++it )
    {
        // Concatenate sample's cells to object's cells
        m_footPrint.insert( sampleCells[it-begin].begin(), sampleCells[it-begin].end() );
    }
}

template<typename DataT, typename IteratorT, bool GAABB>
bool SpatialHashMap<DataT, IteratorT, GAABB>::isInFootPrint( DataT & object, const IteratorT& begin,
        const IteratorT& end ) const
{
    ...
    {
        sampleCells.clear() ;
        generateHashMapCells< DataT, IteratorT > ( object, it, m_invCellSize, sampleCells ) ;

        for ( typename std::vector< Cell >::const_iterator cell = sampleCells.begin() ; cell != sampleCells.end() ; ++cell )
        {
...
public:
    ...
    typedef std::unordered_map<Cell, std::vector<IteratorT>, typename Cell::Hasher,
            typename Cell::Comparer> HashMap;
...
template<typename DataT, typename IteratorT, bool GAABB>
class SpatialHashMap<DataT, IteratorT, GAABB>::ParallelAdder_Input //: public tbb::filter
{
private:
...
public:
    ParallelAdder_Input( const std::vector<DataT *> & objects, unsigned chunkSize ) : //tbb::filter(true),
            m_objects( objects ), m_curIndex( 0 ), m_chunkSize( chunkSize )
    {
    }
    virtual ~ParallelAdder_Input()
    {
//        std::cout << "Did everythin with " << m_buffers.size() << " buffers. Great. " << std::endl ;
    ...
    void* operator()( void* data )
    {
        ...
        {
            unsigned next = std::min( m_curIndex + m_chunkSize, ( unsigned ) m_objects.size() );
...
template<typename DataT, typename IteratorT, bool GAABB >
class SpatialHashMap<DataT, IteratorT, GAABB>::ParallelAdder_Transform //: public tbb::filter
{
...
public:
    ParallelAdder_Transform( const std::vector<DataT *> & objects, const size_t maxSubsamples,
            const Scalar invCellSize ) : //tbb::filter(false),
            m_objects( objects ), m_invCellSize( invCellSize ), m_maxSubSamples( maxSubsamples )
    {
    }
    ...
    void* operator()( void* data )
    {
        ...
        for ( int i = b.m_begin; i < b.m_end; ++i )
        {
            ...
            for ( IteratorT it = samplesBegin; it < samplesEnd; ++it )
            {
                ...
                generateHashMapCells< DataT, IteratorT >( *m_objects[i], it, m_invCellSize, sampleCells[it] );

                // Concatenate sample's cells to object's cells
                b.m_objectCells[curIdx].insert( sampleCells[it].begin(), sampleCells[it].end() );
            }
    ...
    public:
        ...
        StaticArg() :
                m_object( NULL ), m_data( NULL )
        {
        }

        void setFunctor( ParallelAdder_Transform &f )
        {
            m_object = &f;
        }
...
template<typename DataT, typename IteratorT, bool GAABB>
class SpatialHashMap<DataT, IteratorT, GAABB>::ParallelAdder_Output //: public tbb::filter
{
...
public:
    ...
    const std::vector<DataT *> & objects, bool doCleanUp ) : //tbb::filter( true ),
            m_hashMap( hashMap ), m_objects( objects ), m_doCleanUp( doCleanUp )
    {
    }
...
#pragma omp critical
        {
            ...
            for ( int i = b.m_begin; i < b.m_end; ++i )
            {
                ...
                for ( typename ParallelAdder_Buffer::HashMap::iterator iter =
                        b.m_objectMaps[curIdx].begin(); iter != b.m_objectMaps[curIdx].end();
                        ++iter )
                        {
                ...
                if( m_doCleanUp )
                {
                    ...
                    for ( typename HashSet::const_iterator cell = objectCells.begin();
                            ...
                            {
                        if ( b.m_objectCells[curIdx].end() == b.m_objectCells[curIdx].find( *cell ) )
                        {
...
template<typename DataT, typename IteratorT, bool GAABB>
void SpatialHashMap<DataT, IteratorT, GAABB>::batchUpdate( const std::vector<DataT *> & objects,
...
#pragma omp critical
    ...
    if( doCleanUp )
    {
        for ( typename HashMap::iterator iter = m_hashMap.begin(); iter != m_hashMap.end(); ++iter )
        {
...
//    std::cout << "batchUpdate " << nObj << "-> Current hash map size:" << m_hashMap.size() << ", cell size: " << m_cellSize << std::endl ;
//    std::cout << m_hashMap.load_factor() << std::endl ;
    ...
    if ( m_cellSize >= 2 * m_minCellSize )
    {
        ...
        unsigned objSum = 0;
        for ( typename HashMap::const_iterator iter = m_hashMap.begin(); iter != m_hashMap.end();
                ++iter )
                {
        ...
        //std::cout << "Cell size: " << m_cellSize << "; Mean objects per cell: " << meanOPC << std::endl ;
...
void SpatialHashMap<DataT, IteratorT, GAABB>::removeObject( DataT & object )
{
    ...
    //std::cout << "Remove -> Num of Object's cells:" << objectCells.size() << std::endl ;

    for ( typename HashSet::iterator cell = objectCells.begin(); cell != objectCells.end(); ++cell )
    {
    ...
    //std::cout << "Remove -> Current hash map size:" << m_hashMap.size() << std::endl ;
...
template<typename DataT, typename IteratorT, bool GAABB>
void SpatialHashMap<DataT, IteratorT, GAABB>::updateObject( DataT & object, const IteratorT& begin,
        const IteratorT& end )
{
...
template<typename DataT, typename IteratorT, bool GAABB>
bool SpatialHashMap<DataT, IteratorT, GAABB>::Result::next( Collisions &collisions )
{
    ...
    {
        collisions[ m_objectsIters[ iterIdx ]->first ] ; // Create external hashmap structure
    }

#pragma omp parallel for
    for( int iterIdx = begin ; iterIdx < end ; ++ iterIdx )
    {
        m_hashMap->compute( m_objectsIters[ iterIdx ],
                            m_doSelfCollisions, m_checkAllObjectsInCells, collisions ) ;
    }
...
template<typename DataT, typename IteratorT, bool GAABB>
void SpatialHashMap<DataT, IteratorT, GAABB>::compute( const typename ObjectCells::const_iterator objectCellsIt,
        ...
        typename Result::Collisions &collisions ) const
{
    ...
    for ( typename HashSet::const_iterator cell = cells.begin(); cell != cells.end(); ++cell )
    {
        typename HashMap::const_iterator mapCell = m_hashMap.find( *cell );
        if ( mapCell != m_hashMap.end() )
        {
            const Samples& samples = mapCell->second;
            const typename Samples::const_iterator object1 = samples.find( objectCellsIt->first ) ;
            if( object1 != samples.end() )
            {
                compute( mapCell, object1, selfCollisions, checkAllObjectsInCells, collisions );
            }
        }
    }
}
...
template<typename DataT, typename IteratorT, bool GAABB>
void SpatialHashMap<DataT, IteratorT, GAABB>::compute( typename HashMap::const_iterator iter,
        typename Samples::const_iterator object1, bool selfCollisions, bool checkAllObjectsInCells,
        typename Result::Collisions &collisions ) const
{
    ...
    {
        if ( !selfCollisions && object2 == object1 )
            continue;
...
template<typename DataT, typename IteratorT, bool GAABB>
void SpatialHashMap<DataT, IteratorT, GAABB>::compute( Result &result,
        const std::list<DataT *> * objects ) const
{
    ...
    if ( objects )
    {
                ...
                object != objects->end(); ++object )
        {
            ...
            typename ObjectCells::const_iterator objectCellsIt = m_objectCells.find( *object );
    ...
    else
    {
        ...
        int i = 0 ;
        for(  auto objectCellsIt = m_objectCells.begin() ; objectCellsIt != m_objectCells.end() ; ++objectCellsIt )
        {
            objectsIters[ i++ ] = objectCellsIt ;
        }
            ...
            ContainerT &cells )
{
    ...
    int permuted[3] = { (int) std::floor( cur[ip] ), (int) std::floor( cur[jp] ), (int) std::floor( cur[kp] ), };
...
//    if ( ( imax - imin ) * ( jmax - jmin ) * ( kmax - kmin ) > MAX_CELLS_PER_OBJECT )
//    {
//        std::cerr << "SHM - Warning: Unusually big object " << std::endl;
//        return;
//    }
