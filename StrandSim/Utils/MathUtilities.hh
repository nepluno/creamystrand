...
#ifndef M_PI
const Scalar M_PI = 3.;
#endif
...
namespace strandsim
{
    ...
    inline Vec3x parallelTransport( const Vec3x& u, const Vec3x& t0, const Vec3x& t1 )
    {
     ...
     */
    inline Vec3x normalParallelTransport( const Vec3x& u, const Vec3x& t0, const Vec3x& t1 )
    {
     ...
     */
    inline Vec3x orthonormalParallelTransport( const Vec3x& u, const Vec3x& t0, const Vec3x& t1 )
    {
        // This should be called only to transport an orthogonal vector
        assert( isSmall(u.dot(t0)) );
    ...
    inline void check_isnan(const char* name, const std::vector< Eigen::Matrix<T, Eigen::Dynamic, 1> >& v)
    {
        ...
        for(const auto& vec : v) {
            std::ostringstream oss;
            oss << name << " [" << i << "]";
            check_isnan(oss.str().c_str(), sqrt(vec.squaredNorm() / (double) std::max(1, (int) vec.size())), vec);
            ++i;
        }
    }
    ...
    {
        check_isnan(name, sqrt(v.squaredNorm() / (double) std::max(1, (int) v.size())), v);
    }
    ...
    inline Scalar wedge_wet_abscissa(const Scalar& A0, const Scalar& A1, const Scalar& d0, const Scalar& d1, const Scalar& theta)
    {
        if(fabs(d0 - d1) < 1e-12) {
            if(fabs(A0 - A1) < 1e-12) {
                ...
                return clamp((A0-(d0*d0)*1.0/pow(theta*(1.0/2.0)+1.0,2.0))/(A0-A1), 0.0, 1.0);
            }
        } else {
            ...
            const Scalar t15 = t7+t8+t9+t10+t11+t12+t13+t14-A0*A1*8.0-A0*A1*t2*2.0-A0*A1*theta*8.0-A0*d0*d1*1.6E1-A1*d0*d1*1.6E1;
            if(t15 < 0.0) {
                return 0.0;
            }
            const Scalar t16 = sqrt(t15);
            return clamp(1.0/pow(d0-d1,2.0)*(A0*-4.0+A1*4.0+t5*8.0+t16*2.0-A0*t2+A1*t2-A0*theta*4.0+A1*theta*4.0-d0*d1*8.0+t16*theta), 0.0, 1.0);
        }
    }
	...
	inline void d2WsdF2B(const Scalar J, const Mat3x& F,
						 const Mat3x& b_bar, const Scalar& mu, const Mat3x& B, Mat3x& ret)
	{
		...
		ret = mu * (J_fac * B
					- 2.0 / 3.0 * (invF * B).trace() * M0
					- 1.0 / 3.0 * invFT * (J_fac * (F.transpose() * B).trace() * Mat3x::Identity() - b_bar.trace() * B.transpose() * invFT));
	}
    ...
    inline T poly6_kernel( const T& r2, const T& h ) {
        return std::max( (T) pow(h * h - r2, (T) 3.0), (T) 0.0 ) / pow(h, 9.0) * 1.5666814711; // unit: cm^{-3}
    }
	...
	inline Scalar grad_N_kernel_x(const Vec3x& x, const Scalar& h, const int order) {
	...
	inline Scalar grad_N_kernel_y(const Vec3x& x, const Scalar& h, const int order) {
		std::function<Scalar(const Scalar&)> val = (order == 1) ? linear_kernel : ((order == 3) ? cubic_kernel : quad_kernel);
		std::function<Scalar(const Scalar&)> grad = (order == 1) ? grad_linear_kernel : ((order == 3) ? grad_cubic_kernel : grad_quad_kernel);
	...
	inline Scalar grad_N_kernel_z(const Vec3x& x, const Scalar& h, const int order) {
		std::function<Scalar(const Scalar&)> val = (order == 1) ? linear_kernel : ((order == 3) ? cubic_kernel : quad_kernel);
		std::function<Scalar(const Scalar&)> grad = (order == 1) ? grad_linear_kernel : ((order == 3) ? grad_cubic_kernel : grad_quad_kernel);
...
//        std::cout<<"s:"<<s<<std::endl;
	...
	inline void grad_triangle(const Vec3x& x0, const Vec3x& x1, const Vec3x& x2, Mat3x& grad_coeff)
	{
	...
	inline void get_div_triangle(const Scalar& va0, const Scalar& va1, const Scalar& va2, const Scalar& thickness0, const Scalar& thickness1, const Scalar& thickness2, const Vec3x& x0, const Vec3x& x1, const Vec3x& x2, Vec3x& div0, Vec3x& div1, Vec3x& div2)
	{
		Vec3x n = (x2 - x0).cross(x1 - x0);
		const Scalar len = n.norm();
		if(len < 1e-63) {
	...
	inline Scalar interpolate_2nd_order(const Vec3x& p, const Array3T& arr, Callable func)
	{
		Vec3x frac_p = Vec3x(p(0) - floor(p(0)), p(1) - floor(p(1)), p(2) - floor(p(2)));
		Vec3i base_p = Vec3i((int) floor(p(0)) + (frac_p(0) < 0.5 ? -1 : 0),
								   (int) floor(p(1)) + (frac_p(1) < 0.5 ? -1 : 0),
								   (int) floor(p(2)) + (frac_p(2) < 0.5 ? -1 : 0));
		
		Vec3x local_p = frac_p + Vec3x( frac_p(0) < 0.5 ? 1 : 0,
										  frac_p(1) < 0.5 ? 1 : 0,
										  frac_p(2) < 0.5 ? 1 : 0);
    ...
    inline void dhdr_yarn(Scalar mu, Scalar la, Scalar r22, Scalar r23, Scalar r33, Scalar *dhdr22, Scalar *dhdr23, Scalar *dhdr33){
    ...
    inline void dgdr_cloth(Scalar mu, Scalar r13, Scalar r23, Scalar *dhdr13, Scalar *dhdr23){
        *dhdr13 = mu * r13;
        *dhdr23 = mu * r23;
    }
    ...
    inline Scalar twist_component( const Eigen::Quaternion<Scalar>& rot, const Vec3x& dir )
    {
    	Vec3x ra = rot.vec();
	...
	void print_histogram_analysis(const std::vector< VecXx >&, int, const std::string&, bool);
    ...
    T find_in_ordered( const Eigen::Matrix<T, Eigen::Dynamic, 1>& vec, const T& x ) {
        ...
        if(l < 0) return (T) l;
        if(l >= vec.size() - 1) {
            if(x > vec[vec.size() - 1]) return vec.size();
            else return l;
        }
