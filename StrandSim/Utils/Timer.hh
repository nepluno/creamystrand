...
#define FETCH_TIMER(name)               \
  Timer::getTimer(name).computeElapsedTime()

#define SAVE_TIMERS(filename)          \
  Timer::writeTimers(filename)
...
public:
  ...
  // Returns the timer for a given name. Reuses the timer if it exists, otherwise creates a new one.
...
private:
  ...
  int m_nesting_level;
  // The deepest nesting level encountered (helps spot unbalanced begin/end-block)
