...
#define SS_MTREE_LOG_LINE( a ) do { std::cerr << a << std::endl ; } while( 0 )
#else
...
/*!
  ...
  Based on a classical M-Tree with some ideas from Sparse Spatial Selection [ Brisaboa et al 2008 ]
  \tparam ScalarT Type of the codomain of the metric ( usually \c float or \c double )
  \tparam DataT Type of the arguments ( data points ) of the metric.
  \tparam LeafDataT Tag that can be associated with the objects inserted in the tree.
    Those tags will be retrieved when running queries on the MetricTree.
  \taparam MetricT Type of the metric functor
     The following signature should be defined:
     ScalarT MetricT::operator() (const Data&, const DataT& ) ;
*/
template < typename ScalarT, typename DataT, typename LeafDataT, typename MetricT >
class MetricTree
{
public:
    //! Index of an entry ; useful for easily retrieving objects already inserted in the tree.
    typedef size_t EntryIndexT ;

    //! Maximum number of children for one node of the tree. Thie higher M, the flatter the tree
    static const int M = 24 ;
    //! When trying to insert a object into a node N with existing children, if its distance to the
    //! center of all existing children is greater than alpha()*diam( N ), create a new node
    inline static ScalarT alpha()
    { return 0.275 ; }


    //! Constructor
    /*! \param metric The functor used as the metric
      \param nObj Approximate number of objects that will be inserted, for performance purposes
      \param Approximate diameter of the objects that will be inserted, for performance purposes
      */
    ...
    EntryIndexT insert( const DataT& point, const LeafDataT &leafData )
    {
        ...
        m_leafNodeIndexes.push_back(
                    insert( m_rootIndex, point, eIdx, m_diam ) );
    ...
    void update( const EntryIndexT eIdx, const DataT& newPoint )
    {
       ...
       m_leafNodeIndexes[ entry.leafData() ] = insert( m_rootIndex, newPoint, eIdx, m_diam ) ;
    ...
    /*! \p point does not need to have been inserted in the tree.
      \param leadfData The tags of all the objects of the tree that are with \p radius of \p point
      */
    void range ( const DataT& point, const ScalarT radius, std::vector< LeafDataT > &leafData ) const
    {
    ...
    //! kNearestNeighbours query; finds the \p k entries closest to the \p point and return their tags
    void kNN ( const DataT& point, const unsigned k, std::vector< LeafDataT > &leafData ) const
    {
    ...
    //! kNearestNeighbours query; finds the \p k entries closest to the \p point and return their tags
    /*! Implementation as successive range-queries with increasing radii ; for comparsion purposes only
      */
    void old_kNN ( const DataT& point, unsigned k, std::vector< LeafDataT > &leafData ) const
    {
        Scalar radius = 1.e-2 ;
    ...
    //! Returns the tag associated with the data point that has entry index \p eIdx
    ...
    std::ostream& print( std::ostream& os ) const
    {
        ...
        for( unsigned i = 0 ; i < m_entries.size() ; ++i )
        {
            ...
            os << i << "\t" ;
            if ( entry.isLeaf() )  os << m_leafData[ entry.leafData() ] ;
            else                   os << entry.m_node ;
            os << "\t" ;
            os << entry.m_dist << "\t" ;
        ...
        os << " ====== " << std::endl ;
        os << " Population: " << m_size << " / Distance calls: " << m_nCalls << std::endl ;
    ...
    {
        std::cout << " Population: " << m_size << " / Distance calls: " << m_nCalls << std::endl ;
        std::cout << " Num entries: " << m_entries.size() << " / Num nodes: " << m_nodes.size() << std::endl ;
        std::cout << " Entries per node: " << ( ( ( ScalarT) m_entries.size() ) / m_nodes.size() ) <<  std::endl ;
        std::cout << " Entries per obj: " << ( ( ( ScalarT) m_entries.size() ) / m_size ) <<  std::endl ;
    }
...
private:
    ...
    {
        return node.m_parent == -1 ;
    }

    //! Wrapper above the metric in order to count calls
    ScalarT distance( const DataT &P1, const DataT &P2 ) const
    {
//#pragma omp atomic // We do not really need an accurate count ( except for profiling )
        ++ m_nCalls ;
...
//        std::cerr << " Did not find parent for node #" << nodeIdx << std::endl ;
        assert( false ) ;
        return -1 ;
    }
    ...
    /*! eventually chosing new centers for those entries at the same time.
      \param createRoot If true, we have just created a new root and those entries are directly below it.
      Therefore, we cannot use the center of the parent entry to help us find a good partition
      */
    void partition( Entry &pe1 , Entry& pe2, const bool createRoot )
    {
        ...
        if ( createRoot )
        {
            ...
            for( short i = 0 ; i < M+1 ; ++i )
            {
                ...
                {
                    ds[ i + (M+1)*j ] = ds[ j + (M+1)*i ] =
                            distance( m_entries[ list[ i ] ].m_center,
                                      m_entries[ list[ j ] ].m_center ) ;
                }
            }
            ...
            for( short i = 0 ; i < M+1 ; ++i )
            {
                for( short j = i+1 ; j < M+1 ; ++j )
                {
                    ...
                    {
                        maxRad = std::max( maxRad,
                                           std::min( di[k], dj[k] ) ) ;
                    }

//                    std::cout << " Max rad for # " << i << " " << j << " -> " << maxRad << std::endl ;
            ...
            SS_MTREE_LOG_LINE( " Re-part root with children " << mink1 << " and " << mink2 ) ;
...
//            pe2.m_radius = std::min( oldRadius, pe2.m_radius ) ;
    ...
    void createParentNodeAndEntry( NodeIndexT& newNodeIdx, EntryIndexT& newEntryIdx, const EntryIndexT childEntryIdx )
    {
       newNodeIdx = newNode() ;
    ...
    {
        SS_MTREE_LOG_LINE( " Splitting node #" <<  nodeIdx << " to add entry #" << eIdx );
        ...
        bool createRoot ;
        if ( ( createRoot = isRoot( m_nodes[ nodeIdx ] ) ) ) // Pretty sure this is meant as an assignment :)
        {
           m_rootIndex = newNode() ;
    ...
    /*!
      ...
      \param dist Distance of \p point to the center of the parent entry of \p nodeIdx
      */
    NodeIndexT insert( const NodeIndexT nodeIdx, const DataT &point, const EntryIndexT eIdx, const ScalarT diam, const ScalarT dist = 0. )
    {

        SS_MTREE_LOG_LINE( " Insert " << eIdx << " in node #" << nodeIdx << ", dist = " << dist ) ;
        ...
        if ( isLeaf( node ) )
        {
            ...
            if ( mink == -1 )
            {
                ...
                {
                    SS_MTREE_LOG_LINE( " Too far away, adding node ( " << d[mink] << " vs " << diam ) ;
                    ...
                    return insert( m_entries[ node.m_entries[mink] ].m_node, point, eIdx, 2.*d[mink], d[mink] ) ;
                }
            } else {
                SS_MTREE_LOG_LINE( " Inserting in satisfying child " << mink ) ;

                return insert( m_entries[ node.m_entries[mink] ].m_node, point, eIdx, 2. * m_entries[ node.m_entries[mink] ].m_radius, d[mink] ) ;
            }
        }
    ...
    /*!
      ...
      \param dist Distance of \p point to the center of the parent entry of \p nodeIdx
      */
    void range ( const NodeIndexT nodeIdx, const DataT& point, const ScalarT radius, const ScalarT dist,
                 std::list< QueryResult >& indexes ) const
    {
    ...
    /*!
      ...
      \param nodeIdx Current node to examine
      \param point Center of the query
      \param radius Current radius of the query ; will be refined as more nearest neighbours are found
      \param dist Distance of \p point to the center of the parent entry of \p nodeIdx
      */
    void kNN ( const unsigned K, const NodeIndexT nodeIdx, const DataT& point, ScalarT &radius, const ScalarT dist,
...
template < typename ScalarT, typename DataT, typename LeafDataT, typename MetricT >
std::ostream& operator<<( std::ostream& os, const MetricTree< ScalarT, DataT, LeafDataT, MetricT > & M )
{
    return M.print( os ) ;
}
