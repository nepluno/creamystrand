...
std::ostream*
TextLog::getStream(const MsgInfo& info)
{
    ...
    if (severity == MsgInfo::kError ||
        severity == MsgInfo::kSevere ||
...
/// Varargs method for writing a message that includes a filename and line number.
void
TextLog::WriteWhereV(MsgInfo info, const char* filename, int line,
                     const char* format, va_list ap)
{
    ...
    *out << info;
    ...
    // Format and write the message.  For now we use vsnprintf with a fixed-sized local buffer.
