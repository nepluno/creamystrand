...
class Sorter {
 public:
  ...
  inline int dim_size(int d) const { return Vec3i(ni, nj, nk)(d); }
  ...
  void for_each_bucket(Callable func, bool forced_serial = false) const {
    ...
    if (forced_serial) {
      ...
      for_each(0, nsystem, [&](int bucket_idx) { func(bucket_idx); });
    }
  }
  ...
  inline int get_neighbor_bucket_particles_size(int bucket_idx,
  ...
  template <typename Callable>
  void loop_neighbor_bucket_particles(int bucket_idx, Callable func, int wl,
                                      int wh, int hl, int hh, int dl,
                                      int dh) const {
    Vec3i center_handle = bucket_handle(bucket_idx);
