...
public:
    ...
    // dynamic constructor
    SimpleMeshController(const std::string& base_szfn,
    ...
    bool initMeshFlow( const std::shared_ptr< FluidScriptingController >& fluid_controller, const Scalar& init_height );
    ...
    std::shared_ptr<TriangularMesh> getPreviousMesh()
    {
        return m_previousMesh;
    }
    
    std::shared_ptr<TriangularMesh> getNextMesh()
    {
        return m_nextMesh;
    }
    
    std::shared_ptr<TriangularMesh> getCurrentMesh()
    {
        return m_currentMesh;
    }
	
	virtual int getIFrame()
	{
		return m_currentMeshFrame;
	}
	
	virtual void setIFrame(int iframe)
	{
		m_currentMeshFrame = iframe;
	}
    ...
    std::shared_ptr<TriangularMeshFlow> getMeshFlow() const
    {
        return m_meshFlow;
    }
