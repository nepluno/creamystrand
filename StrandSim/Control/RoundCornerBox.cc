...
#include "RoundCornerBox.hh"

using namespace strandsim;

namespace strandsim {
RoundCornerBox::RoundCornerBox(int N, const Vec3x& b, const double& radius, strandsim::TriangularMesh* mesh, bool inverted)
: m_mesh(mesh)
{
	int N_edge = 2 * (N + 1);
	m_N_edge = N_edge;
	...
	for(int kidx = 0; kidx < 2; ++kidx) {
		const int k = ks[kidx];
		Vec3x origin = Vec3x(-b(0) - radius, -b(1) - radius, (b(2) + radius) * sign[kidx]);
		for(int j = 0; j <= N; ++j) for(int i = 0; i <= N; ++i)
		{
			Vec3x pos = origin + Vec3x(dx * i, dx * j, 0.0);
			AddVertex(i, j, k, pos, Vec3x(-b(0), -b(1), b(2) * sign[kidx]));
			
			pos = origin + Vec3x(dx * i + 2.0 * b(0) + radius, dx * j, 0.0);
			AddVertex(i + N + 1, j, k, pos, Vec3x(b(0), -b(1), b(2) * sign[kidx]));
			
			pos = origin + Vec3x(dx * i + 2.0 * b(0) + radius, dx * j + 2.0 * b(1) + radius, 0.0);
			AddVertex(i + N + 1, j + N + 1, k, pos, Vec3x(b(0), b(1), b(2) * sign[kidx]));
			
			pos = origin + Vec3x(dx * i, dx * j + 2.0 * b(1) + radius, 0.0);
			AddVertex(i, j + N + 1, k, pos, Vec3x(-b(0), b(1), b(2) * sign[kidx]));
		}
		// corners
		for(int j = 0; j < N; ++j) for(int i = 0; i < N; ++i)
		{
	...
	for(int kidx = 0; kidx < 2; ++kidx) {
		const int k = ks[kidx];
		Vec3x origin = Vec3x(-b(0) - radius, (b(1) + radius) * sign[kidx], -b(2) - radius);
		for(int j = 0; j <= N; ++j) for(int i = 0; i <= N; ++i)
		{
			Vec3x pos = origin + Vec3x(dx * i, 0.0, dx * j);
			AddVertex(i, k, j, pos, Vec3x(-b(0), b(1) * sign[kidx], -b(2)));
			
			pos = origin + Vec3x(dx * i + 2.0 * b(0) + radius, 0.0, dx * j);
			AddVertex(i + N + 1, k, j, pos, Vec3x(b(0), b(1) * sign[kidx], -b(2)));
			
			pos = origin + Vec3x(dx * i + 2.0 * b(0) + radius, 0.0, dx * j + 2.0 * b(2) + radius);
			AddVertex(i + N + 1, k, j + N + 1, pos, Vec3x(b(0), b(1) * sign[kidx], b(2)));
			
			pos = origin + Vec3x(dx * i, 0.0, dx * j + 2.0 * b(2) + radius);
			AddVertex(i, k, j + N + 1, pos, Vec3x(-b(0), b(1) * sign[kidx], b(2)));
		}
		// corners
		for(int j = 0; j < N; ++j) for(int i = 0; i < N; ++i)
		{
	...
	for(int kidx = 0; kidx < 2; ++kidx) {
		const int k = ks[kidx];
		Vec3x origin = Vec3x((b(0) + radius) * sign[kidx], -b(1) - radius, -b(2) - radius);
		for(int j = 0; j <= N; ++j) for(int i = 0; i <= N; ++i)
		{
			Vec3x pos = origin + Vec3x(0.0, dx * i, dx * j);
			AddVertex(k, i, j, pos, Vec3x(b(0) * sign[kidx], -b(1), -b(2)));
			
			pos = origin + Vec3x(0.0, dx * i + 2.0 * b(1) + radius, dx * j);
			AddVertex(k, i + N + 1, j, pos, Vec3x(b(0) * sign[kidx], b(1), -b(2)));
			
			pos = origin + Vec3x(0.0, dx * i + 2.0 * b(1) + radius, dx * j + 2.0 * b(2) + radius);
			AddVertex(k, i + N + 1, j + N + 1, pos, Vec3x(b(0) * sign[kidx], b(1), b(2)));
			
			pos = origin + Vec3x(0.0, dx * i, dx * j + 2.0 * b(2) + radius);
			AddVertex(k, i, j + N + 1, pos, Vec3x(b(0) * sign[kidx], -b(1), b(2)));
		}
		// corners
		for(int j = 0; j < N; ++j) for(int i = 0; i < N; ++i)
		{
