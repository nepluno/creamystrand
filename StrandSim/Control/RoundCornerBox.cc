...
namespace strandsim {
RoundCornerBox::RoundCornerBox(int N, const Vec3x& b, const double& radius,
  ...
  for (int kidx = 0; kidx < 2; ++kidx) {
    const int k = ks[kidx];
    Vec3x origin =
        Vec3x(-b(0) - radius, -b(1) - radius, (b(2) + radius) * sign[kidx]);
    for (int j = 0; j <= N; ++j)
      for (int i = 0; i <= N; ++i) {
        ...
        pos = origin + Vec3x(dx * i + 2.0 * b(0) + radius,
                             dx * j + 2.0 * b(1) + radius, 0.0);
        AddVertex(i + N + 1, j + N + 1, k, pos,
                  Vec3x(b(0), b(1), b(2) * sign[kidx]));
  ...
  for (int kidx = 0; kidx < 2; ++kidx) {
    const int k = ks[kidx];
    Vec3x origin =
        Vec3x(-b(0) - radius, (b(1) + radius) * sign[kidx], -b(2) - radius);
    for (int j = 0; j <= N; ++j)
      for (int i = 0; i <= N; ++i) {
        ...
        pos = origin + Vec3x(dx * i + 2.0 * b(0) + radius, 0.0,
                             dx * j + 2.0 * b(2) + radius);
        AddVertex(i + N + 1, k, j + N + 1, pos,
                  Vec3x(b(0), b(1) * sign[kidx], b(2)));
  ...
  for (int kidx = 0; kidx < 2; ++kidx) {
    const int k = ks[kidx];
    Vec3x origin =
        Vec3x((b(0) + radius) * sign[kidx], -b(1) - radius, -b(2) - radius);
    for (int j = 0; j <= N; ++j)
      for (int i = 0; i <= N; ++i) {
        ...
        pos = origin + Vec3x(0.0, dx * i + 2.0 * b(1) + radius,
                             dx * j + 2.0 * b(2) + radius);
        AddVertex(k, i + N + 1, j + N + 1, pos,
                  Vec3x(b(0) * sign[kidx], b(1), b(2)));
