...
namespace strandsim {
    SimpleMeshController::SimpleMeshController(const std::string& base_szfn,
                                               ...
                                               int startMeshFrame,
                                               int endMeshFrame) :
    MeshScriptingController( 0., (endMeshTime - startMeshTime) / (double) (endMeshFrame - startMeshFrame - 1)),
    ...
    bool SimpleMeshController::initMeshFlow( const std::shared_ptr< FluidScriptingController >& fluid_controller, const Scalar& init_height )
    {
        m_meshFlow = std::make_shared<TriangularMeshFlow>( m_currentMesh, fluid_controller, init_height );
    ...
    {
        // 1st make the previous mesh equal the next mesh as we're about to update next mesh
        m_previousMesh->vertices().swap( m_nextMesh->vertices() );
        
        // TODO : enforce that current_mesh->vertices == m_m_previousMesh->vertices() here after this swap or elsewhere, i.e., in problem stepper?
        
        // load next mesh
        std::stringstream name;
        name << m_base_szfn << "." << setfill('0') << setw(5) << frame_num << ".obj";
        std::string obj_file_name = name.str().c_str();
        std::shared_ptr<TriangularMesh> nextFrameMesh = std::make_shared<TriangularMesh>();
        ObjParser objparser;
    ...
    bool SimpleMeshController::interpolateCurrentMesh( Scalar alpha )
    {
        ...
        {
            Vec3x currentPoint = ( 1. - alpha ) * m_previousMesh->getVertex( vtx ) + alpha * m_nextMesh->getVertex( vtx );
            
            m_currentMesh->setDisplacement( vtx, currentPoint - m_currentMesh->getVertex( vtx ) );
...
//            std::cout << v[i] << ", " << m_dt << std::endl;
    ...
    bool SimpleMeshController::updateMesh( double i_time, bool update_sub_data )
    {
        ...
        double tfrac = i_time / m_dt - (double) (m_currentMeshFrame - m_startMeshFrame);
