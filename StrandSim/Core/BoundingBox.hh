...
struct BoundingBox
{
	...
	BoundingBox(ScalarT minx, ScalarT miny, ScalarT minz, ScalarT maxx, ScalarT maxy,
		ScalarT maxz) :
	...
	{
		return (max - min).array().prod();
	}

	ScalarT maxDim() const
	{
		return (max - min).maxCoeff();
	}

	template<typename ScalarT1> friend std::ostream& operator<<(std::ostream& os,
		const BoundingBox<ScalarT1>& elem);

	template<typename ScalarT1> friend bool intersect(const BoundingBox<ScalarT1>& bbox_a,
		const BoundingBox<ScalarT1>& bbox_b);

	template<typename ScalarT1> friend bool is_contained(const BoundingBox<ScalarT1>& bbox1,
		const BoundingBox<ScalarT1>& bbox2, const Scalar tol);

	template<typename ScalarT1> friend BoundingBox<ScalarT1> intersection(const BoundingBox<ScalarT1>& bbox1,
		const BoundingBox<ScalarT1>& bbox2);

	template<typename ScalarT1> friend bool is_left(const BoundingBox<ScalarT1>& bbox,
		const int axis, const Scalar pivot);

	template<typename ScalarT1> friend void insert(BoundingBox<ScalarT1>& bbox,
		const BoundingBox<ScalarT1>& bbox2);

	template<typename ScalarT1> friend BoundingBox<ScalarT1> merge(const BoundingBox<ScalarT1>& bbox1,
		const BoundingBox<ScalarT1>& bbox2);

    PointType ComputeCenter() const
    {
        return 0.5 * ( min + max );
    }
...
template<typename ScalarT>
bool is_contained(const BoundingBox<ScalarT>& bbox1, const BoundingBox<ScalarT>& bbox2,
	const Scalar tol)
{
	if (bbox1.max.x() > bbox2.max.x() + tol || bbox1.min.x() < bbox2.min.x() - tol
		|| bbox1.max.y() > bbox2.max.y() + tol || bbox1.min.y() < bbox2.min.y() - tol
		|| bbox1.max.z() > bbox2.max.z() + tol || bbox1.min.z() < bbox2.min.z() - tol)
		return false;
	else
		return true;
}
...
{
	return BoundingBox<ScalarT>(max(bbox1.min, bbox2.min), min(bbox1.max, bbox2.max));
}

template<typename ScalarT>
bool is_left(const BoundingBox<ScalarT>& bbox, const uint32_t axis, const ScalarT pivot)
{
...
template<typename ScalarT>
BoundingBox<ScalarT> merge(const BoundingBox<ScalarT>& bbox1, const BoundingBox<ScalarT>& bbox2)
{
	return BoundingBox<ScalarT>(
