...
public:
    ...
    VecXx col( int i ) const
    {
        ...
        const unsigned len = std::min(
                ( unsigned ) ( kl + ku + 1 ), (unsigned) ( m_rows-1 - start ) );
    ...
    void fixFirstDOFs( int startDof = 0 )
    {
        ...
        {
            for ( int j = std::max( 0, (int) i - (int) kl ); j <= std::min( ( int ) m_cols - 1, (int) i + (int) ku ); ++j )
                ( *this )( i, j ) = 0.0;
    ...
    // Add the local Jacobian to the diagonal part of matrix, top left corner at "start" position on the diagonal.
    ...
    BandMatrix<ScalarT, kl, ku> operator+( const BandMatrix<ScalarT, kl, ku> &rhs ) const
    {
    ...
    BandMatrix<ScalarT, kl, ku>& operator+=( const BandMatrix<ScalarT, kl, ku> &rhs )
    {
    ...
    BandMatrix<ScalarT, kl, ku>& operator*=( const BandMatrix<ScalarT, kl, ku> &rhs )
    {
        ...
        for ( IndexType i = 0; i < rows(); ++i )
        {
            ...
            {
                if ( indicesValid( i, j ) ) // Should theorically not be used, but gcc seems to use the wrong operator()
                {
                    A( i, j ) = ( *this )( i, j );
                    B( i, j ) = rhs( i, j );
                }
    ...
    Scalar diagNorm()
    {
        return  diagonal().array().abs().sum() / m_cols  ;
    }
