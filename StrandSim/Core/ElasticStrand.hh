...
namespace strandsim
{
    ...
    template<typename ViscousT> class FixingForce;
    ...
    public:
        ...
        typedef std::list<std::shared_ptr<RuntimeForceBase> > RuntimeForcesContainerType;
        ...
        typedef std::list<const ElasticStrand*>::iterator ClumpingAttractorsIteratorType;
        typedef std::list<const ElasticStrand*>::const_iterator ClumpingAttractorsConstIteratorType;
        
        ElasticStrand( const VecXx& dofs, ParametersType& parameters, CollisionParameters& collision_parameters,
                      DOFScriptingController* controller, int globalIndex = -1, const Vec3x& initRefFrame1 =
                      Vec3x() );
        
        ElasticStrand( const VecXx& dofs, const VecXx& area_dofs, ParametersType& parameters, CollisionParameters& collision_parameters,
                      DOFScriptingController* controller, int globalIndex = -1, const Vec3x& initRefFrame1 =
                      Vec3x() );
        ...
        void getCurrentVertices( Eigen::MatrixBase<Derived> &vertices ) const
        {
            ...
            {
                vertices.template segment<3>( 3 * vtx ) = dofs.segment<3>( 4 * vtx ).cast<OutScalar>();
            }
        }
        Vec2x& getReservoir()
        {
            return m_flow_reservoir;
        }
        const Vec2x& getReservoir() const
        {
            return m_flow_reservoir;
        }
        ...
        {
            return m_futureState->getEdgeVector( vtx ) / m_futureState->getEdgeLength( vtx );
        }
        ...
        size_t numberOfClumpingAttractors()
        {
            return m_clumpingAttractors.size();
        }
        
        void removeRuntimeForce( const RuntimeForceBase* toDelete );
        void removeSharedRuntimeForce( const std::shared_ptr<RuntimeForceBase> toDelete );
        ...
        void setIsClumpCenterLine( bool isit )
        {
            m_isClumpCenterLine = isit;
        }
        
        bool getIsClumpCenterLine() const
        {
            return m_isClumpCenterLine;
        }
        ...
        {
            return m_restLengths[ std::max((IndexType) 0, std::min(vtx, (IndexType) (m_numEdges - 1))) ];
        }
        ...
        Vec4x getKappaBar( const IndexType vtx ) const
        {
        ...
        Scalar getEdgeFlowInertia( IndexType i ) const
        {
            ...
            return 0.25 * mass1 * ( square( a ) + square( b ) + ( square(ha00) + square(ha01) + square(ha10) + square(ha11) ) * 0.5 );
        }
		...
		Scalar getCurrentEdgeFlowInertia( IndexType i ) const
		{
			const Scalar a = m_parameters.getRadiusA( i, getNumVertices() );
			const Scalar b = m_parameters.getRadiusB( i, getNumVertices() );
			const Scalar mass1 = (getCurrentSurfaceFlowMass(i) + getCurrentSurfaceFlowMass(i + 1)) * 0.5;
		...
		StrandState& getSavedState()
		{
			return *m_savedState;
		}
		
		const StrandState& getSavedState() const
		{
			return *m_savedState;
		}
        ...
        void createDynamics();
        const StrandDynamicTraits& dynamics() const
        {
            return *m_dynamics;
        }
        StrandDynamicTraits& dynamics()
        {
            return *m_dynamics;
        }
        ...
        void getAABB( unsigned elementID, Vec3x &min, Vec3x &max,
                     CollisionParameters::CollisionType type = CollisionParameters::SELF );
        void getFutureAABB( unsigned elementID, Vec3x &min, Vec3x &max,
                           CollisionParameters::CollisionType type = CollisionParameters::SELF );
        ...
        const Vec4xArray& getRestKappas() const
        {
            return m_restKappas;
        }
        ...
        void setParent(StrandImplicitManager* parent) {
            m_parent = parent;
        }
        
        StrandImplicitManager* getParent() const
        {
            return m_parent;
        }
    ...
    private:
        void getAABB( StrandState& geometry, unsigned elementID, Vec3x &min, Vec3x &max,
        ...
        void accumulateRuntimeForces( RuntimeForceBase::Quantities q, StrandState& geometry );
        ...
        template<typename ViscousT> friend class FixingForce;
