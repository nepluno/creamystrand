...
namespace strandsim
{
	...
	typedef Eigen::Matrix<Scalar, 27, 4> Mat27x4x; ///< 27x4d scalar vector (stencil for local forces)
	typedef Eigen::Matrix<Scalar, 27, 5> Mat27x5x; ///< 27x5d scalar vector (stencil for local forces)
	...
	typedef Eigen::Matrix<int, Eigen::Dynamic, 1> VecXi; ///< arbitrary dimension unsigned vector
	typedef Eigen::Matrix<unsigned char, Eigen::Dynamic, 1> VecXuc; ///< arbitrary dimension unsigned char vector
	...
	typedef Eigen::Matrix<Scalar, 1, 11> Vec11xT; ///< 11d scalar vector (stencil for local forces)
	
	typedef Eigen::Matrix<Scalar, 1, Eigen::Dynamic> VecXxT; ///< arbitrary dimension scalar vector
	typedef Eigen::Matrix<unsigned, 1, Eigen::Dynamic> VecXuT; ///< arbitrary dimension unsigned vector
	typedef Eigen::Matrix<int, 1, Eigen::Dynamic> VecXiT; ///< arbitrary dimension unsigned vector
	typedef Eigen::Matrix<unsigned char, 1, Eigen::Dynamic> VecXucT; ///< arbitrary dimension unsigned char vector
	
	typedef std::vector<Vec2x, Eigen::aligned_allocator<Vec2x> > Vec2xArray; ///< an array of 2d scalar vectors
	//typedef std::vector<Vec3x, Eigen::aligned_allocator<Vec3x> > Vec3xArray; ///< an array of 3d scalar vectors
	...
	typedef Eigen::Matrix<int, 27, 2> Mat27x2i; ///< 27x2d int vector (stencil for local forces)
	typedef Eigen::Matrix<int, 27, 3> Mat27x3i; ///< 27x3d int vector (stencil for local forces)
	typedef Eigen::Matrix<int, 27, 4> Mat27x4i; ///< 27x4d int vector (stencil for local forces)
	typedef Eigen::Matrix<int, 27, 5> Mat27x5i; ///< 27x5d int vector (stencil for local forces)
    
	typedef std::vector<Vec3i, Eigen::aligned_allocator<Vec3i> > Vec3iArray; ///< an array of 3d float vectors
	typedef std::vector<Vec4i, Eigen::aligned_allocator<Vec4i> > Vec4iArray; ///< an array of 4d float vectors
    typedef Eigen::Matrix<int, Eigen::Dynamic, Eigen::Dynamic> MatXi; ///< arbitrary dimension scalar matrix
    ...
    typedef Eigen::Matrix<short, 27, 2> Mat27x2s; ///< 27x2d int vector (stencil for local forces)
    typedef Eigen::Matrix<short, 27, 3> Mat27x3s; ///< 27x3d int vector (stencil for local forces)
    typedef Eigen::Matrix<short, 27, 4> Mat27x4s; ///< 27x4d int vector (stencil for local forces)
    typedef Eigen::Matrix<short, 27, 5> Mat27x5s; ///< 27x5d int vector (stencil for local forces)
    
    typedef std::vector<Vec3s, Eigen::aligned_allocator<Vec3s> > Vec3sArray; ///< an array of 3d float vectors
    typedef std::vector<Vec4s, Eigen::aligned_allocator<Vec4s> > Vec4sArray; ///< an array of 4d float vectors
    typedef Eigen::Matrix<short, Eigen::Dynamic, Eigen::Dynamic> MatXs; ///< arbitrary dimension scalar matrix
    ...
    typedef Eigen::Matrix<float, 11, 1> Vec11f; ///< 11d scalar vector (stencil for local forces)
    typedef Eigen::Matrix<float, 27, 2> Mat27x2f; ///< 27x2d scalar vector (stencil for local forces)
    typedef Eigen::Matrix<float, 27, 3> Mat27x3f; ///< 27x3d scalar vector (stencil for local forces)
    typedef Eigen::Matrix<float, 27, 4> Mat27x4f; ///< 27x4d scalar vector (stencil for local forces)
    typedef Eigen::Matrix<float, 27, 5> Mat27x5f; ///< 27x5d scalar vector (stencil for local forces)
    typedef Eigen::Matrix<float, Eigen::Dynamic, 1> VecXf; ///< arbitrary dimension scalar vector
	...
	//typedef std::vector<Vec3d, Eigen::aligned_allocator<Vec3d> > Vec3dArray; ///< an array of 3d double vectors
	typedef std::vector<Vec3d, Eigen::aligned_allocator<Vec3d> > Vec3dArray; ///< an array of 3d double vectors
    typedef std::vector<Vec4d, Eigen::aligned_allocator<Vec4d> > Vec4dArray; ///< an array of 3d double vectors
    ...
    typedef Eigen::Matrix<float, 11, 11> Mat11f; ///< 11x11 scalar matrix (stencil for local forces)
    typedef std::vector<Mat11f, Eigen::aligned_allocator<Mat11f> > Mat11fArray; ///< an array of 11d scalar matrices
    typedef std::pair<Mat11f, Mat11f> Mat11fPair;
    typedef Eigen::Matrix<float, Eigen::Dynamic, Eigen::Dynamic> MatXf; ///< arbitrary dimension scalar matrix
	...
	// Use int as index type ; as there is no MFnUintArrayData in maya API, it simplifies conversions
	// There is a MfnUint64ArrayData type though, but that would be overkill even if we some day manage
	// to have 150k strands inside the simulation
