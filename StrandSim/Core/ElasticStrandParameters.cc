...
namespace strandsim
{
	...
	ElasticStrandParameters::ElasticStrandParameters( const ElasticStrandParameters& other, const VecXx& radiusMultiplier ) :
    m_paramIndex( other.m_paramIndex ),
	m_density( other.m_density ), //
	m_viscosity( other.m_viscosity ), //
	m_airDrag( other.m_airDrag ), //
	...
	ElasticStrandParameters::ElasticStrandParameters( int paramIndex, Scalar radiusA, Scalar radiusB,
													 Scalar YoungsModulus, Scalar shearModulus, Scalar density, Scalar viscosity, Scalar airDrag,
													 Scalar baseRotation, Scalar stretchMultiplier, Scalar fixingMultiplier, Scalar minBendingAngle, Scalar maxFlowGradientRatio,  Scalar minBendingMultiplier,
	...
	ElasticStrandParameters::ElasticStrandParameters( int paramIndex, Scalar radiusA, Scalar radiusB,
													 Scalar YoungsModulus, Scalar shearModulus, Scalar density, Scalar viscosity, Scalar airDrag,
													 Scalar baseRotation, Scalar stretchMultiplier, Scalar fixingMultiplier, Scalar minBendingAngle, Scalar maxFlowGradientRatio,
                                                     const VecXx& radiusMultiplier, Scalar minBendingMultiplier, bool flowOpenAtStart, bool flowOpenAtEnd ) :
    m_paramIndex( paramIndex ),
	m_density( density ), //
	m_viscosity( viscosity ), //
	m_airDrag( airDrag ), //
	...
	void ElasticStrandParameters::computeViscousForceCoefficients( Scalar dt )
	{
		...
		m_viscousKt = M_PI_4 * m_radiusA * m_radiusB * ( m_radiusA * m_radiusA + m_radiusB * m_radiusB )
		* m_viscosity / dt;
		m_viscousBendingCoefficientBase = 3 * m_viscosity / dt;
		m_viscousLamb = m_airDrag / dt;
	}
	
	Scalar ElasticStrandParameters::interpolatedRadiusMultiplier( int vtx, int num_verts ) const
	{
		const Scalar s = vtx / (Scalar) (num_verts - 1) * (Scalar) (m_radiusMultiplier.size() - 1);
		const int i = (int) s;
		const Scalar frac = s - (Scalar) i;
		const int inext = std::min((int)m_radiusMultiplier.size() - 1, i + 1);
		
		return ( frac * m_radiusMultiplier[i] + ( 1. - frac ) * m_radiusMultiplier[inext] );
	}
