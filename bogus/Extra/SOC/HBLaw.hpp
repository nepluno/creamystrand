...
public:
	...
	/*!
	  ...
	  */
	HBLaw( const unsigned n, const double * yields, const double * etas, const double * powers ) ;
	...
	{
		typedef HerschelBulkleyFischerBurmeister< Traits::dimension, typename Traits::Scalar > FBFunction ;

		typename Traits::Vector fb( x.rows() ) ;
		return FBFunction::computeEnergy( m_yields[problemIndex], m_etas[problemIndex], m_powers[problemIndex], scaling, x, y ) ;
	}
