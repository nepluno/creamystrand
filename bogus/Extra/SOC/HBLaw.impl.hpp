...
namespace bogus {
    ...
    HBLaw< Dimension, Scalar, Strat >::HBLaw(const unsigned n, const double * yields, const double * etas, const double * powers )
    : m_yields(yields), m_etas(etas), m_powers(powers), m_n(n), m_localTol( std::pow( NumTraits< Scalar >::epsilon(), .75 ) )
    {
    }
    
    
    template < DenseIndexType Dimension, typename Scalar, local_soc_solver::Strategy Strat >
    bool HBLaw< Dimension, Scalar, Strat >::solveLocal(const unsigned problemIndex,
                                                                    const typename Traits::Matrix &A,
                                                                    const typename Traits::Vector &b,
                                                                    typename Traits::Vector &xm , const Scalar scaling ) const
    {
        typedef LocalHBSolver< Traits::dimension, typename Traits::Scalar, Strat > LocalSolver ;
        return m_localTol > LocalSolver::solve(  A, b, xm, m_yields[ problemIndex ], m_etas[ problemIndex ], m_powers[ problemIndex ], m_localTol, scaling ) ;
    }
    
    template < DenseIndexType Dimension, typename Scalar, local_soc_solver::Strategy Strat >
    void HBLaw< Dimension, Scalar, Strat >::projectOnConstraint(const unsigned problemIndex, typename Traits::Vector &x ) const
    {
        ...
        const Scalar yield_func = FBFunc::yieldFunction( m_yields[ problemIndex ], m_etas[ problemIndex ], m_powers[ problemIndex ], x, Traits::Vector::Zero() );
