...
struct AnalyticLocalSOCSolver {
  ...
  static bool solveOrthogonality(const typename Traits::Matrix &,
...
struct AnalyticLocalSOCSolver<3u, Scalar, true> {
  ...
  static bool solveOrthogonality(const typename Traits::Matrix &W,
    ...
    const Scalar coeffs[4] = {C * C - D * B.squaredNorm(),
                              2 * (C * A - D * bbar.dot(B)),
                              2 * C + A * A - D * bbar.squaredNorm(), 2 * A};

    Scalar roots[4];
    const unsigned nRoots = polynomial::getRealRoots(
        coeffs, roots, polynomial::StrictlyPositiveRoots);
...
struct AnalyticLocalSOCSolver<3u, Scalar, false> {
  ...
  static bool solveOrthogonality(const typename Traits::Matrix &W,
    ...
    const Scalar A = (b[1] * W(1, 2) - b[2] * W(1, 1)) * mu * mu +
                     b[0] * W(0, 2) - b[2] * W(0, 0);
    const Scalar B =
        (b[0] * W(1, 2) + b[1] * W(0, 2) - 2 * b[2] * W(0, 1)) * mu;
    const Scalar C = 2 * ((b[1] * W(2, 2) - b[2] * W(1, 2)) * mu * mu -
                          b[0] * W(0, 1) + b[1] * W(0, 0));
    const Scalar D =
        2 * (b[0] * (W(1, 1) - W(2, 2)) - b[1] * W(0, 1) + b[2] * W(0, 2)) * mu;
    ...
    Scalar roots[4];
    const unsigned nRoots = bogus::polynomial::getRealRoots(
        coeffs, roots, bogus::polynomial::AllRoots);
...
Scalar LocalSOCSolver<Dimension, Scalar, DeSaxceCOV, Strat>::solve(
    ...
    typename Traits::Vector &x, const Scalar mu, const Scalar tol,
  ...
  // Sliding case
  if (!AnalyticLocalSOCSolver<Dimension, Scalar,
                              DeSaxceCOV>::solveOrthogonality(A, b, x, mu)) {
    x = x0;
  }
