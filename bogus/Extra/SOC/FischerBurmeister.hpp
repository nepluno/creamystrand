...
namespace bogus {
    ...
    struct SimpleFBBaseFunction
    {
        ...
        static Scalar computeJacobian( const Scalar& x, const Scalar& y, Scalar& dFb_dx, Scalar& dFb_dy ) ;
        
        //! Computation of the FB function with its Hessian on the cone of aperture \p mu
        static Scalar computeHessian( const Scalar& x, const Scalar& y, Scalar& d2Fb_dx2, Scalar& d2Fb_dxy, Scalar& d2Fb_dy2 ) ;
        ...
        static Scalar compute( const Scalar& x, const Scalar& y, Scalar& dFb_dx, Scalar& dFb_dy ) ;
    ...
    //! Fischer-Burmeister function and jacobian computation for Herschel-Bulkley-type constraint, with optional change of variable
    /*! \tparam DeSaxceCOV whther to perform the DeSaxce change of variable. \sa SOCLaw */
    template< DenseIndexType Dimension, typename Scalar >
    ...
    public:
        ...
        //! Constructs an object modeling the function \f$ f : x \mapsto FB \left( mu, scaling \times x, A x + b \right) \f$
        HerschelBulkleyFischerBurmeister(
                                         const Scalar yield,
                                         const Scalar eta,
                                         const Scalar n,
                                         const Matrix& A,
                                         const Vector& b,
                                         const Scalar scaling )
        : m_yield( yield ), m_eta( eta ), m_n( n ), m_scaling( scaling ), m_A( A ), m_b( b )
        {}
        ...
        //! Sets \f$ fb :=  FB \left( mu, x, y \right) \f$
        static void compute( const Scalar yield, const Scalar eta, const Scalar n, const Scalar scaling, const Vector& x, const Vector& y, const Matrix& A, Vector& fb ) ;
        
        static Scalar computeEnergy( const Scalar yield, const Scalar eta, const Scalar n, const Scalar scaling, const Vector& x, const Vector& y ) ;
        
        static void computeJacobian( const Scalar yield, const Scalar eta, const Scalar n, const Scalar scaling, const Vector& x, const Vector& y, const Matrix& A, Matrix& fb ) ;
        
        static Scalar yieldFunction( const Scalar yield, const Scalar eta, const Scalar n, const Vector& x, const Vector& y );
    private:
        static void dxdr( const Scalar eta, const Scalar n, const Scalar scaling, const Vector& x, const Vector& y, const Matrix& A, Vector& fb );
        static void dydr( const Vector& y, const Matrix& A, Vector& fb );
        static void d2xdr2( const Scalar eta, const Scalar n, const Scalar scaling, const Vector& x, const Vector& y, const Matrix& A, Matrix& d2xdr2 );
        static void d2ydr2( const Vector& y, const Matrix& A, Matrix& d2ydr2 );
        static Scalar func( const Scalar yield, const Scalar eta, const Scalar n, const Scalar scaling, const Vector& x, const Vector& y );
