...
static double solve( const DualFrictionProblem< Dimension >& dual,
		...
		double *r, const bool staticProblem, const bool herschelBulkleyProblem )
{
    ...
    if(herschelBulkleyProblem)
    {
        res = gs.solve( typename DualFrictionProblem< Dimension >::HBLawType
                       ( dual.W.rowsOfBlocks(), dual.yields.data(), dual.etas.data(), dual.powers.data() ), dual.b, r_map );
    } else {
        res = staticProblem
...
static double eval( const DualFrictionProblem< Dimension >& dual,
		const ConstrainedSolverBase< Method, MatrixT > &gs,
		const double *r_data, const bool staticProblem, const bool herschelBulkleyProblem  )
{
    ...
    if(herschelBulkleyProblem)
    {
        res = gs.eval( typename DualFrictionProblem< Dimension >::HBLawType
                      ( dual.W.rowsOfBlocks(), dual.yields.data(), dual.etas.data(), dual.powers.data() ), u, r );
    } else {
        res = staticProblem
