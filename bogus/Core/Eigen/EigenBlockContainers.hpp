...
// We do not have to use the specialized allocator if the size is dynamic or not
...
struct ResizableSequenceContainer<
  ...
  typedef typename TypeSwapIf<
      Rows == Eigen::Dynamic || Cols == Eigen::Dynamic ||
          0 != (static_cast<std::size_t>(Rows * Cols * sizeof(Scalar)) & 0xf),
