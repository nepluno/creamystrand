...
class Lock {
 public:
  ...
  Lock(const Lock&) { init(); }

  Lock& operator=(const Lock&) { return *this; }

  ~Lock() { omp_destroy_lock(data()); }

  void set() const { m_impl.set(); }

  void unset() const { m_impl.unset(); }

  lock_impl::RawData* data() const { return &m_impl.data.api_type; }
