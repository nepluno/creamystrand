...
template <unsigned Dimension, typename Scalar>
unsigned RootsFinder<Dimension, Scalar>::getRealRoots(const Scalar *coeffs,
                                                      Scalar *realRoots,
                                                      RealRootsFilter filter) {
  typedef CompanionMatrix<Dimension, Scalar> CM;
  typename CM::ReturnType matrix = CM::get();

  matrix.template block<Dimension, 1>(0, Dimension - 1) =
      -Eigen::Matrix<Scalar, Dimension, 1>::Map(coeffs);
  const typename Eigen::EigenSolver<typename CM::BaseType>::EigenvalueType &ev =
      matrix.eigenvalues();
...
template <unsigned Dimension, typename Scalar>
unsigned PossiblyDegenerateRootsFinder<Dimension, Scalar>::getRealRoots(
    Scalar *coeffs, Scalar *realRoots, RealRootsFilter filter) {
  if (NumTraits<Scalar>::isZero(coeffs[Dimension])) {
    return PossiblyDegenerateRootsFinder<Dimension - 1, Scalar>::getRealRoots(
