...
// Standard shared_ptr -- BOGUS_SHARED_PTR_NS should be set to std, std::tr1 or
// boost
...
/*! There's no reason to use this implementation over any other one from boost,
  tr1, or c++11, except for c++98 compability. Fortunately it is only used for
  an edge case ( see EigenSparseLinearSolvers.hpp )

  \warning This class is *NOT* thread-safe. However, multiple NaiveSmartPointers
  pointing to the same shared instance should be able to be safely reset in
  parallel.
...
class NaiveSharedPtr {
 ...
 public:
  ...
  NaiveSharedPtr(const NaiveSharedPtr<T>& rhs) { add_ref(rhs); }
  ...
  operator bool() const { return BOGUS_NULL_PTR(T) != m_instance; }
 ...
 private:
  T* add_ref() const {
    ...
    if (instance && sync_add<1>() <= 1) {
      // Here m_refCount may already been deleted if another thread is
      // simultaneously releasing this object. We hope that the memory location
      // is still accessible and check for destruction But as I said previously,
