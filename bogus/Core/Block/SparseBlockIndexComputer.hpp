...
struct SparseBlockIndexGetter {
  ...
  static const ReturnType& getOrCompute(
...
struct SparseBlockIndexGetter<Derived, true> {
  ...
  static const ReturnType& getOrCompute(
      const MatrixType& matrix, typename MatrixType::UncompressedIndexType&) {
    return matrix.majorIndex();
  }
};
...
struct SparseBlockIndexComputer<MatrixType, ColWise, Transpose, true> {
  ...
  typedef CompoundSparseBlockIndex<typename MatrixType::MajorIndexType,
