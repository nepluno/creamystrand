...
struct BlockTraits {
  ...
  //! Type for storing the result of transpose_block( BlockType ), useful for
  //! cacheTranspose()
  typedef BlockType TransposeStorageType;

  enum {
    //! Number of rows spanned by a block at compile time ; useful for efficient
    //! segmentation
    RowsAtCompileTime = BlockType::RowsAtCompileTime,
    //! Number of cols spanned by a block at compile time ; useful for efficient
    //! segmentation
    ColsAtCompileTime = BlockType::ColsAtCompileTime,

    //! Can be set to true if "const Scalar* data_pointer( const BlockType& )"
    //! exist.
...
// Specialization of these structures should define a ReturnType if the
// operation is allowed

//! Defines the return type of an associated transpose_block( const BlockType& )
//! function
template <typename BlockType>
struct BlockTransposeTraits {};

//! Defines the type of the vectors resulting from the multiplication of a
//! BlockMatrix and an instance of VectorTypea
/*! Should be the return type of an associated get_mutable_vector( const
 * VectorType & ) function */
template <typename VectorType>
struct BlockVectorProductTraits {};

//! Defines the return type of the product of two blocks potentially transposed
template <typename LhsBlockType, typename RhsBlockType, bool TransposeLhs,
          bool TransposeRhs>
