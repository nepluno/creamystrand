...
struct bindings<double> {
  ...
  static void bsrmv(char *transa, MKL_INT *m, MKL_INT *k, MKL_INT *lb,
                    Scalar *alpha, char *matdescra, Scalar *val, MKL_INT *indx,
                    MKL_INT *pntrb, MKL_INT *pntre, Scalar *x, Scalar *beta,
                    Scalar *y) {
    mkl_dbsrmv(transa, m, k, lb, alpha, matdescra, val, indx, pntrb, pntre, x,
               beta, y);
  }
};
...
struct bindings< float >
{
        ...
        static void bsrmv (char *transa, MKL_INT *m, MKL_INT *k, MKL_INT *lb,
Scalar *alpha, char *matdescra, Scalar  *val, MKL_INT *indx,  MKL_INT *pntrb,
MKL_INT *pntre, Scalar *x, Scalar *beta, Scalar *y)
        {
...
template <typename Scalar>
void bsrmv(bool Symmetric, bool Transpose, MKL_INT Dimension, MKL_INT m,
           MKL_INT k, const std::size_t offset, const MKL_INT *rowIndex,
           const MKL_INT *columns, const Scalar *data, const Scalar *rhs,
  ...
  for (int i = 0; i < rhsCols; ++i) {
    bindings<Scalar>::bsrmv(&transa, &m, &k, &lb, &alpha, matdescra, a, indx,
                            pntrb, pntre, x + i * Dimension * k, &beta,
                            y + i * Dimension * k);
  }
}
...
static void rowmv(const SparseBlockIndex<true, MKL_INT, BlockPtr> &index,
                  ...
                  const typename BlockTraits<BlockType>::Scalar *rhs,
                  int rhsCols, typename BlockTraits<BlockType>::Scalar *res) {
  ...
  mkl::bsrmv<typename BlockTraits<BlockType>::Scalar>(
      false, false, BlockTraits<BlockType>::RowsAtCompileTime, 1,
      index.innerSize(), 0, rowIndex, columns,
      data_pointer(data[offset + index.base]), rhs, rhsCols, res, 1, 1);
}
...
struct SparseBlockMatrixOpProxy<true, true, double, MKL_INT> {
  ...
  static void multiply(const SparseBlockMatrixBase<Derived> &matrix,
    ...
    mkl::bsrmv<Scalar>(Traits::is_symmetric, Transpose, Derived::RowsPerBlock,
                       matrix.rowsOfBlocks(), matrix.colsOfBlocks(), 0,
                       matrix.majorIndex().rowIndex(),
                       matrix.majorIndex().columns(),
                       data_pointer(matrix.data()[0]), rhs.data(), rhs.cols(),
                       res.data(), alpha, beta);
  }
  ...
  static void splitRowMultiply(const SparseBlockMatrixBase<Derived> &matrix,
    ...
    mkl::rowmv(matrix.majorIndex(), matrix.data(), row, rhs.data(), rhs.cols(),
               res.data());

    if (Traits::is_symmetric) {
      mkl::rowmv(matrix.transposeIndex(), matrix.data(), row, rhs.data(),
                 rhs.cols(), res.data());
    }
