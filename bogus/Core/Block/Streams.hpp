...
std::ostream &operator<<(std::ostream &out,
                         const bogus::SparseBlockMatrixBase<Derived> &sbm) {
  typedef bogus::SparseBlockIndexComputer<Derived, false, false>
      IndexComputerType;
  ...
  out << " Total rows: " << sbm.rows() << " / cols: " << sbm.cols()
      << std::endl;
  for (unsigned i = 0; i < (unsigned)sourceIndex.outerSize(); ++i) {
    out << "Row " << i << ": ";
    for (typename SourceIndexType::InnerIterator it(sourceIndex, i); it; ++it) {
      out << " " << it.inner() << "@" << it.ptr() << "; ";
    }
    out << std::endl;
  }
