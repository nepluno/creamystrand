...
template <typename Matrix, typename Vector, typename RhsT, typename ResT,
          typename Scalar>
Scalar init(const Matrix &A, const RhsT &b, ResT x, Vector &r0, Scalar &scale) {
  r0 = b;
  r0.noalias() -= A * x;
...
template <bool DoTranspose, typename Preconditioner, typename RhsT,
          typename ResT>
void apply(const Preconditioner *P, const RhsT &b, ResT &x) {
  if (P)
    P->template apply<DoTranspose>(b, x);
  else
    x = b;
}
...
typename CG<Mat, Prec, Traits>::Scalar CG<Mat, Prec, Traits>::vectorSolve(
...
template <typename Mat, typename Prec, typename Traits>
template <typename RhsT, typename ResT>
typename BiCG<Mat, Prec, Traits>::Scalar BiCG<Mat, Prec, Traits>::vectorSolve(
    const RhsT &b, ResT x) const {
  typedef typename Traits::template MutableClone<RhsT>::Type Vector;
  Vector r;
...
BiCGSTAB<Mat, Prec, Traits>::vectorSolve(const RhsT &b, ResT x) const {
  ...
  Vector p = nu;
  Vector s;
  Vector t(m_A->rows()), y(m_A->rows()), z(m_A->rows());
...
template <typename Mat, typename Prec, typename Traits>
template <typename RhsT, typename ResT>
typename CGS<Mat, Prec, Traits>::Scalar CGS<Mat, Prec, Traits>::vectorSolve(
    const RhsT &b, ResT x) const {
  typedef typename Traits::template MutableClone<RhsT>::Type Vector;
  Vector r;
  ...
  Vector u = r, p = r, q;
  Scalar rho1, rho0 = 1, alpha, beta;

  Vector y(m_A->rows()), nu(m_A->rows());

  for (unsigned k = 0; k < m_maxIters; ++k) {
    rho1 = r0h.dot(r);

    if (NumTraits<Scalar>::isZero(rho1)) break;
...
template <typename Mat, typename Prec, typename Traits>
template <typename RhsT, typename ResT>
typename GMRES<Mat, Prec, Traits>::Scalar GMRES<Mat, Prec, Traits>::vectorSolve(
    const RhsT &b, ResT x) const {
  typedef typename Traits::template MutableClone<RhsT>::Type Vector;
  ...
  do {
    ...
    for (k = 0; k < m && res >= m_tol; ++k) {
      ...
      if (vhn > NumTraits<Scalar>::epsilon())  // If vhn is zero, the algorithm
                                               // shall stop at this step
        V.col(k + 1) /= vhn;
      ...
      U.col(k).head(k + 1) =
          O.topLeftCorner(k + 1, k + 1) * H.col(k).head(k + 1);
      U(k + 1, k) = vhn;
      ...
      //			std::cout << " ==== Iteration " << globalIter <<
      //" + " << k <<std::endl
      //			          << "H" << std::endl
      //			          << H.topLeftCorner( k+2, k+1 )<<
      //std::endl
      //			          << "O" << std::endl
      //			          << O.topLeftCorner( k+2, k+2 )<<
      //std::endl
      //			          << "U" << std::endl
      //			          << U.topLeftCorner( k+2, k+1 )<<
      //std::endl
      //			          << "V" << std::endl
      //			          << V.leftCols( k+2 ) << std::endl
      //			          << "Orthogonality" << std::endl
      //			          << ( O.topLeftCorner(k+2,k+2) *
      //O.topLeftCorner(k+2,k+2).transpose() - Matrix::Identity( k+2, k+2 )
      //).squaredNorm() << std::endl
      //			          << "Equality" << std::endl
      //			          << ( O.topLeftCorner(k+2,k+2) *
      //H.topLeftCorner(k+2,k+1) - U.topLeftCorner(k+2,k+1) ).squaredNorm()<<
      //std::endl
      //			          << "Solve" << std::endl
      //			          << ( U.topLeftCorner( k+1, k+1 )*y -
      //g.segment(0,k+1) ).transpose()<< std::endl
      //			          << "res" << std::endl
      //			          << g(k+1) << std::endl
      //			          << ( m_A*x - b ).norm()
      //			          << std::endl ;
    ...
    if (res < m_tol || globalIter >= m_maxIters) break;
...
template <typename Mat, typename Prec, typename Traits>
template <typename RhsT, typename ResT>
typename TFQMR<Mat, Prec, Traits>::Scalar TFQMR<Mat, Prec, Traits>::vectorSolve(
    const RhsT &b, ResT x) const {
  typedef typename Traits::template MutableClone<RhsT>::Type Vector;
  Vector r;
  ...
  Vector d(m_A->rows()), v(m_A->rows()), nu(m_A->rows()), y(m_A->rows());
