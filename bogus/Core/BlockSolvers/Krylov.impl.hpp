...
template <typename BlockMatrixType,
          template <typename BlockMatrixT> class PreconditionerType>
Krylov<BlockMatrixType, PreconditionerType>::Krylov()
    : Base(BOGUS_NULL_PTR(const BlockObjectBase<BlockMatrixType>), 100,
...
Krylov<BlockMatrixType, PreconditionerType>::solve(
    const RhsT &b, ResT &x, krylov::Method method) const {
  switch (method) {
#define BOGUS_PROCESS_KRYLOV_METHOD(MethodName) \
  case krylov::MethodName:                      \
    return solve_##MethodName(b, x);
