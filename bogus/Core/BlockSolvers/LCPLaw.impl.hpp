...
void LCPLaw<Scalar>::projectOnConstraint(const unsigned,
                                         typename Traits::Vector &x) const {
  x[0] = std::max(x[0], 0.);
}

template <typename Scalar>
Scalar LCPLaw<Scalar>::eval(const unsigned, const typename Traits::Vector &x,
...
template <typename Scalar>
bool LCPLaw<Scalar>::solveLocal(const unsigned,
