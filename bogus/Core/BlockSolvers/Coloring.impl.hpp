...
void Coloring::compute(const SparseBlockMatrixBase<Derived>& matrix) {
  // Affects each contact to the first color with which it does not have any
  // interaction This is not optimimal, but optimality would be costly
  ...
  for (std::ptrdiff_t i = 0; i < n; ++i) {
    ...
    for (unsigned c = 0; c < nColors; ++c) {
      ...
      ok = true;
      for (typename Derived::MajorIndexType::InnerIterator it(
               matrix.majorIndex(), i);
           it && it.inner() < i; ++it) {
        if (color.end() !=
  ...
  for (unsigned c = 0; c < tmp_colors.size(); ++c) {
    ...
    permutation.insert(permutation.end(), tmp_colors[c].begin(),
                       tmp_colors[c].end());
  }
}
...
template <typename Derived>
void Coloring::update(const bool enable,
